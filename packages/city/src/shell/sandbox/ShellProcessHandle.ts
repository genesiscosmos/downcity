...
import { spawn as spawn_pty } from "node-pty";
 ...
 */
export function create_pipe_process_handle(
  child: ChildProcess,
): ShellProcessHandle {
 ...
 */
export function create_pty_process_handle(pty: IPty): ShellProcessHandle {
  let writable = true;
  return {
    pid: pty.pid,
    get writable() {
 ...
 */
export function spawn_pty_process_handle(params: {
  command: string;
  args: string[];
  cwd: string;
  ...
  const pty = spawn_pty(params.command, params.args, {
    name: "xterm-256color",
    cwd: params.cwd,
    env,
  ...
  return create_pty_process_handle(pty);
}
