...
import type { ResolvedSandboxPolicy } from "@downcity/type/shell";
...
export async function resolve_sandbox_policy(
  ...
  const configured_root_path = path.resolve(context.root_path);
  const root_path = await fs.realpath(configured_root_path).catch(() => configured_root_path);
  const sandbox_dir = path.join(path.resolve(context.data_path), "sandbox");
...
export function resolve_sandbox_cwd(
  ...
  const root_path = path.resolve(context.root_path);
  const normalized_cwd = path.resolve(requested_cwd);
  if (is_path_inside_root(root_path, normalized_cwd)) return normalized_cwd;
  context.logger?.warn("[sandbox] cwd escapes project root and was reset to root_path", {
    root_path: root_path,
