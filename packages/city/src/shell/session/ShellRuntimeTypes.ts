...
import type { ResolvedShellRuntimeOptions } from "@downcity/type/shell";
...
export type ShellSessionRuntimeState = {
   ...
   */
  write_chain: Promise<void>;
  /**
   * 终态后的延迟清理定时器。
   */
  cleanup_timer: NodeJS.Timeout | null;
  /**
   ...
   */
  completion_promise: Promise<void>;
  /**
   * 当前 shell 进入终态时触发的完成回调。
   */
  resolve_completion: () => void;
};
