/**
 * ActionSchedule：power action 延迟执行能力的共享类型。
 *
 * 关键点（中文）
 * - 这里描述的是“某个 power action 在未来某个时间执行”的通用记录。
 * - 它不是独立 Power，也不表达业务语义，只服务于 Power Action 调度。
...
export interface CreateActionScheduleJobInput {
  /** 任务执行时解析 PowerContext 所需的 Workspace 稳定标识；省略时使用当前活动 Workspace。 */
  workspace_id: string;
  /**
   * 目标 power 名称。
   */
...
/**
 * 统一 power action 延迟执行输入。
 */
export interface PowerActionScheduleInput {
  /**
   * 计划执行时间（毫秒时间戳）。
   */
  run_at_ms: number;
