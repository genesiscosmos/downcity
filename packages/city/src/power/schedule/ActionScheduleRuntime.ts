/**
 * ActionSchedule Runtime：power action 延迟执行的轮询运行时。
 *
 * 关键点（中文）
 * - ActionSchedule 不是 power，因此不出现在 power 列表、状态和 lifecycle 中。
 * - 这里只跟随 Agent 长期运行生命周期启动，用于消费持久化的 action schedule 任务。
 * - 普通 power 先启动，ActionSchedule 再开始轮询，避免到期 action 执行到未启动的 power。
 */

import type { AgentStorage } from "@downcity/agent";
import type { PowerContext } from "@/power/index.js";
import type { Logger } from "@downcity/agent";
import { run_due_action_schedule_jobs } from "@/power/schedule/ActionScheduleExecutor.js";
import { ActionScheduleStore } from "@/power/schedule/ActionScheduleStore.js";

const ACTION_SCHEDULE_POLL_INTERVAL_MS = 1_000;
const ACTION_SCHEDULE_LOG_PREFIX = "[ACTION_SCHEDULE]";

function formatActionScheduleLogMessage(message: string): string {
  return `${ACTION_SCHEDULE_LOG_PREFIX} ${message}`;
}

/**
 * ActionSchedule runtime 停止句柄。
 */
export interface ActionScheduleRuntimeHandle {
  /**
   * 停止轮询并释放持久化存储资源。
   */
  stop(): void;
}

/**
 * 启动 ActionSchedule 轮询 runtime。
 */
export async function start_action_schedule_runtime(
  agent_id: string,
  storage: AgentStorage,
  logger: Logger,
  resolve_context: (workspace_id?: string) => PowerContext | null,
): Promise<ActionScheduleRuntimeHandle> {
  const store = new ActionScheduleStore(
    storage.files,
    storage.root_path,
    agent_id,
  );
  const recovered = await store.reset_running_jobs_to_pending();
  if (recovered > 0) {
    logger.warn(
      formatActionScheduleLogMessage("Recovered interrupted running jobs"),
      { recovered },
    );
  }

  let ticking = false;
  let stopped = false;
  let timer: NodeJS.Timeout | null = null;

  const tick = async (): Promise<void> => {
    if (stopped || ticking) return;
    ticking = true;
    try {
      await run_due_action_schedule_jobs({
        store,
        resolve_context,
        logger,
      });
    } finally {
      ticking = false;
    }
  };

  await tick();
  timer = setInterval(() => {
    void tick();
  }, ACTION_SCHEDULE_POLL_INTERVAL_MS);

  logger.info(formatActionScheduleLogMessage("Runtime started"));

  return {
    stop: () => {
      if (stopped) return;
      stopped = true;
      if (timer) {
        clearInterval(timer);
        timer = null;
      }
      store.close();
      logger.info(formatActionScheduleLogMessage("Runtime stopped"));
    },
  };
}
