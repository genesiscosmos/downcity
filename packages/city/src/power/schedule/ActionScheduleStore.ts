 ...
 *
 * 关键点（中文）
 * - ActionSchedule 是 power action 的延迟执行能力，不是一个独立 power。
...
} from "@/power/schedule/ActionSchedule.js";
import { generate_id } from "@downcity/agent";
import { get_downcity_schedule_db_path } from "@/power/schedule/ActionSchedulePath.js";
import type { FileSystem } from "@downcity/type/workspace";
...
function normalizeJobRecord(
  ...
  return {
    ...
    power_name: String(input.power_name || "").trim(),
