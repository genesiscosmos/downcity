...
/** 归一化 ASR 输入。 */
export function normalize_asr_input(payload: PowerJsonValue | undefined): SoundAsrInput {
  const record = to_record(payload ?? {});
...
/** 归一化 TTS 输入。 */
export function normalize_tts_input(payload: PowerJsonValue | undefined): SoundTtsInput {
  const record = to_record(payload ?? {});
...
/** 归一化 JSON 对象。 */
function normalize_json_object(value: unknown): PowerJsonObject | undefined {
  const record = to_record(value);
  return record ? record as PowerJsonObject : undefined;
}
