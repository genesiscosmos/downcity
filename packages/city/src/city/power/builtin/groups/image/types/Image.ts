...
export interface ImageAiService {
  ...
  /** 创建图片生成任务。 */
  image_create(input: PowerJsonObject): Promise<unknown>;

  /** 查询图片生成任务。 */
  image_result(input: PowerJsonObject): Promise<unknown>;
}
...
export interface ImageCreateInput {
  ...
  provider_options?: PowerJsonObject;
  /** 允许调用方透传其他 JSON 可序列化参数。 */
  [key: string]: PowerJsonValue | ImageContent[] | undefined;
}
...
export interface ImageResolvedCreateInput {
  ...
  provider_options?: PowerJsonObject;
  /** 允许透传其他 JSON 可序列化参数。 */
  [key: string]: PowerJsonValue | ImageResolvedMessage[] | undefined;
}
