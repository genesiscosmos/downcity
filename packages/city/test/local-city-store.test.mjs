/**
 * LocalCityStore SQLite 配置装配测试。
 *
 * 测试只验证配置关系和 City 装配，不调用远程模型或 Plugin 生命周期。
 */
...
test("LocalCityStore 提供配置并由 City 装配 Agent", async () => {
  ...
  store.create_agent_config({ agent_id: "test_agent", execution: {} });
  store.bind_agent_workspace("test_agent", workspace.workspace_id);
  const city = new City(store, new LocalCityEnvironment({
    root_path,
    data_source: store,
  }));
  try {
    await city.ready();
    assert.equal(city.agent("test_agent")?.id, "test_agent");
    assert.equal(city.agent("test_agent")?.workspace.path, await fs.realpath(workspace_path));
  } finally {
...
test("LocalCityStore 按固定优先级合并 env 并过滤平台身份变量", async () => {
  ...
  try {
    ...
    const environment = new LocalCityEnvironment({ root_path, data_source: store });
    const env = environment.reload_workspace_env(workspace_path);
...
test("LocalCityStore 只提供宿主显式选择的 Agent 配置", async () => {
  ...
  for (const agent_id of ["first_agent", "second_agent"]) {
    setup_store.create_agent_config({ agent_id, execution: {} });
    setup_store.bind_agent_workspace(agent_id, workspace.workspace_id);
  }
  setup_store.close();

  const scoped_store = new LocalCityStore({ root_path, agent_ids: ["second_agent"] });
  const city = new City(scoped_store, new LocalCityEnvironment({
    root_path,
    data_source: scoped_store,
  }));
  await city.ready();
