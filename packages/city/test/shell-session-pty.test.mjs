...
test("shell_exec defaults to a ten-minute total timeout", () => {
  assert.equal(create_shell_runtime_state().options.default_exec_timeout_ms, 600_000);
});
...
test("shell_session uses PTY while shell_exec stays non-interactive", async () => {
  ...
  const state = create_shell_runtime_state({
    default_inline_wait_ms: 80,
    default_exec_timeout_ms: 2000,
  });
  try {
    const detect_tty_cmd = "[ -t 1 ] && printf tty || printf notty";
    const exec_result = await exec_shell_command(state, fixture.context, {
      cmd: detect_tty_cmd,
    ...
    const session_result = await start_shell_session(state, fixture.context, {
      cmd: detect_tty_cmd,
      cwd: fixture.root_path,
      shell: "/bin/sh",
      login: false,
    ...
    const observed_result = session_result.chunk?.output
      ? session_result
      : await wait_shell_session(state, fixture.context, {
          shell_id: session_result.shell.shell_id,
          after_version: session_result.shell.version,
          from_cursor: 0,
          timeout_ms: 1000,
        });
...
test("shell_exec honors an explicit short total timeout", async () => {
  ...
  try {
    await assert.rejects(
      exec_shell_command(state, fixture.context, {
        cmd: "sleep 0.2",
        cwd: fixture.root_path,
        shell: "/bin/sh",
        login: false,
...
test("shell_exec timeout waits for the process exit event before returning", async () => {
  ...
  const state = create_shell_runtime_state({
    default_inline_wait_ms: 10,
    default_exec_timeout_ms: 20,
    min_wait_ms: 1,
  });
  try {
    await assert.rejects(
...
test("shell_exec closes stdin so commands waiting for EOF can exit", async () => {
  ...
  try {
    const result = await exec_shell_command(state, fixture.context, {
      ...
      tool_call_id: "call-stdin-eof",
    });
    assert.equal(result.shell.status, "completed");
    assert.equal(result.shell.stdin_writable, false);
...
test("unrestricted shell without an Approval Gateway is denied before execution", async () => {
  ...
  assert.equal(result.shell.approval_status, "denied");
...
test("unrestricted shell waits for the injected Approval Gateway before execution", async () => {
  ...
  const execution = exec_shell_command(state, fixture.context, {
