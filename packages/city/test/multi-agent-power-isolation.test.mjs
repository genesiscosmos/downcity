/**
 * @file 验证多个 Agent 经 session.prompt 执行 power_call 时保持 registry 隔离。
 *
 * 两个 CityModel 并发进入原生 Downcity Model Protocol 工具循环，第二次模型调用必须只
 * 收到各自 Agent power action 返回的 owner。
 */
...
/** 返回一次 skill power tool call。 */
function create_power_call_stream(model_id) {
  // 工具名即 power 名；动作 id 与参数放在同一个工具输入里。
  const tool_name = "skill";
  const tool_input = JSON.stringify({ action: "lookup", args: {} });
  const call_id = `call_${model_id}`;
  return {
    stream: new ReadableStream({
      start(controller) {
        controller.enqueue({ type: "stream-start", warnings: [] });
        controller.enqueue({ type: "tool-input-start", id: call_id, toolName: tool_name });
...
function create_test_model(model_id, model_requests) {
  ...
  const language_model = new MockModelClient({
    ...
    doStream: async (options) => {
      ...
      return request_count === 1
        ? create_power_call_stream(model_id)
  ...
  return Object.assign(language_model, {
    id: model_id,
    name: model_id,
    description: "Multi-agent power isolation model",
...
/** 创建通过动态 Context 返回当前 Agent ID 的共享 skill power。 */
function create_owner_power(executed_owners) {
  return create_power({
    name: "skill",
    title: "Shared Skill",
...
test("multiple session prompts use only their owning Agent power registry", async () => {
  const model_requests = new Map();
  const executed_owners = [];
  const root_a = await fs.mkdtemp(path.join(os.tmpdir(), "downcity-power-isolation-a-"));
  const root_b = await fs.mkdtemp(path.join(os.tmpdir(), "downcity-power-isolation-b-"));
  ...
  const registration = create_power_registration(create_owner_power(executed_owners));
  ...
  try {
    ...
    const [turn_a, turn_b] = await Promise.all([
      session_a.prompt({ query: "Call your skill power" }),
      session_b.prompt({ query: "Call your skill power" }),
    ]);
