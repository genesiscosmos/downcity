...
function create_fixture(initial_messages = []) {
  ...
  const store = {
    ...
    create_message: async (build_message) => {
      const message = build_message({
        ...
        messages: structuredClone(stored_messages),
      });
      stored_messages.push(structuredClone(message));
      return structuredClone(message);
    },
    update_message: async (message_id, expected_revision, build_message) => {
      const index = stored_messages.findIndex((candidate) => candidate.message_id === message_id);
  ...
  return {
    messages,
    publications,
    read_active: () => structuredClone(stored_messages.filter((message) => message.role !== "agent" || message.status !== "streaming")),
    read_draft: () => structuredClone(stored_messages.find((message) => message.role === "agent" && message.status === "streaming") ?? null),
  };
}
