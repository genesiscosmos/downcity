...
function create_user_message(state) {
  return {
    ...
    created_at: 10,
    updated_at: 10,
    parts: [{ part_id: "part-user-1", sequence: 1, type: "text", text: "你好", state: "done" }],
  };
}
...
test("Message 与 Part 分表原子保存并刷新列表投影", async () => {
  ...
  try {
    ...
    assert.deepEqual(Object.keys(JSON.parse(part.content)).sort(), ["state", "text"]);
  } finally {
    database.close();
  }

  await assert.rejects(storage.update_message("user-1", 2, (message) => message), /revision conflict/);
  const updated = await storage.update_message("user-1", 1, (message) => ({
    ...message,
    revision: 2,
    updated_at: 20,
    parts: [{ ...message.parts[0], text: "你好，SQLite" }],
  }));
...
test("重新打开时收口全部中断状态并同步预览", async () => {
  ...
  const message = await reopened.read_message("agent-1");
  assert.equal(message.status, "stopped");
  assert.equal(message.parts[0].state, "done");
  assert.equal(message.parts[1].state, "failed");
  assert.equal(message.parts[2].state, "failed");
...
test("Composer Policy 只能写自己的派生 namespace，clear 触发 FK 清理", async () => {
  ...
  await assert.rejects(policy.transaction((transaction) => transaction.execute("ALTER TABLE messages ADD COLUMN invalid TEXT")), /can only mutate/);
