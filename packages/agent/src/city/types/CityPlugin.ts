...
export interface AgentCityExtensionBinding {
  ...
  subscribe(subscriber: (change: {
    /** 变化类型。 */ readonly type: "register" | "unregister";
    /** Plugin 稳定 ID。 */ readonly plugin_name: string;
    /** 是否属于 Agent 加入 City 时的初始装配。 */ readonly initial: boolean;
  }) => void): () => void;
