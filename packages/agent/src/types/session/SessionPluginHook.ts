/**
 * Session 暴露给现有 Plugin Hook 系统的值协议。
 *
 * 关键点（中文）
 * - Session 只定义通用检查点和值结构，不包含任何 Memory 领域概念。
 * - Hook 值只包含可序列化快照，Plugin 不能持有或修改 canonical Session 状态。
...
/** Plugin 在当前 Turn 中返回的一条低权限动态参考内容。 */
export interface SessionPluginContextBlock {
...
export interface SessionPluginUserMessage {
...
export interface SessionTurnContextHookValue {
  ...
  user_messages: SessionPluginUserMessage[];

  /** Pipeline 当前累计的低权限动态内容块。 */
  blocks: SessionPluginContextBlock[];
}
