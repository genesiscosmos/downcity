 ...
 *
 * 关键点（中文）
 * - Agent 与 Session 对外统一接收 Downcity ModelClient。
 * - CityModel 自身实现 ModelClient，executor 可以直接调用。
 ...
 * Agent SDK 可接受的模型输入。
 */
export type AgentModel = ModelClient;

/**
 * 将 Agent 可接受的模型输入归一为 Downcity ModelClient。
 */
export function normalize_agent_model(model: AgentModel): ModelClient {
  return model;
}
 ...
 *
 * 只有 CityModel 携带可信的模型目录元数据；其它 ModelClient 未声明时返回空。
 */
