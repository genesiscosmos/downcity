...
TaskPower owns one City-level Task store and scheduler. Each Task explicitly binds one rebindable execution Agent and Workspace, and can optionally bind a fixed Agent/Workspace/Session delivery target. Rebinding execution does not move result delivery. Definition mutations commit serially, one-shot completion updates only the latest matching definition, and a running Task cannot be deleted.
