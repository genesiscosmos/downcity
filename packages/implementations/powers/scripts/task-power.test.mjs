/**
 * Task Power 统一存储与生命周期 scheduler 回归测试。
 *
 * 覆盖 Task 定义的 City 级唯一事实源、Agent 投影隔离，以及 Power initialize
...
import { TaskPower } from "../bin/task.js";
import {
...
/** 把测试实例包装为 City 持有的统一 Power 注册。 */
function create_task_registration(power) {
  return {
    id: "task",
    title: "Task",
    description: "Task test power",
...
/** 从 City Storage scope 创建 Task Store 使用的 PowerStorage。 */
function create_power_storage(provider) {
  const scope = provider.open_scope(["powers", "task"]);
...
function create_lifecycle_context(storage, invocations) {
  return {
    power: { id: "task", action() {}, config_action() {} },
    storage,
...
test("scheduler 从统一 Store 注册全部 Agent/Workspace 的 Task", async () => {
  ...
  const storage = create_power_storage(new LocalStorageProvider(data_path));
...
test("scheduler dispose 会等待已进入的触发回调", async () => {
  const storage = create_power_storage(new MemoryStorageProvider());
  const definitions_repository = new TaskDefinitionRepository(
    storage,
    new TaskExecutionCoordinator(),
  );
  ...
  context.system.invoke_agent_power = async () => {
...
test("TaskPower scheduler 释放失败时仍等待运行并清空生命周期引用", async () => {
  const power = new TaskPower();
  ...
  power.lifecycle_context = { marker: true };
  power.definitions = { marker: true };
  assert.equal(
    power.executions.start("dispose-running-task", async () => await execution_released),
    true,
  );
  let disposal_finished = false;
  const disposing = power.dispose()
    .then(() => undefined)
    .catch((error) => error)
    .finally(() => { disposal_finished = true; });
  await new Promise((resolve) => setTimeout(resolve, 0));
  ...
  assert.equal(power.lifecycle_context, undefined);
  assert.equal(power.definitions, undefined);
});
...
test("TaskPower initialize 无需 Session 调用即可恢复已有 schedule", async () => {
  ...
  const task_storage = create_power_storage(storage_provider);
  const definitions_repository = new TaskDefinitionRepository(
    task_storage,
    new TaskExecutionCoordinator(),
  );
  ...
  await createTaskDefinition({
    ...
    request: {
      title: "restart-task",
      description: "验证 Power 初始化恢复 schedule",
  ...
  try {
    await city.powers.add(create_task_registration(new TaskPower()));
    ...
    assert.equal(
      await fs.stat(path.join(city_data_path, "agents", agent.id, "powers", "task", "tasks"))
        .then(() => true)
        .catch(() => false),
      false,
    );
...
test("统一 Store 仍按 Task 的 agent_id 向 Agent 投影", async () => {
  ...
  try {
    await city.powers.add(create_task_registration(new TaskPower()));
    const scope_a = city.powers.scope({ agent_id: agent_a.id, workspace_id: workspace.id });
    const scope_b = city.powers.scope({ agent_id: agent_b.id, workspace_id: workspace.id });
    ...
    const [list_a, list_b] = await Promise.all([
      scope_a.run_action({ power: "task", action: "list", payload: {} }),
      scope_b.run_action({ power: "task", action: "list", payload: {} }),
    ]);
...
test("Task 定义通过 Storage 文件端口支持 City 默认内存存储", async () => {
  ...
  try {
    await city.powers.add(create_task_registration(new TaskPower()));
    const scope = city.powers.scope({ agent_id: agent.id, workspace_id: workspace.id });
    ...
    assert.equal(created.success, true);
    const listed = await scope.run_action({ power: "task", action: "list", payload: {} });
...
test("Task 重新绑定执行 Agent 后仍向原始 Agent Session 交付结果", async () => {
  ...
  try {
    ...
    await city.powers.add(create_task_registration(new TaskPower()));
    await city.powers.invoke("task", "tasks.update", {
      agent_id: agent_b.id,
      workspace_id: workspace.id,
    ...
    const scope = city.powers.scope({ agent_id: agent_b.id, workspace_id: workspace.id });
