/**
 * SkillPower action 目录与动态 system prompt 回归测试。
 *
 * 关键点（中文）
...
import { SkillPower } from "../bin/index.js";

/** 创建 Skill 测试所需的公开 PowerContext。 */
...
test("SkillPower 暴露四个职责清晰的 actions", () => {
  const power = new SkillPower();
...
test("SkillPower 根据扫描参数生成 action 工作流提示", async () => {
  ...
  const prompt = await power.system(create_skill_context(project_root));
...
test("find 和 install actions 只返回提示词且不创建扫描目录", async () => {
  ...
  const find_result = await power.actions.find.execute({
    context: create_skill_context(project_root),
    input: { query: "web access" },
  });
  const install_result = await power.actions.install.execute({
    context: create_skill_context(project_root),
...
test("install action 在没有扫描根时返回配置提示", async () => {
  ...
  const power = new SkillPower({ use: [] });
  const result = await power.actions.install.execute({
    context: create_skill_context(project_root),
