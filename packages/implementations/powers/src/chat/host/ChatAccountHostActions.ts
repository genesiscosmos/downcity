...
/** 注册 Chat Power 的 Desktop 业务 Actions。 */
...
/** 把结构化 View 转为 Power JSON。 */
function as_json(value: unknown): PowerJsonValue {
  return value as PowerJsonValue;
}

/** 读取 JSON Object。 */
function read_object(input: PowerJsonValue | undefined): PowerJsonObject {
...
/** 读取一个必填 ID。 */
function read_required_id(input: PowerJsonValue | undefined, key: string): string {
  return read_required_string(read_object(input), key);
}

/** 读取一个必填字符串。 */
function read_required_string(source: PowerJsonObject, key: string): string {
...
/** 读取布尔字段。 */
function read_boolean(input: PowerJsonValue | undefined, key: string): boolean {
  return read_object(input)[key] === true;
}

/** 读取 Chat Access Scope。 */
function read_scope(value: PowerJsonValue | undefined): "direct" | "group" | "all" {
...
function read_optional_scope(
  value: PowerJsonValue | undefined,
