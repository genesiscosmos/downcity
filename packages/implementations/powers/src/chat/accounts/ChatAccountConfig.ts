...
/** 把 Power Config 解析成经过完整校验的 Account 配置。 */
export function read_chat_accounts_config(value: PowerJsonObject): ChatAccountsConfig {
...
export function create_chat_account(
  ...
  draft_value: PowerJsonValue | undefined,
...
/** 把 Account 配置转成 Power Config JSON。 */
export function serialize_chat_accounts_config(config: ChatAccountsConfig): PowerJsonObject {
  return structuredClone(config) as unknown as PowerJsonObject;
}

/** 校验一条已经持久化的 Account。 */
function normalize_persisted_account(value: PowerJsonValue): ChatAccountConfig {
...
function read_account_draft(value: PowerJsonValue | undefined): ChatAccountDraft {
...
/** 要求 JSON 值是普通对象。 */
function as_object(value: PowerJsonValue | undefined, label: string): PowerJsonObject {
...
/** 读取支持的平台类型。 */
function read_provider(value: PowerJsonValue | undefined): ChatProvider {
...
/** 读取一个 JSON 字符串字段。 */
function read_string(source: PowerJsonObject, key: string): string {
