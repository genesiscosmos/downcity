/** Chat Power 面向 Agent 的严格会话内 Actions。 */

import { create_action } from "@downcity/city/power";
import type { PowerActions } from "@downcity/city/power";
import { z } from "zod";
...
export function create_chat_agent_actions(
  ...
  return ({
    context: create_action({
      ...
      returns:
        "conversation_id, account_id, external_chat_id, chat_type, thread_id, title, "
        + "agent_id, workspace_id, session_id, status, last_message_at",
      access: "read",
      ...
      execute: async ({ context, execution, input }) => {
        ...
        return { success: true, data: conversation as unknown as import("@downcity/city/power").PowerJsonValue };
      },
    }),
    list: create_action({
      description: "List external chat conversations owned by the current Agent.",
      returns:
        "conversations(conversation_id, external_chat_id, chat_type, title, session_id, status, last_message_at)",
      access: "read",
      input_schema: z.object({}).strict(),
      execute: async ({ context }) => ({
        success: true,
        data: resolve_runtime().store.list_agent_conversations(context.agent.id) as unknown as import("@downcity/city/power").PowerJsonValue,
      }),
    }),
    history: create_action({
      description: "Read canonical Agent Session messages for a Chat conversation.",
      returns: "messages(canonical Session messages in order)",
      access: "read",
      input_schema: session_input,
      execute: async ({ context, execution, input }) => {
        const session_id = input.session_id || execution.snapshot.session_id || "";
    ...
    send: create_action({
      ...
      returns: "delivery_id, status, session_id",
      access: "write",
