...
import type {
  ...
  ChatPowerAttachment,
} from "@/chat/types/ChatPower.js";

/**
 * 自动转写使用的 power 与动作 id。
 *
 * 关键点（中文）
 * - sound 不是独立 power：它的动作属于 City 自有 power `city`，
 *   对模型与程序化调用都暴露为 `sound.asr` 这个点号动作 id。
 */
const CITY_POWER_ID = "city";
const SOUND_ASR_ACTION_ID = "sound.asr";
...
function to_display_src(root_path: string, attachment: ChatPowerAttachment): string {
...
export async function transcribe_inbound_voice(input: {
  ...
  const powers = input.context.city.powers;
  if (!powers.get(CITY_POWER_ID)) return input.inbound;
  ...
  for (const attachment of voice_attachments) {
    try {
      const result = to_record((await powers.run_action({
        power: CITY_POWER_ID,
        action: SOUND_ASR_ACTION_ID,
        payload: {
