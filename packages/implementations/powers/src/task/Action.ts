...
function resolveTaskStatus(input: PowerJsonValue | undefined, fallback: ShipTaskStatus): ShipTaskStatus {
...
function buildDefaultTaskBody(): string {
  return [
    ...
    "- 如果任务需要通知外部渠道，由执行任务的 Agent 自己调用 chat power。",
    "",
    "# 触发与状态建议",
    "",
    ...
    "- 任务运行记录由 Task Power 统一保存；需要交付给用户的文件应明确写入项目目录。",
    "- 任务完成结果写入关联 Session 的 assistant 消息，不会替 Agent 调用 chat power。",
    "",
  ].join("\n");
}
...
export async function createTaskDefinition(params: {
  ...
  /** 由 Power 调用上下文捕获的固定结果交付 Session。 */
