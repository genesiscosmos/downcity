 ...
 * Agent/Workspace PowerContext。真正触发时才按照 Task 定义中的 agent_id 和
...
export class TaskSchedulerCoordinator {
  ...
  /** 已进入 TaskPower 的定时触发回调，dispose 必须等待其收口。 */
  ...
  constructor(
    /** TaskPower 加入 City 后获得的稳定生命周期上下文。 */
    private readonly context: PowerLifecycleContext,
    /** cron 表达式使用的 IANA 时区。 */
    private readonly timezone: string,
    /** TaskPower 唯一的定义事务入口。 */
    private readonly definitions: TaskDefinitionRepository,
    /** 当前 Power 实例唯一的 timer engine；测试可以注入确定性实现。 */
   ...
   *
   * Agent 与 Workspace 可以晚于 Power 注册；这里只登记 timer，触发时再解析执行目标。
   */
  ...
  private async execute(task_id: string, trigger: ShipTaskRunTriggerV1): Promise<void> {
    try {
      ...
      const raw_result = await this.context.system.invoke_agent_power({
