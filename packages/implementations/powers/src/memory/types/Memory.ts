/**
 * Memory Power 的公开领域协议。
 *
 * 关键点（中文）
 * - MemoryPower 只依赖 MemoryProvider，不感知文件、数据库或远程服务。
...
export interface MemoryDigestInput {
  ...
  /** 由 MemoryPower 从 canonical Session 提取的文本。 */
  transcript: string;
...
export interface MemoryProvider {
  ...
  /** 初始化当前 City Power 实例持有的 Provider 生命周期。 */
...
/** MemoryPower 的显式构造参数。 */
export interface MemoryPowerOptions {
  /** 当前启用的 Memory Provider。 */
  provider?: "builtin";
  /** 当前启用的 Memory Storage。 */
  storage?: "file";
  /** City 为当前 MemoryPower 唯一实例分配的私有绝对数据目录。 */
