/**
 * Memory Power 的 system 使用说明与上下文内容构建器。
 *
 * 关键点（中文）
...
/** 构建不包含 Memory 内容的稳定 Power 使用说明。 */
export function build_memory_power_system_text(
  provider: MemoryProvider,
): string {
  return [
    "# Memory Power",
    "",
    `MemoryPower provides long-term memory through the ${provider.name} provider.`,
