/**
 * Skill power prompt section 渲染器。
 *
 * 关键点（中文）
...
function render_install_root_instructions(
  ...
  if (roots.length === 0) {
    return [
      ...
      "  Do not install a skill until the SkillPower configuration provides a discoverable root.",
    ].join("\n");
  }
...
export function render_skill_install_prompt(
  project_root: string,
  options: SkillPowerOptions | null | undefined,
  spec: string,
): string {
