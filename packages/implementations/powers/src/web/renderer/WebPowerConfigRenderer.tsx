 ...
 *
 * 只负责三类 Provider 的选择、凭据写入与能力测试；界面不回显已保存的 API Key，
 * 也不读取 Provider 状态，后者属于 Mainview 的只读状态页。
 */
...
} from "@downcity/city/power/react";
import type {
  WebPowerConfigDraft,
...
/** Web Power 配置 Renderer。 */
export function WebPowerConfigRenderer({ config, ui }: PowerConfigComponentProps) {
  const {
...
function SecretRow(props: {
  ...
  clear(): void;
  restore(): void;
  components: Pick<PowerRendererUiComponents, "Button" | "Inline" | "Input" | "Row" | "Status">;
}) {
