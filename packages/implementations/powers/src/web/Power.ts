/**
 * WebPower：provider-neutral 的联网与浏览器能力边界。
 *
 * 关键点（中文）
 * - 浏览器 provider 在 Action 首次使用时按执行作用域惰性创建。
 * - 搜索、文档读取与浏览器 session 是三个独立能力。
 * - 浏览器长期资源由 provider 拥有，并在 Power dispose 时统一释放。
 */
...
import { WEB_POWER_ACTIONS } from "@/web/types/WebPower.js";
...
function failure_result(error: unknown): PowerActionResult<PowerJsonObject> {
  const message = error instanceof Error ? error.message : String(error);
  return { success: false, error: message, message };
}

/** WebPower：组合显式注入的 Web provider。 */
export class WebPower extends Power {
  /** 当前 power 稳定名称。 */
  readonly name = "web";
  ...
  private readonly options: WebPowerOptions;

  /** WebPower 自己拥有的搜索 Provider。 */
  private readonly search_provider: WebSearchProvider | undefined;

  /** WebPower 自己拥有的文档 Provider。 */
  ...
  constructor(options: WebPowerOptions = {}) {
    super();
  ...
  async dispose(): Promise<void> {
    ...
    if (errors.length > 0) {
      throw new AggregateError(errors, "WebPower disposal failed");
    }
  }
  ...
  private async dispose_all_browser_providers(): Promise<void> {
    ...
    const errors = rejected_reasons(results);
    if (errors.length > 0) {
      throw new AggregateError(errors, "WebPower browser providers disposal failed");
    }
  }

  /** 返回当前作用域配置的浏览器 provider。 */
  private async ensure_browser_provider(
    context: import("@downcity/city/power").PowerContext,
  ...
  availability(
    ...
    return {
      enabled: true,
      available,
      reasons: available ? [] : ["WebPower has no configured provider"],
    };
  }
  ...
  readonly actions = {
    [WEB_POWER_ACTIONS.search]: create_action({
      ...
      execute: async ({ context, input }) => {
        ...
        if (!search_provider) {
          return failure_result("WebPower search provider is not configured");
        }
        try {
    ...
    [WEB_POWER_ACTIONS.open]: create_action({
      ...
      execute: async ({ context, input }) => {
        ...
        if (!document_provider) {
          return failure_result("WebPower document provider is not configured");
        }
        try {
    ...
    [WEB_POWER_ACTIONS.browser_create_session]: create_action({
      description: "Create a browser session with an optional initial URL.",
      returns: "session_id, provider, url",
      access: "write",
      input_schema: z.object({
    ...
    [WEB_POWER_ACTIONS.browser_observe]: create_action({
      description: "Observe the current browser page as text and an optional screenshot.",
      returns: "url, title, text, screenshot_path",
      access: "read",
      input_schema: z.object({
    ...
    [WEB_POWER_ACTIONS.browser_semantic_act]: create_action({
      description: "Execute one natural-language browser action through a semantic provider.",
      returns: "ok, instruction, url, observation",
      access: "write",
      input_schema: z.object({
        session_id: SESSION_ID_SCHEMA,
      ...
      execute: async ({ context, input }) => {
        ...
        if (!browser_provider?.semantic_act) {
          return failure_result(
            "WebPower semantic browser action provider is not configured",
          );
        }
        try {
    ...
    [WEB_POWER_ACTIONS.browser_extract]: create_action({
      description: "Extract text from the page body or a CSS selector.",
      returns: "url, text, truncated, selector",
      access: "read",
      input_schema: z.object({
        session_id: SESSION_ID_SCHEMA,
        selector: z.string().trim().min(1).optional(),
    ...
    [WEB_POWER_ACTIONS.browser_semantic_extract]: create_action({
      description: "Extract page content through a semantic provider.",
      returns: "url, text, extracted, truncated",
      access: "read",
      input_schema: z.object({
        session_id: SESSION_ID_SCHEMA,
        instruction: z.string().trim().min(1),
      ...
      execute: async ({ context, input }) => {
        ...
        if (!browser_provider?.semantic_extract) {
          return failure_result(
            "WebPower semantic browser extract provider is not configured",
          );
        }
        try {
    ...
    [WEB_POWER_ACTIONS.browser_close_session]: create_action({
  ...
  } as unknown as Power["actions"];
}

/** 返回浏览器资源的 Agent 配置隔离键。 */
function web_provider_key(context: import("@downcity/city/power").PowerContext): string {
  return JSON.stringify([context.agent.id, context.workspace.id]);
}

/** 校验 WebPower 当前公开支持的内建浏览器类型。 */
function assert_supported_config(config: WebPowerConfig | undefined): void {
...
function create_playwright_browser_provider(
  scope: import("@/web/types/WebPower.js").BrowserProviderScope,
): BrowserProvider {
  const config = scope.config;
  if (!config.cdp_url) {
    throw new Error("WebPower Playwright browser requires cdp_url");
  }
