...
/** Chat Plugin Renderer 定义。 */
export const CHAT_PLUGIN_RENDERER = define_plugin_renderer({
  sidebar: function ChatPluginSidebar({ plugin, navigation, ui }) {
    ...
    useEffect(() => {
      let disposed = false;
      set_error("");
      const refresh = () => void plugin.invoke<ChatDesktopSnapshot>("accounts.snapshot")
  ...
  mainview: function ChatPluginMainview({ plugin, navigation, ui }) {
    ...
    useEffect(() => {
      ...
      let disposed = false;
      const refresh = () => void plugin.invoke<ChatAccountDetailSnapshot>("accounts.detail", { account_id })
        .then((value) => { if (!disposed) set_detail(value); })
        .catch((reason) => { if (!disposed) set_error(to_error_message(reason)); });
      refresh();
      const timer = setInterval(refresh, 5_000);
      return () => { disposed = true; clearInterval(timer); };
    }, [account_id, plugin, ui.revision]);
    ...
    const mutate = async <TResult,>(action_id: string, input: object, message: string): Promise<TResult | undefined> => {
      ...
      set_error("");
      try {
        const result = await plugin.invoke<TResult>(action_id, input as PluginJsonValue);
        ui.invalidate();
...
/** 读取 Plugin 路由字符串。 */
function read_route(value: PluginJsonValue | undefined): string {
