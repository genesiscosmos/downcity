...
export const CHAT_PLUGIN_RENDERER = define_plugin_renderer({ config: function ChatPluginRenderer({ config: config_gateway, ui }) {
  ...
  const [draft, set_draft] = useState<ChatPluginPublicConfig>();
  ...
  const load = useCallback(async () => {
    ...
    set_error("");
    try {
      set_draft(await config_gateway.invoke("config.read") as unknown as ChatPluginPublicConfig);
    } catch (reason) {
  ...
  const save = async () => {
    ...
    set_error("");
    try {
      const saved = await config_gateway.invoke("config.save", draft as unknown as PluginJsonValue);
      set_draft(saved as unknown as ChatPluginPublicConfig);
      ui.toast({ type: "success", message: "Chat 配置已保存" });
    } catch (reason) {
  ...
  const add_channel = () => {
    if (!draft) return;
    const used_types = new Set(draft.channels.map((channel) => channel.type));
  ...
  if (loading && !draft) return <LoadingState label="正在读取 Chat 配置…" />;
  ...
  return <Page>
    ...
    <Section title="Delivery" description="指定唯一接收入站消息的 Agent 与 Workspace。" surface={false}>
      <Group>
        <Row label="Agent ID" trailing={<Input value={draft?.owner_agent_id ?? ""} on_value_change={(owner_agent_id) => set_draft((current) => current ? { ...current, owner_agent_id } : current)} />} />
        <Row label="Workspace ID" trailing={<Input value={draft?.owner_workspace_id ?? ""} on_value_change={(owner_workspace_id) => set_draft((current) => current ? { ...current, owner_workspace_id } : current)} />} />
      </Group>
    </Section>
    <Section title="Queue" description="控制入站消息的并发与合并等待。" surface={false}>
      <Group>
        <Row label="Maximum concurrency" trailing={<Input type="number" value={number_value(draft?.queue.max_concurrency)} minimum={1} maximum={32} placeholder="4" on_value_change={(value) => update_queue("max_concurrency", value)} />} />
        <Row label="Merge debounce" description="连续消息停止到达后等待的毫秒数。" trailing={<Input type="number" value={number_value(draft?.queue.merge_debounce_ms)} minimum={0} maximum={60000} on_value_change={(value) => update_queue("merge_debounce_ms", value)} />} />
        <Row label="Maximum merge wait" description="一组消息允许等待合并的最大毫秒数。" trailing={<Input type="number" value={number_value(draft?.queue.merge_max_wait_ms)} minimum={0} maximum={120000} on_value_change={(value) => update_queue("merge_max_wait_ms", value)} />} />
      </Group>
    </Section>
    <Section title="Channels" description="同一种 Channel 类型在当前配置中只能出现一次。" action={<Button on_click={add_channel}>添加 Channel</Button>} surface={false}>
      {!draft?.channels.length ? <EmptyState title="还没有消息渠道" description="添加 Telegram、Feishu 或 QQ Channel。" action={<Button variant="primary" on_click={add_channel}>添加 Channel</Button>} size="compact" />
        : <Stack>{draft.channels.map((channel, index) => <ChannelEditor
          key={`${channel.type}:${index}`}
          channel={channel}
          used_types={new Set(draft.channels.filter((_item, item_index) => item_index !== index).map((item) => item.type))}
          components={{ Button, Group, Input, Row, Select, Switch }}
          update={(next) => set_draft((current) => current ? { ...current, channels: current.channels.map((item, item_index) => item_index === index ? next : item) } : current)}
          remove={() => set_draft((current) => current ? { ...current, channels: current.channels.filter((_item, item_index) => item_index !== index) } : current)}
        />)}</Stack>}
    </Section>
  </Page>;
} });
