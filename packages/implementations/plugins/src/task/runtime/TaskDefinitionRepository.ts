 ...
 * Task 定义仓储的实例级事务入口。
 *
 * TaskPlugin 的 Agent action、宿主 action 与 Scheduler 共享同一个实例。所有会改变
...
export class TaskDefinitionRepository {
  ...
  async mutate<TResult>(
    operation: (storage: PluginStorage) => Promise<TResult>,
  ): Promise<TResult> {
