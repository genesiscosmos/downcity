 ...
 * Task Plugin 的功能工作区。
 *
 * Sidebar 直接展示 TaskPlugin 拥有的全部 Task；Mainview 承担创建、编辑、定义查看和
...
export const TASK_PLUGIN_RENDERER = define_plugin_renderer({
  sidebar: function TaskPluginSidebar({ plugin, navigation, notifications, ui }) {
    ...
    const toggle_task = async (task: TaskMainviewItem) => {
      ...
      try {
        const next = await plugin.invoke<TaskMainviewHistorySnapshot>("tasks.history", task_action_input(task));
  ...
  mainview: function TaskPluginMainview({ plugin, navigation, notifications, ui }) {
    ...
    useEffect(() => {
      ...
      set_run_error("");
      void plugin.invoke<TaskMainviewRunDetailSnapshot>("tasks.run_detail", { ...task_action_input(task), timestamp: run_timestamp })
      ...
      return () => { disposed = true; };
    }, [plugin, run_timestamp, selected_run_updated_at, task, view]);
