...
export const SKILL_PLUGIN_RENDERER = define_plugin_renderer({
  sidebar: function SkillPluginSidebar({ plugin, navigation, ui }) {
    ...
    const remove_skill = async (skill: SkillMainviewItem) => {
      ...
      if (!confirmed) return;
      try {
        const result = await plugin.invoke<SkillMainviewMutationResult>("skills.remove", skill_input(skill));
  ...
  mainview: function SkillPluginMainview({ plugin, navigation, ui }) {
    ...
    useEffect(() => {
      ...
      set_content("");
      set_reading(true);
      void plugin.invoke<SkillMainviewReadResult>("skills.read", skill_input(selected_skill))
      ...
      return () => { disposed = true; };
    }, [plugin, selected_skill ? skill_key(selected_skill) : "", ui.revision]);
    ...
    const install = async (hit: SkillMainviewSearchHit) => {
      ...
      set_error("");
      try {
        const result = await plugin.invoke<SkillMainviewMutationResult>("skills.install", { scope: target_scope, spec: hit.spec, ...(target_scope === "workspace" ? { workspace_id: install_target } : {}) });
...
/** 创建 Workspace 范围的稳定 Plugin route。 */
