...
import type {
  WebPluginConfigSaveInput,
  WebPluginConfigTestInput,
  WebPluginConfigTestResult,
  WebPluginConfigView,
} from "@/web/types/WebPluginSettings.js";

const DEFAULT_TIMEOUT_MS = 30_000;
const DEFAULT_MAX_OBSERVATION_CHARS = 12_000;

/** Web 配置 actions 的资源回调。 */
export interface WebPluginConfigActionOptions {
  /** 配置保存后释放旧 Provider。 */
  after_save(): Promise<void>;
}

/** 注册 Web Plugin 的脱敏配置与能力测试 actions。 */
export function register_web_plugin_config_actions(
  context: PluginLifecycleContext,
  options: WebPluginConfigActionOptions,
): void {
  ...
  context.plugin.config_action({
    ...
    run: async (input, action_context) => {
      ...
      await action_context.config.set(next as unknown as PluginJsonObject);
      await options.after_save();
      return public_config(next as unknown as PluginJsonObject);
    },
  });
  context.plugin.config_action({
    id: "config.test",
...
export function public_config(source: PluginJsonObject): WebPluginConfigView {
  const config = source as WebPluginConfig;
  return {
...
export function normalize_config(
  ...
  current_source: PluginJsonObject,
): WebPluginConfig {
  if (!input || typeof input !== "object" || Array.isArray(input)) {
    throw new Error("Web Plugin config must be an object");
  }
  const source = input as unknown as WebPluginConfigSaveInput;
  const current = current_source as WebPluginConfig;
  const result: WebPluginConfig = {
...
async function test_capability(
  context: PluginLifecycleContext,
  input: PluginJsonValue | undefined,
): Promise<WebPluginConfigTestResult> {
  ...
  const action_input: PluginJsonObject = capability === "search"
  ...
  try {
    ...
    if (capability === "browser") {
      const data = result.data as PluginJsonObject | undefined;
      const session_id = typeof data?.session_id === "string" ? data.session_id : "";
      if (session_id) {
        await context.system.invoke_agent_plugin({
          agent_id: agent.agent_id,
          workspace_id: workspace.workspace_id,
          plugin_id: context.plugin.id,
...
/** 读取测试能力枚举。 */
function read_test_capability(input: PluginJsonValue | undefined): WebPluginConfigTestInput["capability"] {
  if (!input || typeof input !== "object" || Array.isArray(input)) throw new Error("Invalid test input");
  const capability = (input as PluginJsonObject).capability;
  if (capability === "search" || capability === "document" || capability === "browser") return capability;
  throw new Error("Invalid Web capability");
}

/** 读取搜索 Provider 枚举。 */
function read_search_provider(value: unknown): NonNullable<WebPluginConfig["search_provider"]> {
  return value === "tavily" || value === "exa" || value === "disabled" ? value : "auto";
}

/** 读取文档 Provider 枚举。 */
function read_document_provider(value: unknown): NonNullable<WebPluginConfig["document_provider"]> {
  return value === "firecrawl" || value === "disabled" ? value : "fetch";
}

/** 读取浏览器 Provider 枚举。 */
function read_browser_provider(value: unknown): NonNullable<WebPluginConfig["browser_provider"]> {
