/** Web Plugin 的 Desktop 配置界面。 */

import { useCallback, useEffect, useState } from "react";
import { define_plugin_renderer } from "@downcity/city/plugin/react";
import type {
  WebPluginConfigDraft,
  WebPluginConfigTestResult,
  WebPluginConfigView,
} from "@/web/types/WebPluginSettings.js";

/** Web Plugin 专用配置 Renderer。 */
export const WEB_PLUGIN_RENDERER = define_plugin_renderer({
  config: function WebPluginSettingsRenderer({ config, ui }) {
    const {
      Button, Callout, Group, Inline, Input, LoadingState, Page, Row, Select,
      Stack, Status, Toolbar,
    } = ui.components;
    const [draft, set_draft] = useState<WebPluginConfigDraft>();
    const [loading, set_loading] = useState(true);
    const [saving, set_saving] = useState(false);
    const [testing, set_testing] = useState<"search" | "document" | "browser">();
    const [error, set_error] = useState("");
    const [test_message, set_test_message] = useState<WebPluginConfigTestResult>();

    const load = useCallback(async () => {
      set_loading(true);
      set_error("");
      try {
        set_draft(to_draft(await config.invoke<WebPluginConfigView>("config.read")));
      } catch (reason) {
    ...
    const update = <TKey extends keyof WebPluginConfigDraft>(
      key: TKey,
      value: WebPluginConfigDraft[TKey],
    ...
    const save = async (): Promise<boolean> => {
      ...
      try {
        const saved = await config.invoke<WebPluginConfigView>("config.save", draft);
    ...
    const test = async (capability: "search" | "document" | "browser") => {
      ...
      try {
        const result = await config.invoke<WebPluginConfigTestResult>("config.test", { capability });
    ...
    return <Page>
      ...
      <Callout tone="warning">API Key 会写入 Downcity 本地 Plugin 配置文件，不会回显到界面；当前尚未使用系统钥匙串。</Callout>
      <Stack>
        <Group label="搜索">
          ...
          ]} on_value_change={(value) => update("search_provider", value as WebPluginConfigDraft["search_provider"])} />} />
        ...
        <Group label="网页读取">
          <Row label="Document provider" description="Fetch 无需密钥；Firecrawl 适合动态或难解析页面。" trailing={<Select value={draft.document_provider} options={[
            ...
            { value: "disabled", label: "关闭" },
          ]} on_value_change={(value) => update("document_provider", value as WebPluginConfigDraft["document_provider"])} />} />
        ...
        <Group label="浏览器">
          <Row label="Browser provider" description="Local 自动启动独立 Chrome；CDP 用于云浏览器或现有端点。" trailing={<Select value={draft.browser_provider} options={[
            ...
            { value: "disabled", label: "关闭" },
          ]} on_value_change={(value) => update("browser_provider", value as WebPluginConfigDraft["browser_provider"])} />} />
...
function SecretRow(props: {
  ...
  clear(): void;
  restore(): void;
  components: Pick<import("@downcity/city/plugin/react").PluginRendererUiComponents, "Button" | "Inline" | "Input" | "Row" | "Status">;
}) {
...
/** 把脱敏配置转换为可编辑状态。 */
function to_draft(config: WebPluginConfigView): WebPluginConfigDraft {
  return {
    ...config,
    tavily_api_key: "",
    exa_api_key: "",
