...
const PaginationLink = ({ className, isActive = false, ...props }: React.ComponentProps<"a"> & { isActive?: boolean }) => <a data-slot="pagination-link" aria-current={isActive ? "page" : undefined} className={cn("inline-flex size-6 items-center justify-center rounded-md text-xs text-muted-foreground hover:bg-interaction-hover hover:text-foreground aria-[current=page]:bg-interaction-selected aria-[current=page]:text-foreground", className)} {...props} />;
...
const PaginationEllipsis = ({ className, ...props }: React.ComponentProps<"span">) => <span data-slot="pagination-ellipsis" aria-hidden="true" className={cn("inline-flex size-6 items-center justify-center text-xs text-muted-foreground", className)} {...props}>…</span>;
