...
宿主最小 UI 集包括 `Page/Section/Group/Row`、`Stack/Inline/Toolbar/Tabs`、`Button/Input/Select/Switch`、`CodeBlock` 与统一反馈组件。多级导航统一使用 `SidebarTreeItem` 在 Plugin Sidebar 内表达，不在 Mainview 中创建第二层 Sidebar。框架不使用配置 Schema 生成 UI。
