...
**每一级的字号与配对行高都落在 1/16rem（0.0625rem）网格上**，因此 100% 缩放下像素值全是整数（§3.3）。
...
### 3.3 全部数值落在 1/16rem 网格上（100% 缩放下像素值为整数）

每一级的字号与配对行高都是 **0.0625rem 的整数倍**。1rem = 16px，所以 0.0625rem 正好是 1px：

| 档位 | 字号 rem / px | 行高 rem / px |
| --- | --- | --- |
| `3xs` | 0.625 / 10 | 0.875 / 14 |
...
这是 Tailwind 自己用的网格（`0.75` / `0.875` / `1` 都是它的倍数），也是 CSS 里表达「干净像素尺寸」的标准做法。

**注意：直接用 0.05rem 作步进是无效的**（0.05rem = 0.8px）。早先一版试图用「向上取整到 0.05rem 倍数」来消除小数像素，结果只改了 `0.825 → 0.85`，像素值仍是小数。有效的是 1/16rem。

**两个边界：**

1. **只在 100% 缩放下成立。** `ui_scale` 会改写根字号（如 125% → 20px），那时 0.0625rem = 1.25px，小数不可避免——这是 rem 方案的固有性质，不是本设计的缺陷。
2. **新增档位必须继续落在网格上**，否则又会出现半像素字形。`font_scale.test.ts` 的「都落在 1/16rem 网格上」断言盯着这一点（并带一条反向断言证明它不是恒真）。
...
2. 它是**侧栏的导航文字**吗（行主标签、侧栏面板标题）？→ `xs`（行描述 `2xs`）。见 §4.1 的说明，这是唯一的整片例外。
3. 这个界面里**唯一的主标题**吗？→ `2xl`。是一个**区块的标题**吗？→ `lg`。是**面板的标题**吗？→ `xl`。
4. 它属于某个**更大文字的附属信息**吗（描述、时间、计数、说明）？→ 降一档到两档：列表行主标签用 `base`，描述用 `xs`，元信息用 `2xs`，不承载阅读的用 `3xs`。
