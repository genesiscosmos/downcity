...
当前阶段不引入完整虚拟列表，也【不使用】`content-visibility` 一类离屏布局跳过。消息包含 Markdown、代码块、公式和图表，真实高度会在渲染后变化；
离屏行按占位高度记账、进入视口后才换成真实高度，而行高普遍相差一个数量级，等于每一行经过视口边界时都在改变视口上方的高度，
用户向上浏览时会反复看到内容被推走又弹回（详见 11.2）。保留下来的是 React 侧的分段与 memo 边界：

- 完整 DOM、浏览器查找和文本选择语义保持不变。
- 浏览器原生滚动锚定负责补偿视口上方的高度变化；消息行不得把自己排除出锚定候选集合。
- 若实际长会话分析仍显示 DOM 节点数量是主要瓶颈，再单独设计动态高度虚拟列表；
  届时必须同时接管高度测量、选区恢复与滚动锚点生命周期。
...
3. 加载更早历史前先记录首个可见消息 ID 与视口偏移，提交后按同一消息恢复位置；只有消息不存在时才退回高度差补偿。
   恢复点由内容头部标识（首条消息 ID）变化触发的 layout effect 承担，必须与 React 提交同步：
   Store 更新到 DOM 提交之间隔着 React 调度，任何「await 之后再补一帧」的写法都可能跑在旧 DOM 上。
4. Session 切换会取消上一 Session 未完成的动画帧和历史锚点，避免跨 Session 写入滚动位置。
5. Group 与 Session 共用同一策略，Group 成员状态变化不再无条件抢回底部。
6. 定位到底部直接写 `scrollTop`，不用 `scrollIntoView`（后者会连带滚动所有可滚动祖先）；
   本模块写入的滚动位置会被登记，随后触发的 scroll 事件不得被解释为「用户向上浏览」。

### 11.3 验收标准

- [x] Session 与 Group 的消息行保留完整 DOM，且不引入离屏布局跳过
- [x] 流式 Assistant 消息保持实时布局与绘制
- [x] 用户向上浏览时视口上方的高度变化由原生锚定补偿，不出现位移跳动
