...
- `main`：统一导出 City Plugin 实例、生命周期与宿主管理 actions。
...
`PluginLifecycleContext.storage`，不能复制出按 Agent 的 Plugin 实例。
...
- `agent.json`：身份、版本和默认执行配置；不保存 Plugin 实例或绑定关系。
