...
| 指标 | 基线（A1 前） | 当前（A1 + B + C1 + C2 后） | 变化 | 预算 |
|---|---|---|---|---|
...
主包减少的约 1 MB 是 mermaid 核心及其布局依赖，已拆到独立的 `mermaid.core-*.js`（994.14 kB），首次渲染图表时才加载。C1 的 Worker 化只增加约 2 kB（着色核心本就在主包内，新增的只是 Worker 客户端与消息协议），着色计算本身移到 `highlight.worker-*.js`（355.96 kB）。C2 增加约 2.7 kB（预算上下文与占位 UI）。B 增加约 2.4 kB（分段回收容器）。
...
| 2026-09-21 | C2 图表渲染预算 | 5,421,500 B | 单会话自动渲染图表上限 20 张，超出后需用户手动确认；按图表标识记账 |
...
`mermaid_render_budget.ts` 按图表标识（`useId`）而非次数计数。同一张图在主题切换、重试、组件重挂载时会再次请求渲染，按次数会让这些正常重渲染白白消耗预算。没有 `MermaidRenderBudgetProvider` 时不设限，因为图表块同时被 Power 预览与 Workspace 文件预览使用。
...
`useId` 随组件实例变化，而分段回收会卸载并重建图表组件。用它会让「滚出去再滚回来」被当成一张新图：渲染预算被白消耗，渲染缓存也用不上。标识改为对源码做散列（`hash_mermaid_source`）。
...
首次挂载的 `claim` 写在 `useState` 初始值里，不在 `useEffect` 里。若只在 effect 里结算，首屏就可见的图表会在同一个 commit 内用未结算的值启动渲染，绕过预算。后续内容变化（流式结束）再在 effect 里重新结算。
...
- 改动图表渲染路径 → 复核第五章 5.5 / 5.7 / 5.9：预算按源码标识记账、结算在渲染阶段；
