...
> 后续：本节的块间距已从 `gap-2.5`（10px）改为 `gap-3`（12px），字号上限随之变为 `xl`。
> 见第二十五节。

---

## 二十五、Markdown 垂直节奏收敛（2026-09-19）

> 状态：已实施
>
> 范围：`app/desktop/src/renderer/styles/markdown.css`、`styles/mermaid.css`、`tests/markdown_spacing.test.ts`、`tests/chat_message_layout.test.ts`

### 25.1 触发原因

反馈是「agent message 的 markdown 间距现在稍微有点大」。量一遍之后，问题不是「普遍偏大」，
而是**摊得太开**——同一条消息里最小 7.5px、最大 28px：

| 位置 | 改前 | 来源 |
| --- | --- | --- |
| 段落之间 | 7.5px | `markdown.css` 0.5em |
| 列表项之间 | 9.5px | 0.1em **+ 库的 `py-1`** |
| 段落 ↔ 代码块 | 10.5px | 0.7em |
| 段落 ↔ 图表 | 11.25px | `mermaid.css` 0.75em |
| 段落 ↔ 公式 | 13.5px | 0.9em |
| 段落 ↔ 表格 / 分隔线 | 15px | 1em **与库的 `space-y-4`（16px）取大值** |
| 段落 ↔ 图片 | 28px | 库的 `my-4`（16px）+ 本文件 `img` 0.8em（12px）**相加** |
| 段落 ↔ h1 / h2 | 25.6 / 21.5px | 1.1em **相对标题自身字号** |

两个方向的问题叠在一起：

1. **本地值逐条拍出**：0.7 / 0.75 / 0.8 / 0.9 / 1 / 1.1em，没有依据，只是各自「看着差不多」。
2. **库的间距在静默叠加**：Streamdown 在根节点写 `space-y-4`、在列表项写 `py-1`、
   在图片容器写 `my-4`。它们不在本仓库里、不报错，但会加进已经声明过的节奏里——
   列表项因此比段落还松，图片因此拿到 28px。

### 25.2 决策

**三个间距档 + 标题一档，全部声明在 `.markdown` 上。**

| 令牌 | 值 | 15px 下 | 用在哪 |
| --- | --- | --- | --- |
| `--markdown-tight-gap` | 0.25em | 3.75px | 列表内相邻项、引用内相邻段落 |
| `--markdown-flow-gap` | 0.5em | 7.5px | 段落、列表块、引用块 |
| `--markdown-block-gap` | 0.7em | 10.5px | 代码块、表格、图表、图片、分隔线、公式 |
| `--markdown-heading-gap-before` | 0.9em | 随标题自身字号 | 标题上方 |

- **正文档 0.5em 不动**。它是唯一被验证过的值（第二十四节），也是一条硬约束的下界
  （必须小于消息内块间距），其余两档相对它取值。
- **块级档 0.7em 是原设计意图的最低值**：那些值原本在 1.4–2 倍之间，取 1.4 倍既保留
  「显示型块要停一下」的意图，又与正文档差 3px（> 本仓库的 2px 可感知下限）。
  原表里 1em / 0.9em 那几项与消息级间距只差 1px，会把一个代码块读成另一条消息。
- **令牌写在 `.markdown` 而不是 `tokens.css`**：em 在**使用处**解析（自定义属性只是原样替换），
  所以同一份节奏在 `base` 的会话正文与 `xs` 的 Plugin 说明下各自成立。
- **标题只给上边距**。相邻块的外边距会**折叠**（取较大值），因此标题下方写小于正文档的值
  会被吞掉、写大于正文档的值又让标题离正文更远——两个方向都不对。
  标题与下方内容的距离由下面那个块自己的上边距给出（排版惯例：标题靠上、内容靠下）。
- **库的三处间距全部显式压掉**，并各自写了注释说明是哪一处、不压会怎样。
- **图片外边距只算一次**（容器拿块级档，容器内 `img` 清零）：原实现是 16 + 12 相加。
- **不改 Workspace 文档预览与 Plugin 说明**：它们共用同一套排版，因此自动获得同一份节奏。

### 25.3 顺带修掉的两个既有缺陷

1. **首尾清零规则被盖掉。** `.markdown > :first-child { margin-top: 0 !important }` 原本排在
   文件开头，而块级规则（代码块、表格、图片容器）与它**特异性相同**（都是 `.markdown` + 一个选择器），
   只能靠源码顺序决胜——块级规则在后面，于是那两条在它们身上完全无效。
   后果：以代码块/表格/图片开头的消息，会在身份行下多出一段留白
   （容器是 flex 列，外边距不与 `gap-3` 折叠，留白实打实叠上去）。
   现在这两条移到文件末尾。
2. **列表项的 `py-1` 漏进来。** 本文件只声明了 `padding-left`，库的纵向 `py-1` 静默保留，
   相邻两项间距 9.5px——比段落还松。改用 `padding` 简写把纵向清零。
...
| 段落之间 | 7.5px | 7.5px（不变） |
| 列表项之间 | 9.5px | **3.75px** |
| 段落 ↔ 代码块 / 表格 / 图片 / 分隔线 | 10.5 / 15 / 28 / 15px | **10.5px**（统一） |
| 段落 ↔ 图表 / 公式 | 11.25 / 13.5px | **10.5px**（统一） |
| 段落 ↔ 标题 | h1 25.6 / h2 21.5px | **h1 20.9 / h2 17.6px**（仍随标题字号） |
| 引用块 ↔ 前后段落 | 10.5px | **10.5px**（不变） |

整条消息的间距范围从 7.5–28px 收到 3.75–20.9px，且每一档都能说出依据。

### 25.5 守卫

新增 `tests/markdown_spacing.test.ts`（11 条），锁住：

1. 三档存在、取值与契约表逐项相等，且**不得出现第四档**；
2. 大小关系成立（紧 < 正文 < 块级），且块级与正文档的差值 ≥ 2px；
3. **两个文档方言样式表不再出现字面量 em 外边距**——这是收敛的实际执行者，
   也是唯一能拦住「某处又拍了一个 0.7em」的断言；
4. 反向确认第 3 条的正则不是恒真；
5. 库的三处间距（根节点 `space-y-4`、列表项 `py-1`、图片容器 `my-4`）各自被显式压掉；
6. 根节点兜底规则排在具体规则之前（顺序决定它是否会被盖掉）；
7. 标题只有上边距；
8. 首尾清零排在所有块级规则之后，且带 `!important`；
9. 块级表面与正文块的档位逐条正确。

同时更新 `chat_message_layout.test.ts`：它原来从 `.markdown :where(p)` 里正则抓一个 em 字面量，
现在改为解析 `--markdown-flow-gap` 的真值（段落间距不再写在规则里）。

**探针验证（每条都先失败、还原后逐字节一致）**：正文档改到与块级同值 → 第 1、2 条失败；
删掉根节点兜底 → 第 5 条失败；`mermaid.css` 回到字面量 → 第 3、9 条失败；
列表项回到 `padding-left` → 第 5 条失败；块级档改 0.5em → 第 1、2 条失败；
标题加回下边距 → 第 3、7 条失败；首尾清零去掉 `!important` → 第 8 条失败。

### 25.6 验证

```bash
cd app/desktop && node --test tests/*.test.ts        # 500/500
cd app/desktop && npx tsc -p tsconfig.web.json --noEmit && npx tsc -p tsconfig.node.json --noEmit
```

**本环境不能验证的事**：GUI 观感未经人工目测。取值是按「原意图的最低值 + 2px 可感知下限」
推出来的区间解；若目测仍偏松，改 `--markdown-block-gap` 与 `--markdown-tight-gap` 即可，
**不要动 `--markdown-flow-gap`**——它与消息内块间距的约束是硬的（见第二十四节）。
