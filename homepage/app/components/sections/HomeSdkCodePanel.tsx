...
function create_code_groups(locale: "zh" | "en") {
  return {
    agent: [
      ...
      { key: "agent-bind-city", start_step: 4, order: 13, lines: ["", "const agent = new Agent({", '  id: "repo-helper",', "  city,", "});"] },
      ...
      { key: "session-create", start_step: 15, order: 30, lines: ["", "const session = await agent.sessions.create({", '  workspace: city.workspace("project")!,', "});", "await session.set({ model });"] },
    ...
    city: [
      ...
      { key: "city-research", start_step: 7, order: 2, lines: ["", "const shared_workspace = city.workspace(\"project\");"] },
      { key: "city-operations", start_step: 8, order: 3, lines: ["", "const reviewer = new Agent({ id: \"reviewer\", city });"] },
    ],
    federation: [
