...
- the token expired
- the `bureau_id` does not match the token
...
3. verify that the Token's `bureau_id` belongs to the target product, then check `user_token`, service id, action id, and model id
