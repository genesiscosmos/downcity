...
Local HTTP uses the same product-side model as remote HTTP: `FederationAdmin` uses `admin_secret_key`, public actions can be called as guest, and authenticated user actions require a normal `user_token`.
