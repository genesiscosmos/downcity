...
That is why `@downcity/federation` has not only routes and services, but also a store and table layer.
...
You should still prefer services, and `Embassy Admin` for city-facing or managed entry points.

### Frontends should not treat tables as their API

City-side callers should continue to use `Embassy User` and `Embassy Admin`, not raw table access.

## Read next

- For how the database is wired in, read [Federation](/en/city-sdk-docs/packages/federation/federation)
