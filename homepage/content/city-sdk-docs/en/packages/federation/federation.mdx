---
title: Federation
description: "The role of the Federation instance, runtime and serve inside @downcity/federation."
---

`@downcity/federation`'s `Federation` is the long-running server container of Downcity. It is where a Database Adapter, services, auth, and the shared `/v1/*` route space come together. Product-side access uses the `City` client; see [Federation and City](/en/city-sdk-docs/concepts/federation) for the distinction.
...
Then point Embassy at that HTTP URL:
...
await embassy.admin.list_services();
```

Local HTTP uses the same identity model as remote HTTP: Embassy Admin uses an administrator session token, while authenticated user actions require a normal `user_token`.

If `AccountsService` is configured with `local_login: true`, `/v1/accounts/providers` only returns the local login method. Call `accounts.login/start` with `provider: "local"` and `bureau_id` to receive a `login_id`, then read the normal `user_token` from `accounts.login/result` and recreate or update Embassy User with `user_token` (which contains `bureau_id`).

## Why Federation only takes a Database Adapter

`@downcity/federation` owns the logic that should stay stable across runtimes:
...
Federation is the long-running server runtime. Product-side calling uses the `City` client; see [Client SDK](/en/city-sdk-docs/packages/federation/client).
...
- For what actually gets mounted into Federation, read [Service and Action](/en/city-sdk-docs/packages/federation/service-and-action)
- For hooks and extension points, read [Hooks and Service Mounting](/en/city-sdk-docs/packages/federation/hooks-and-service)
- For persistence and tables, read [Store and Table](/en/city-sdk-docs/packages/federation/store-and-table)
