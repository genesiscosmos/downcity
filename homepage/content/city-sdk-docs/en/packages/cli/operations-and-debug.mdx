...
description: "The boundary between fed, city, Embassy Admin, and direct database inspection, and when the CLI is the better tool."
---
...
The CLI and Desktop share the same `~/.downcity` data while owning independent City processes.
Each City registers its host at startup. If the other host is already running, the new host asks
whether it should close the existing City and take over. Cancelling leaves the existing host
untouched. Stopping City does not remove Agent, Plugin, or Session data.
...
- For the data layer behind those facts, read [Store and Table](/en/city-sdk-docs/packages/federation/store-and-table)
