...
- AI service calls such as `city.ai.stream()` and `city.ai.image_create()` / `city.ai.image_result()`
...
1. create a new `bureau_id`
2. connect `Embassy User` in the product client

## What `bureau_id` means in this model

`bureau_id` is not a model name, and it is not the full abstraction of your tenant system.
...
If you are building a multi-product portfolio, `bureau_id` is the most stable boundary.
...
- you can pass it explicitly into `city.ai.stream({ model, call })`
