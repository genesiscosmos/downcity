...
| `/v1/bureaus/server-url/update` | `POST` | Update a Bureau server entrypoint |
...
City resolves the user and city from the bearer `user_token`. If the request body includes `bureau_id`, it must match the city bound to the token; otherwise City returns `403`.
