...
| `/v1/ai/stream` | `POST` | Downcity Model Protocol stream transport | CityModel |
...
Federation resolves both the user and Bureau identity from the bearer `user_token`. The request body
...
This versioned Downcity Model Protocol transport accepts
`{ protocol_version: 1, model_id, call }`, where `call` is a `ModelCall` containing
messages, tools, reasoning, and generation options. SSE returns
`{ protocol_version: 1, event: ModelStreamEvent }`. Product code should use the
`CityModel` returned by the catalog or call `city.ai.stream({ model, call })`.
Both paths consume the same standard event stream.

When supplied, `call.reasoning.effort` must match the final model catalog's
`reasoning.efforts`. AIService validates it again after fallback selects the
execution model.
