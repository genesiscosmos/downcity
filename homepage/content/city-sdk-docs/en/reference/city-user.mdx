...
- `federation_url`
- `bureau_id`, automatically injected into user service action calls
...
After receiving `session.user_token`, create a User City with `user_token` for AI service calls. Keep `bureau_id` when you want User City to inject it into ordinary service actions.
...
- the request `bureau_id` does not match the city bound to the token
