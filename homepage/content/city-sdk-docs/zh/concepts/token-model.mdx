...
<div className="dc-doc-flow" aria-label="管理员会话和 user_token 的边界图">
  ...
  <div className="dc-doc-lane">
    <div className="dc-doc-node">
      ...
      <span>前端、扩展或 App 使用 Embassy User 携带 `user_token` 调用 Service。</span>
      <code>拿不到管理员凭证</code>
    </div>
  </div>
</div>
...
`user_token` 是给终端用户或产品 client 使用的调用凭证。Embassy Admin 可以基于 `bureau_id + user_id` 向 Federation 申请。
...
使用 `fed deploy` 配置的管理员 ID 和密码登录 Embassy Admin。登录响应会返回有时限的管理员会话 Token，用于创建 Bureau、管理 env，以及请求 Federation 为用户签发 `user_token`。
