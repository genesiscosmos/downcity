...
- `AgentWorkspace` 拥有 `agent.enter(workspace)` 创建的执行作用域，包括 Session、日志、
  调度任务、Workspace 级 Plugin 生命周期和私有运行状态。
- Shell 是 Workspace 能力，不再存在独立的 `@downcity/shell` 包。

一个 Agent 可以进入多个 Workspace。一个 Workspace 实例只能被进入一次，因为其中的 Shell
和私有存储会绑定到一个 AgentWorkspace 生命周期。多个 Agent 操作同一物理项目目录时，应分别
创建 Workspace 实例。
