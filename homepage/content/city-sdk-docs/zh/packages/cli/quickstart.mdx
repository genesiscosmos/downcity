...
`city agent chat` 会持续订阅当前 Session，并实时合并 Message、Part、文本、reasoning、Tool 和 Interaction 状态。使用 `/session` 切换会话时，TUI 会先订阅新 Session，再加载完整快照并合并期间收到的事件。模型提供可见 reasoning 文本时，TUI 会按 Assistant Part 顺序弱化展示；只有 reasoning metadata 时不会生成虚假的推理内容。
