...
- `new UsageService(...)` 聚合 `AIService` 技术用量与 `CreditsService` 已入账消费，并提供当前用户查询入口
...
service 负责把能力挂进 City，真正的产品调用入口仍然是 [@downcity/federation](/zh/city-sdk-docs/packages/federation/federation)。

## 继续阅读

- 想看 city 端怎么统一调用 service，读 [@downcity/federation](/zh/city-sdk-docs/packages/federation/federation)
