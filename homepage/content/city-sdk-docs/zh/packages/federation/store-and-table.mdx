...
这就是为什么 `@downcity/federation` 里不仅有 HTTP 路由，还有 store / table 这一层。
...
前端产品应该继续走 `Embassy User` / `Embassy Admin`，而不是直接访问底层表。

## 继续阅读

- 想知道数据库是怎么接进来的，读 [Federation](/zh/city-sdk-docs/packages/federation/federation)
