...
绑定 Git Workspace 的 Session 会在每个 Turn 执行前后创建隔离的 shadow Git 快照，并把本轮
文件改动写入最后一条 Assistant Message。该过程不会修改项目的 index、stash、branch 或 commit；
Shell、脚本和文件工具产生的改动使用同一套结果。非 Git Workspace 或快照不可用时，对话仍会正常执行。

文件改动使用 `data-session-turn-file-diff` data part 持久化，包含逐文件状态、增删行数和 unified diff。
可以使用公开解析函数安全读取：
