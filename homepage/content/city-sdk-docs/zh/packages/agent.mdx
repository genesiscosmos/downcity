...
description: "Agent、AgentSessions、Session、执行内核与 RemoteAgent。"
---

`@downcity/agent` 提供 Agent、AgentSessions、Session、执行内核与 RemoteAgent。City 组合根由 `@downcity/city` 提供。
Agent 不持有 Plugin；City 负责 Plugin catalog、共享实例与生命周期，Plugin 作者协议位于
`@downcity/plugin`。Workspace 资源由 `@downcity/workspace` 提供。
...
使用 Plugin 前应先通过 `CityOptions.plugins` 或 `city.plugins.provide()` 向 City 登记模块；
`city.agents.add()` 只声明 `plugin_id` 与可选 Profile 绑定。
