...
> `City` 是旧的底层用户客户端。新代码应使用 `@downcity/federation` 的
> `Embassy.user`；本页只保留旧方法面的迁移参考。

它绑定的是一个用户在某个产品下的调用上下文：

- `federation_url`
- `bureau_id` 由 Federation 从 `user_token` 解析，普通 service action 不重复传入
...
拿到 `session.user_token` 后，只需用 `user_token` 创建 User City。普通 service action 的 `bureau_id` 由 Federation 验证 Token 后解析，客户端不重复传入。
...
传 `federation_url + user_token`，适合：
...
- `me` 返回按当地自然日聚合的 Credits 和 AI 技术用量。
- `me/recent` 返回当前用户最近的单次 AI Token 用量，默认 20 条，最多 50 条。
- `next_cursor` 是不透明分页 Cursor；为 `null` 时表示没有下一页。
- `metering_status` 为 `unavailable` 时，该记录的 Token 字段为 `null`，不能从 Credits 反推。
- 两个入口的用户身份都只来自当前 `user_token`，请求参数不接受 `user_id` 或 `bureau_id`。
...
这些都属于可信环境动作，应该交给 [FederationAdmin](/zh/city-sdk-docs/reference/city-admin) 或你自己的后端。
