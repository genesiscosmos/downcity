...
- `federation_url`
- `bureau_id`，调用普通 user service action 时会自动注入
...
拿到 `session.user_token` 后，AI service 可以直接用 `user_token` 调用。需要让 User City 自动给普通 service action 注入 `bureau_id` 时，再同时传入 `bureau_id`。
...
- token 已过期
- token 签名无效
- 请求里的 `bureau_id` 与 `user_token` 绑定的 city 不一致
