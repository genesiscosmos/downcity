...
| `/v1/bureaus/server-url/update` | `POST` | 更新 Bureau 服务入口 |
...
`bureau_id`；如果传了，必须与 token 绑定的 city 一致。
...
```json
{ "bureau_id": "bureau_xxx", "name": "Chrome Extension", "server_url": "https://bureau.example.com", "state": "active", ... }
```
...
```json
{ "user_token": "ub_xxx", "bureau_id": "bureau_xxx", "user_id": "user_123", ... }
```
