...
| 路由 | 方法 | 用途 |
| --- | --- | --- |
| `/v1/ai/stream` | `POST` | Downcity Model Protocol 模型流 |
...
这是 `CityModel` 使用的版本化 Downcity Model Protocol transport。请求包含协议版本、模型 ID 和标准 `ModelCall`：
...
Federation 通过 SSE 返回 `{ "protocol_version": 1, "event": ModelStreamEvent }`。产品代码通常把模型目录返回的 `CityModel` 交给 Agent，或调用 `city.ai.stream({ model, call })`；两者都直接消费同一标准事件流。

`reasoning.effort` 必须匹配最终执行模型目录中的 `reasoning.efforts[].id`。发生 fallback 时，AIService 按最终模型重新校验该档位。
