...
description: 直接用 @downcity/agent 在 Cloudflare Workers + D1 中部署 City，不需要额外的 edge package。
---

如果你准备把 Downcity 部署到 Cloudflare Workers，推荐做法不是等一个单独的 edge package，而是直接把 `@downcity/agent` 接到 Workers runtime。
...
- 如果你还没搭 City 本体，先读 [Federation](/zh/city-sdk-docs/packages/federation/federation)
