...
如果你只是想理解 Federation 怎么接入 Workers，先读 [Cloudflare Workers](/zh/city-sdk-docs/guides/cloudflare-workers)。这一页只讲上线步骤：`federation.json`、本地创建、本地部署、D1 / Queue / R2、交互式管理员配置、env、OAuth callback 和健康检查。
...
部署输出会按阶段汇总显示：`Project`、`Version`、`Build`、`Typecheck`、`D1 Database`、`Queue`、`Storage`、`Wrangler`、`Deployment` 和 `Federation Registry`。
...
| `CREEM_PRODUCT_ID` | Creem 托管 Checkout 使用的一次性、含税支付 product |
| `CREEM_CURRENCY` | Creem product 的币种，必须与产品实际币种一致 |
| `CREEM_WEBHOOK_SECRET` | Creem webhook 签名密钥；缺失时 Provider 不可用 |
