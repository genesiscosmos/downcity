...
Workspace 默认挂载五个文件与搜索工具，Shell 额外增加两个命令工具，因此 Agent 最终获得七个模型工具：
...
文件和搜索工具只允许访问 Workspace 根目录，即使没有 Shell 也可使用。它们没有 unrestricted 模式；项目外操作仍然必须通过 Shell 的 unrestricted 审批流程。`grep` 和 `find` 会遵守 ignore 文件，默认最多返回 200 项，并允许通过 `max_results` 提高到 2,000 项。
