...
- Workspace 只描述项目资源边界；AgentSessions 承载当前 Agent 的 Session，运行状态位于 Agent 存储作用域。
- 多个 Agent 可以指向同一目录，但应使用不同的 `agent.id` 和独立的 Agent 存储作用域。
- Plugin 在加入 City 后初始化；ActionSchedule 是 Agent 自己的运行资源。
- Session 执行会自动等待 Agent runtime 就绪，调用方不管理独立的启动状态。
- `agent.dispose()` 释放 Agent 的 Session；`city.close()` 释放 Plugin、共享 Workspace、Shell 和 Sandbox。
