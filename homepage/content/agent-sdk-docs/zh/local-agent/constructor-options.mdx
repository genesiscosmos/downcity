...
只需要调整模型输入或上下文策略时，直接提供 Composer 工厂。每次创建、恢复或 Fork Session 都会调用一次，因此 Composer 与 Policy 实例不会跨 Session 共享。

```ts
import {
  Agent,
  AdaptivePartContextPolicy,
...
| 方法 | 用途 |
| --- | --- |
| `initialize(input)` | 在当前 Session 的 `session.db` 初始化所选 Policy 派生表 |
| `compose(input)` | 返回当前模型 Step 使用的 `system`、`messages` 和 `tools` |
| `recover_context(input)` | 在真实 usage 压力或 Provider 上下文超限后推进 Policy 派生状态 |
...
Composer 不消费 Prompt Queue，不控制 Turn，也不修改 canonical Message。它内部的 Context Policy 只能写命名空间隔离、可重建的派生表。只有确实要替换 Session 行为时才使用 `session_class`。
