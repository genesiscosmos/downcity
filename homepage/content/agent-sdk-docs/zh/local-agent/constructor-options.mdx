---
title: Agent 构造参数
description: 详细说明 Workspace 以及本地 Agent 的 id、instruction、model、env、tools 和 plugins 参数
---
...
- Workspace 可以被多个 Agent 共享
- Workspace 生命周期属于调用方，不由 Agent 自动释放
...
```ts
const agent = new Agent({ id, workspace, env: { FOO: "1" } });
