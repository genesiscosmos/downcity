...
`AgentOptions` 不接收 `plugins`。City 先登记 `CityPluginRegistration`，再按 `plugin_id` 和可选 Profile 绑定 Agent：
...
City 持有 Plugin catalog、Profile 共享实例与生命周期；Agent 只持有身份、模型、指令、Tools 和 Sessions。联网能力仍需显式配置 Profile，请参阅 [Web Plugin 文档](/zh/plugins-docs/builtins/web)。
