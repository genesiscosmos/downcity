...
Agent 使用 `@downcity/city` 执行本地命令、管理沙箱运行环境、处理交互式审批。
...
# Linux
pnpm add @downcity/city @downcity/sandbox-linux

# Windows（MXC Development）
pnpm add @downcity/city @downcity/sandbox-windows-mxc

# Windows（Anthropic SRT Alpha）
pnpm add @downcity/city @downcity/sandbox-windows-srt
...
`@downcity/city` 只维护 workspace 写边界、路径校验、环境收敛和策略指纹。系统目录、宿主预检、策略编译和进程启动由独立 adapter 负责。MXC 与 Anthropic SRT 只属于各自的 Windows package，macOS/Linux 用户不会因为安装 `@downcity/city` 下载 Windows runtime。Downcity CLI 默认继续使用 MXC；设置 `DC_WINDOWS_SANDBOX=srt` 才会显式启用 SRT。SDK 用户需要安装并注入对应 adapter。
