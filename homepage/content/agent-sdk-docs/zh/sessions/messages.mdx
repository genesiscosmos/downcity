...
`SessionMessage` 是持久化数据结构，内部 `SessionUiMessage` 是 Executor 与 UI 的流式投影。User / Assistant 的可序列化 UI parts 可以转换成 Session parts，再恢复为等价投影；`action` 和 `error` 是 Downcity Session 自己的顶层类型，不会送入模型。
...
| `source` | `source_type`, `source_id`, URL 或文档字段 | URL 或文档来源 |
...
Tool Part 会保留 Provider Adapter 附着在工具调用和工具结果上的 metadata：
...
文本、推理、文件和 source 同样保留各自的 `provider_metadata`。相同 `data_id` 或 `source_id` 的流式更新会替换原 Part 并保留 `sequence`；带 `transient: true` 的 Session data chunk 只用于实时 UI，不写入 Session。
