...
普通 Tool Loop、Provider continuation 和恢复重试不会创建新的 Assistant Message；它们继续向当前 Assistant 追加有序 Part。只有新的 User Message（包括运行中 steer）真正进入消息序列时，才结束之前的 Assistant Message，后续输出再创建下一条 Assistant Message。模型 step 是执行期边界，不作为 Part 持久化。

`SessionMessage` 是持久化与 UI 时间线的唯一事实源。Executor 直接消费 Downcity `ModelStreamEvent`，把文本、推理和工具事件更新到 canonical Assistant Message，同时由 `SessionMutation` 发布实时变化；中间不存在 UI Message 或 chunk 协议。`action` 和 `error` 是 Session 顶层类型，不会送入模型。
