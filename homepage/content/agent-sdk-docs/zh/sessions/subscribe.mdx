...
| --- | --- | --- | --- |
| `message` | `user | agent` | 顶层 Message 的完整快照 | 是 |
| `part` | `text | reasoning | tool | file | data` | Assistant Part 的完整快照；`tool` Part 携带其 `interactions` | 是 |
| `delta` | `text | reasoning | tool_input` | 新增的原始文本或 Tool 输入片段 | 是 |
| `turn` | `start | finish` | Turn 执行生命周期 | 否 |
| `file_diff` | — | 当前 Turn 的文件改动实时摘要 | 否 |
| `session` | `title | config` | Session 自身属性变化 | 否 |
| `warning` | `model_request` | 模型请求失败与重试进度 | 否 |

`message`、`part` 和 `delta` 都在对应完整 Message 持久化成功后才发布。Mutation envelope 本身不写入 canonical history。
...
上下文压缩不是独立 Mutation，也没有对应的 public Handle。它是 `SessionComposer.advance_context()` 的结果，写成 Action Part：成功时为 `completed`，失败时为 `failed`，`action_type` 为 `context-compaction`。它不会改写 canonical 历史，只推进 Composer 的派生表（见 [元数据与落盘](/zh/agent-sdk-docs/sessions/metadata-and-storage)）。

模型调用失败时，`warning / model_request` 会携带 `request_kind`、`code`、`message`、`retryable`、`attempt`、`max_attempts` 与 `will_retry`。主 Turn 请求还携带 `turn_id`，后台标题与历史压缩请求允许不带。它是实时诊断，不写入 Message 历史；`retryable` 表示 Provider 的错误分类，`will_retry: false` 表示本次失败不会再自动重试。
...
`input-streaming` 表示模型仍在输出工具参数。此阶段参数原文通过 `tool_input` Delta 追加到 Tool Part 的 `input_text`；`ready` Part 是输入完整后的校准快照，并携带解析完成的 `input`。UI 应以 `tool_call_id` 或 `part_id` 做稳定 key，不要为每个 Delta 新增一项。

如果参数流结束时不是有效 JSON，同一个 Part 会直接从 `input-streaming` 进入 `failed`，`error` 会说明输入解析失败；Tool 实现不会执行。Tool 正常返回结构化 `{ success: false }` 时也会进入 `failed`。客户端只应根据 Part 的 `state` 和 `error` 展示结果，不要根据输出文本猜测成功与否。
