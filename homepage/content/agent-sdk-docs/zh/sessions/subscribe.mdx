...
上下文压缩不是独立 Mutation，也没有对应的 public Handle。它是一次 Session 操作，结果写成 Action Part：成功时为 `completed`，失败时为 `failed`，`action_type` 为 `context-compaction`。压缩不会改写 canonical 历史，只推进 Context Policy 的派生表（见 [元数据与落盘](/zh/agent-sdk-docs/sessions/metadata-and-storage)）。
