...
| `SessionInteractionRequest` | 当前 Session 等待用户响应的请求 | 审批、问答、后台处理 |
...
两者都有 `prompt()`、`stop()`、`subscribe()`、`messages()`、`interactions()`、`respond()`、`status()`、`set({ security })` 和 `fork()`。本地 Session 还提供 `set({ model })`、`rename()`、`syncshot()` 与 `snapshot()`；远程 Session 的模型和 system 策略由服务端宿主管理。
