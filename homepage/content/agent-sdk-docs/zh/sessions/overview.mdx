...
`Agent` 拥有身份、模型、自定义工具与 Plugin；`AgentWorkspace` 负责组合 Workspace Tool、存储与 Session。真正承载一次连续对话、历史、流式输出与工具审批的是 `Session`。
...
- 管理本地会话：`agent_workspace.sessions.create/get/list/archive/archived/clean_archive/remove/clear_messages()`
