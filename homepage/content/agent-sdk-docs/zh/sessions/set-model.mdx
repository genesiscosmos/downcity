...
`session.set({ model })` 让本地 SDK Session 使用自己的 `ModelClient` 覆盖 Agent 模型。这里可以直接传入 Federation 目录返回的 `CityModel`，或其他实现 Downcity Model Protocol 的客户端。
...
- `persist_action` 默认为 `true`；设为 `false` 时不创建包含配置 Action Part 的 Agent Message。
- `publish_mutation` 默认跟随 `persist_action`；设为 `false` 时可以保留历史 Action Part，但不向 `subscribe()` 发布对应 Message Mutation。
...
- Agent 负责身份、模型、自定义工具与 Plugin；AgentSessions 负责 Session，Session 可选择项目工具
