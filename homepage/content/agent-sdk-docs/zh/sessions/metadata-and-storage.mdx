...
`session_id` 在一个 Agent 内唯一。`meta.json` 总是记录 `agent_id`，只有创建 Session 时传入
Workspace 才记录 `workspace_id`。不同 Agent 可以复用相同 `session_id` 而不会共享数据，项目目录
不会创建 `.downcity`。
...
`meta.json` 保存列表和详情所需的轻量属性，包括 `session_id`、`agent_id`、标题、模型标签、时间戳、消息数、历史字节数和时区。运行时模型实例不会持久化。Session 属性或消息状态变化时，SDK 会同步更新这些摘要字段。
