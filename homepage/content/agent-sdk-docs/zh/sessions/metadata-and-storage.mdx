...
`session_id` 在 Workspace 内唯一。`meta.json` 同时记录 `agent_id` 与 `workspace_id`，SessionStore 通过这两个字段校验归属并过滤列表。不同 Agent 不能在同一 Workspace 中复用相同 `session_id`，项目目录不会创建 `.downcity`。
