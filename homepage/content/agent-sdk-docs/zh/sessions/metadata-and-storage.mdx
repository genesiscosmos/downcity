...
SDK Session 默认保存在用户级 AgentWorkspace 数据目录中。应用应通过 Session API 读写，不要把这些文件当作自己的数据库。
...
`agent_id` 和 `workspace_id` 共同构成存储分区。同一项目中的多个 Agent 可以使用相同 `session_id` 而不冲突，项目目录不会创建 `.downcity`。
