...
Session 以 `(origin.type, session_id)` 作为身份。默认来源是 `chat`；`group`、`task` 或其他任意
非空字符串会进入独立分区，自定义来源类型由路径层安全编码为单个目录段。`meta.json` v2 始终记录
`agent_id` 和完整且不可变的 `origin`，只有创建 Session 时传入 Workspace 才记录 `workspace_id`。
同一个 `session_id` 可以存在于不同来源分区而互不共享数据，项目目录不会创建 `.downcity`。

`schema.json` 标记当前 Message 存储格式。首次读取旧 Session 时，SDK 会在 Session Message 写锁内，把旧顶层 `assistant` 转为 `agent`，并把旧顶层 `action`、`error` 下沉为 Agent Part。Segment、Active 与流式草稿逐文件原子替换，全部完成后才提交版本标记；迁移中断不会产生已完成标记，下次读取会继续重试。归档 Session 在浏览时也执行同一迁移。
...
sequenceDiagram
    ...
    Session->>Loop: Queue 与 Turn 编排
    Loop->>Messages: 创建 User Message
    ...
    Loop->>Messages: 更新 Assistant 草稿与 Part
    Messages->>Store: 原子覆盖 agent_message.json
    Loop->>Messages: 完成 Assistant Message
    ...
    Composer->>Model: 旧 Summary + Active 最旧 50% 上下文
...
`meta.json` 保存 `v: 2`、`session_id`、`agent_id`、完整 `origin`，以及标题、模型标签、时间戳、消息数、历史字节数和时区等列表摘要。运行时模型实例不会持久化。Session 属性或消息状态变化时，SDK 会同步更新摘要字段；`origin` 在创建后不会改变。
