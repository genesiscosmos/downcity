...
- 一个 Assistant Message 只渲染一次角色标题；Tool Loop 和 Provider continuation 产生的新 Part 继续留在同一角色容器中。
- `text` / `reasoning` Delta 追加到当前文本 Part，`tool_input` Delta 追加到当前 Tool Part 的 `input_text`。
- `part` 用稳定 `part_id` 覆盖完整状态。
- `message` 用 `message_id + revision` 覆盖完整 Message。
- 已收口 Agent Message 内的 Error Part 是一次失败唯一的用户可见错误提示。
- 所属 Tool Part 内的 Interaction 为 `pending` 时显示交互控件；该 Tool 为 `waiting-user` 时显示等待状态。

不要创建 Transcript Message DTO 或把工具调用平铺到第二份可持久化列表，也不要用最终 `turn.text` 覆盖已经按 Part 渲染的 Agent Message。`turn.finished.error` 只用于控制流和日志；若已经渲染 Error Part，不要再追加一条错误提示。

## 审批卡片

审批 UI 从所属 Tool Part 上的 pending Interaction 或 `session.interactions()` 得到相同身份：
...
4. 不复用旧会话的 `interaction_id`、`part_id` 或 Turn loading 状态。
