...
订阅会收到 `message | part | delta | turn | file_diff | session | warning` 七类 Mutation。用户参与请求表现为所属 Tool Part 上的 pending Interaction，该 Tool 同时进入 `waiting-user`；模型调用失败会产生 `warning / model_request`。通过 `session.respond(...)` 提交结构化响应。
