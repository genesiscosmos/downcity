...
- `file_diff`：当前 Turn 的文件改动实时摘要
- `session`：Session 自身状态变化，例如 `title`
- `warning`：不写入历史的运行期警告，例如模型请求失败与重试

Interaction 作为所属 Tool Part 的从属数据进入 `pending` 并携带完整请求；该 Tool 同时进入 `waiting-user`。订阅保持为纯数据通道，用户决定通过 Session 命令 API 提交：
