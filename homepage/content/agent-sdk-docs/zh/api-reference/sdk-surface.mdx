...
- `City`：持有 Workspaces、Plugin catalog、共享实例与 Agent 绑定。
...
- `city.plugins`：提供 Plugin、管理 Agent 绑定并调用 main action。
...
- `agent.dispose()` 会释放 Session 使用的资源并解除当前 City 绑定；Plugin 共享实例和生命周期由 City 收口。
