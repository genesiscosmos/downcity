...
```ts
const workspace = new Workspace({ id: workspace_id, path, data_root_path, shell });
...
网络服务实现位于 `@downcity/city`。本地 `Agent` 不监听端口，也不管理 transport 生命周期。
...
- 每个 AgentWorkspace 独立持有 Session、私有数据、Shell 和 Plugin 上下文。
