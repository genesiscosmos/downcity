...
global downcity.db: agent.execution.modelId
  ...
  -> Agent 通过 Downcity Model Protocol 调用 CityModel.stream()
...
- 模型目录
- Downcity Model Protocol 通路
- CityModel 原生 `ModelClient` 通路
- OpenAI 兼容通路
...
1. 读取 City 连接配置
2. 通过 Embassy User 调 `embassy.user.ai.catalog()`
3. 检查 `execution.modelId` 是否存在
4. 从目录取得已实现 `ModelClient` 的 `CityModel`
