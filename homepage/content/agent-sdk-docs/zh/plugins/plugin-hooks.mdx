...
Plugin 内部只能通过 `context.city.plugins.pipeline()`、`effect()` 和 `run_action()` 协作，不能主动触发 `guard` 或 `resolve`。后两者代表宿主控制流和唯一权威决策，只从 `city.plugins.scope({ agent_id: agent.id, workspace_id: workspace.id })` 等宿主执行面调用。
