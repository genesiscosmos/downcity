...
- Agent 级外部能力通过 `context.services` 注入；Plugin 只依赖实际需要的服务接口。
...
`index.mjs` 是安装产物的自包含 ESM 入口。`src/` 和构建配置可以保留在源码仓库中，但安装目录必须包含 `plugin.json`、`package.json`、`README.md` 和入口文件。
...
不要通过重新引入旧的 callback 构造参数来测试。服务应通过 `Agent.services` 或测试用的 `PluginContext` 注入。
