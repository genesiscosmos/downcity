...
```ts
const value1 = await city.plugins.scope({ agent_id: agent.id, workspace_id: workspace.id }).pipeline("chat.enrich_message", input);
await city.plugins.scope({ agent_id: agent.id, workspace_id: workspace.id }).guard("review.require_checked", value1);
await city.plugins.scope({ agent_id: agent.id, workspace_id: workspace.id }).effect("audit.observe", value1);
