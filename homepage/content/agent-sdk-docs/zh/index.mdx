...
```mermaid
flowchart LR
    Host["SDK 宿主"] --> Agent["Agent<br/>身份与 Plugin 注册表"]
...
| 对象 | 负责 | 不负责 |
| --- | --- | --- |
| `Workspace` | 项目文件、Env、Tools、Shell 与 Sandbox | Agent 身份、Plugin、Session 持久化 |
| `Agent` | 模型、指令、PluginRegistry，并可进入多个 Workspace | HTTP/RPC transport |
| `AgentWorkspace` | 当前 Workspace 的 Session、私有 Storage 与 PluginContext | 其他 Workspace 的状态 |
...
网络服务不属于 Agent。需要暴露本地 Agent 时，使用 `@downcity/agent` 提供的 `AgentHTTP` 或 `AgentRPC`。
...
`agent.dispose()` 会释放 Agent 进入的全部 AgentWorkspace，包括 Plugin lifecycle、ActionSchedule、Shell 进程、PTY 和 Sandbox。项目目录不会创建 `.downcity`。
