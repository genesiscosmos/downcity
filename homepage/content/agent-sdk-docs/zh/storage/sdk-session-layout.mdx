...
本地 SDK Agent 的 Session 默认集中落盘到用户级数据根目录。`data_root_path` 通常是 `~/.downcity`：
...
`agent_id` 隔离 Agent，`workspace_id` 隔离 Agent 进入的项目作用域，`session_id` 隔离每条连续对话。项目目录不会创建 `.downcity`。
