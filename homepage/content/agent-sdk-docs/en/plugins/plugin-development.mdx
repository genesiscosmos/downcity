...
- Host capabilities are supplied by the Agent through `context.services`; a Plugin depends only on the service interfaces it needs.
...
`index.mjs` is the self-contained ESM entry in the installation artifact. Source files and build configuration may remain in the source repository, but an installed directory must contain `plugin.json`, `package.json`, `README.md`, and the entry file.
...
const agent = new Agent({
  ...
  services: {
    ai: {
      list_models: () => city.services.list_models(),
      image_create: (input) => city.services.image_create(input),
      image_result: (input) => city.services.image_result(input),
      asr: (input) => city.services.asr(input),
      tts: (input) => city.services.tts(input),
    },
  },
...
```ts
const models = await context.services.ai?.list_models();
...
run: create_action({
  ...
  execute: async ({ input, execution_context }) => {
    execution_context.signal.throwIfAborted();
...
Do not restore legacy callback constructor arguments just to make tests easier. Inject services through `Agent.services` or a test `PluginContext`.
