...
Inside a Plugin, `context.city.plugins` exposes only `pipeline()`, `effect()`, and `run_action()` for collaboration. `guard` and `resolve` remain host-control and authority surfaces and are called from host execution entries such as `entry.plugins`.
