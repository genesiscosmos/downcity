...
When an action is called by a Session tool, its parameters also include the explicit `run_context` for that Session turn. It may be absent for direct CLI, HTTP, or scheduler calls. Use `run_context.workspace_env` and `run_context.agent_systems` for the effective step configuration; `context.workspace_env` is the current Workspace snapshot and `context.instructions` contains Agent instructions.
