...
When an action is called by a Session tool, its parameters also include the explicit `execution_context` for that Session Turn. It may be absent for direct CLI, HTTP, or scheduler calls. Use `execution_context.workspace_env` and `execution_context.agent_systems` for the effective Step configuration; `context.workspace_env` is the current Workspace snapshot and `context.instructions` contains Agent instructions.
