...
`execution.call_id` matches the Tool Call ID for model tools. For Session calls, `execution.session` contains `session_id`, the complete `origin`, `turn_id`, and optional `interactions`. `execution.snapshot` contains `session_origin`, effective `workspace_env`, and `agent_systems`. Direct CLI, HTTP, and scheduler calls receive a runtime-generated `call_id` and cancellation signal without a Session scope. Pass `execution.abort_signal` to network requests, polling, and other long-running operations.
