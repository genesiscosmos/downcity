...
- `agent.sessions.get(session_id, origin_type = "chat")`
- `agent.sessions.list(input?)`
- `agent.sessions.archive({ id, origin_type? })`
...
You can also handle a pending Interaction through the unified Mutation stream:
