...
## Paths and the Workspace mount

File and search tools run in the host process, while Shell commands run inside the Sandbox. Both accept a sandbox-internal path such as `/workspace/src/main.ts`: the sandbox path is mapped back to the project root before the path policy runs, so `/workspace/src/main.ts` and `src/main.ts` address the same file. Results always report host-side paths. Paths outside the mount, such as `/etc/hosts`, are still rejected.
