...
The Agent uses `@downcity/city` for local command execution, sandboxed runners, shell session management, and interactive approval flows.
...
`@downcity/city` owns the workspace write boundary, path validation, environment convergence, and policy fingerprint. Independent adapters own system paths, host preflight, policy compilation, and process startup. MXC and Anthropic SRT belong only to their Windows packages, so installing `@downcity/city` does not download a Windows runtime on macOS or Linux. The Downcity CLI keeps MXC as its default and enables SRT only when `DC_WINDOWS_SANDBOX=srt` is set. SDK applications install and inject their selected adapter explicitly.
