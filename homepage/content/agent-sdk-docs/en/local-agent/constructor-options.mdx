...
`id` and `name` become the Agent Identity block at the top of every Session system prompt:
...
Both values are always given, so the model can use the readable name while hosts can still reconcile against the stable `id`.
...
- the host pushes one AgentTool per Power, named after that Power
