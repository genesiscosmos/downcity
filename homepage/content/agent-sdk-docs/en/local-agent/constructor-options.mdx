...
## `workspace` and `agent.enter(workspace)`
...
- Agent is not bound to a Workspace at construction time; `agent.enter(workspace)` creates an AgentWorkspace execution scope
...
`Workspace` requires a stable `id`, the real project `path`, and a user-level `data_root_path`:
