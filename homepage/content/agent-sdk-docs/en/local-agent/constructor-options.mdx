...
Use a factory when you only need to customize model input or the context policy. The factory is called once for every created, restored, or forked Session, so Composer and Policy instances are never shared across Sessions.
...
| Method | Purpose |
| --- | --- |
| `initialize(input)` | Initialize the selected Policy's derived tables in this Session's `session.db` |
| `compose(input)` | Return the `system`, `messages`, and `tools` for the current model Step |
| `recover_context(input)` | Advance Policy-derived context after real usage pressure or a provider context-limit error |
...
A Composer does not consume the Prompt Queue, control Turns, or modify canonical Messages. Its Context Policy may only write namespaced, rebuildable derived tables. Use `session_class` only when replacing Session behavior itself.
