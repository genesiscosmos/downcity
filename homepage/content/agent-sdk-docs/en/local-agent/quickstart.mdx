...
```bash
pnpm add @downcity/agent @downcity/federation @downcity/city @downcity/sandbox-macos
```
...
- Workspace describes project resources only; AgentSessions owns Sessions and Agent runtime state remains in the Agent storage scope.
- Multiple Agents may target the same directory, using distinct agent IDs and isolated Agent storage scopes.
- Plugins initialize after entering City; ActionSchedule is an Agent-owned runtime resource.
- Session execution waits for the Agent runtime automatically; callers do not manage a separate startup state.
- `agent.dispose()` releases the Agent's Sessions. `city.close()` releases Plugins, shared Workspaces, and Shell resources.
