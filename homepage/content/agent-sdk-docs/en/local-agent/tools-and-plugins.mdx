...
`image_result` reads the current job state once by default. If it returns `queued` or `running`, keep the `job_id` and call `image_result` again later. For short jobs, pass `until_done: true` to wait for a terminal state in one tool call, with optional `max_wait_ms` / `poll_interval_ms`. If it returns `succeeded`, `data` contains the job result and image File Parts are also copied into the current Assistant Message. The URL in each File Part is returned by the configured image provider or City Storage and may be a remote URL.
...
The `image_result` callback returns a UIMessage File Part with the provider's image URL. The URL may be remote, a data URL, or a local path; ImagePlugin does not download or relocate the result. Session persists that File Part as returned, and the action also exposes the result in `data` for direct callers.
