...
Plugin author APIs come from `@downcity/plugin`. Action, Hook, System, and Availability receive a `PluginContext` with restricted `city`, `agent`, `workspace`, `session`, `turn`, `profile`, and `storage` handles.
