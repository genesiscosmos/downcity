...
Workspace creates WorkspaceTools from five file/search tools by default, and Shell contributes two command tools. The Session combines them with Agent custom Tools:
...
When `read` opens a PNG, JPEG, GIF, WebP, BMP, or PDF, the result includes a User File Part for the next model step. The internal Tool object is not a public Agent or Workspace API; the Session assembles and invokes these tools during `prompt()`.
...
`context.sessions` references the Agent's `AgentSessions`; it is not a second Session owner. Plugins use `context.sessions.runtime(session_id, origin_type = "chat")` for the synchronous runtime port. SDK callers restore an existing public Session with `await agent.sessions.get(session_id, origin_type, { workspace })`.
...
The `zod` schema handles runtime validation. `json_schema` and `examples` are returned by `plugin_read` for the model or UI. Every Action receives a stable `context` capability view and a per-call `execution` object. Session identity and interactions are available through `execution.session`; the effective Step env is available through `execution.snapshot.workspace_env`. Direct CLI, HTTP, or scheduler calls have no `execution.session`. `context.workspace_env` remains the current Workspace configuration snapshot.
...
The `image_result` callback may return a remote URL or a local path. For a remote URL, ImagePlugin uses the downloaded Workspace path as the File Part's primary `url`. If local storage fails, it retains the remote URL and reports the failure through the action message. Session persists the resulting File Part, while direct callers can read the same result from `data`.

## plugin HTTP

If Downcity exposes the City HTTP gateway, registered plugins with `plugin.http.server.register(...)` are also mounted onto the same HTTP app automatically.

So:

- plugin existence comes from the `plugins` array you pass into `Agent`
- plugin HTTP exposure happens when Downcity publishes the City HTTP gateway
