...
`context.sessions` references the Agent's `AgentSessions`; it is not a second Session owner. Plugins use `context.sessions.runtime(session_id)` for the synchronous runtime port. SDK callers restore an existing public Session with `await agent.sessions.get(session_id, { workspace })`.
