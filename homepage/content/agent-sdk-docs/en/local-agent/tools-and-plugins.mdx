...
It supports `text`, `single_select`, and `multi_select`. Once the user responds, the answers become
the Tool Result and the model continues in the next Step of the same Turn. See
...
The `image_result` callback must save generated images locally before returning a UIMessage File Part with either an absolute local path or a path relative to the Agent project root. HTTP and data URLs are not valid generated-result paths. Session persists that File Part as returned; it does not maintain a shared Resource directory or duplicate file summaries in `plugin_call` output.
