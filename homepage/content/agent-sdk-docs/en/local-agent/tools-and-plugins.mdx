...
Workspace mounts five file and search tools by default. Shell adds two command tools, so the Agent receives seven model tools in total:
...
File and search tools can only access the Workspace root and remain available without Shell. They have no unrestricted mode; operations outside the project still require the Shell unrestricted approval flow. `grep` and `find` respect ignore files, return at most 200 results by default, and accept `max_results` up to 2,000.
