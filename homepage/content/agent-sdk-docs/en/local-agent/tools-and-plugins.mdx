...
Agent always provides its Session interaction tool. Workspace creates WorkspaceTools from five file/search tools by default, and Shell contributes two command tools. Agent registers all of them in `agent.tools`:

- `ask_question`: pause the current Tool and ask structured questions when missing information would materially change the outcome
