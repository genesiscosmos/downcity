...
- when the Session is idle, the command runs immediately without waiting for another `prompt()`
- when a turn is active, it runs at the next Session step checkpoint
- if the active turn ends before another checkpoint, the command runs independently after the turn closes
- compact commands, configuration commands, and steer prompts preserve their queue order
- calling `compact()` alone never creates a turn; it starts its own model request when a summary is required
...
`subscribe()` still emits `compact` lifecycle mutations and timeline actions for UI state. Application logic should normally await `handle.finished` instead of inferring completion from Action Parts.
