...
```text
~/.downcity/agents/<agent_id>/sessions/<origin_type>/<session_id>/instruction.md
```
...
`snapshot()` writes the complete system, including the Agent identity block, custom instruction, SDK core, Power system, and Session context. It does not persist message history, model, or environment variables. Multiple system blocks are merged in order into one Markdown document, and later calls atomically replace the existing file.
