...
SDK Sessions are persisted under the user-level AgentWorkspace data directory. Applications should read and write through the Session API rather than use these files as their own database.
...
`agent_id` and `workspace_id` form the storage partition. Multiple Agents in one project can use the same `session_id` without conflict, and the project directory never receives `.downcity`.
