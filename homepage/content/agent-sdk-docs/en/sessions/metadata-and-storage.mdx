...
`session_id` is unique within a Workspace. `meta.json` records both `agent_id` and `workspace_id`; SessionStore validates those fields and filters lists by ownership. Different Agents cannot reuse the same `session_id` in one Workspace, and the project directory never receives `.downcity`.
