...
`session_id` is unique within one Agent. `meta.json` always records `agent_id` and records `workspace_id`
only when the Session was created with a Workspace. Different Agents may reuse the same `session_id`
without sharing data, and the project directory never receives `.downcity`.
