...
SDK Sessions are persisted under the Agent scope supplied by City storage. Without a City, the default
provider is in-memory. Applications should read and write through the Session API rather than use these
files as their own database.
...
A Session is identified by `(origin.type, session_id)`. The default origin is `chat`; `group`, `task`,
or any other non-empty string selects another partition. The path layer safely encodes custom origin types
as one directory segment. Metadata schema v2 always records `agent_id` and the complete immutable `origin`,
and records `workspace_id` only when the Session was created with a Workspace. The same `session_id` may
exist in different origin partitions without sharing data, and the project directory never receives `.downcity`.

`schema.json` marks the current Message storage format. On the first read of an older Session, the SDK takes the Session Message write lock, converts top-level `assistant` records to `agent`, and moves top-level `action` and `error` records into Agent Parts. Segments, Active, and the streaming draft are replaced atomically one file at a time, and the version marker is committed only after all files succeed. An interrupted migration is retried on the next read. Archived Sessions follow the same migration path when browsed.
...
sequenceDiagram
    ...
    Session->>Loop: Queue and Turn orchestration
    ...
    Model-->>Loop: text / tool / file chunks
    Loop->>Messages: update Assistant draft and Parts
    Messages->>Store: atomically replace agent_message.json
    ...
    Composer->>Model: old Summary + oldest 50% of Active context
...
`meta.json` stores `v: 2`, `session_id`, `agent_id`, the complete `origin`, and lightweight list and detail fields such as title, model label, timestamps, message count, history bytes, and timezone. Runtime model instances are never persisted. The SDK updates these summaries as Session properties and Message state change; `origin` is fixed at creation time.
