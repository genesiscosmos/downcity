...
`Agent` owns identity, model, custom tools, and Plugins. `AgentWorkspace` assembles Workspace tools, storage, and Sessions. A `Session` owns one continuous conversation, its history, streaming output, and tool approvals.
...
- Manage local Sessions: `agent_workspace.sessions.create/get/list/archive/archived/clean_archive/remove/clear_messages()`
