...
Both expose `prompt()`, `stop()`, `compact()`, `subscribe()`, `messages()`, `interactions()`, `respond()`, and `fork()`. Local Sessions additionally provide `syncshot()` to regenerate the system and `snapshot()` to explicitly persist it; remote refresh and persistence policy belongs to the server host.
...
- Tool approval: `session.interactions()`, `session.respond()`, and `session.approval_mode()`
