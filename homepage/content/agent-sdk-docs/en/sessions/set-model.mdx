...
- `persist_action` defaults to `true`. When false, no configuration Action Message is created.
- `publish_mutation` defaults to the value of `persist_action`. Set it to false to keep the historical Action without publishing its Mutation through `subscribe()`.
