...
`assistant_message` exists only when the Turn produced actual Assistant content. If the model or runtime fails before producing output, the result contains only `success: false` and `error`; the Session persists the user-visible failure as a canonical `error` Message.
