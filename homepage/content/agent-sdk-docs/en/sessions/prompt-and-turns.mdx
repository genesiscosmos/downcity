...
The `SessionUserMessagePart` input uses `mediaType`; persisted `SessionMessage` values use `media_type`. These are the Session input and persistence boundaries, so do not feed a read Message Part back into `prompt()` unchanged.
