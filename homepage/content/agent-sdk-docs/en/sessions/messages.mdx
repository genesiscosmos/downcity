...
`SessionMessage` is the persistence format; the internal `SessionUiMessage` is the streaming projection used by the Executor and UI. Serializable User and Assistant UI parts can be projected into Session parts and restored into an equivalent view. `action` and `error` are Downcity Session top-level types and are never sent to the model.
...
| `source` | `source_type`, `source_id`, URL or document fields | A URL or document source |
...
A Tool Part preserves metadata attached to the tool call and tool result by the Provider Adapter:
...
Text, reasoning, file, and source parts also preserve their own `provider_metadata`. Stream updates with the same `data_id` or `source_id` replace the existing Part without changing its `sequence`. A Session data chunk with `transient: true` is delivered to the live UI but is not persisted in the Session.
