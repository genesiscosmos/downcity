...
Context compaction is not a separate Mutation and has no public Handle. It is a `SessionComposer.advance_context()` result recorded as an Action Part: `completed` on success, `failed` on failure, with `action_type` set to `context-compaction`. It never rewrites canonical history; it only advances the Composer's derived tables (see [Metadata and storage](/en/agent-sdk-docs/sessions/metadata-and-storage)).
