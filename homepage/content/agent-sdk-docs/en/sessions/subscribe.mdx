...
| `part` | `text | reasoning | tool | file | data` | Complete Assistant Part snapshot; a `tool` Part carries its `interactions` | Yes |
...
Context compaction is not a separate Mutation and has no public Handle. It is a Session operation whose result is recorded as an Action Part: `completed` on success, `failed` on failure, with `action_type` set to `context-compaction`. Compaction never rewrites canonical history; it only advances the Context Policy's derived tables (see [Metadata and storage](/en/agent-sdk-docs/sessions/metadata-and-storage)).
