...
After a disconnect, do not assume Deltas can be replayed. Lost Deltas have already been merged into `agent_message.json` or a final Message snapshot, so reloading `messages()` is what restores correct state.
