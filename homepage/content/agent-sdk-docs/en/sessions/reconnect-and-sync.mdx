...
| `delta` | Append `delta` to matching text/reasoning `text` or Tool `input_text` |
| `turn` | Update runtime state by `turn_id` |
| `session` | Update Session metadata such as the title |

After a disconnect, do not assume Deltas can be replayed. Deltas from completed steps are already part of a stable SQLite snapshot; an interrupted unfinished projection may be discarded and closed with an Error Part. Reload `messages()` to restore authoritative state.
...
The initial snapshot loads the most recent Message page. To load earlier history, pass its `next_before_sequence` with an appropriate `limit`. Live Mutations update the current snapshot; never let an older revision replace a loaded Message.
...
- Append Deltas only while the stream is continuous; validate `tool_input` against its `tool_call_id`; after reconnect, prefer the snapshot.
