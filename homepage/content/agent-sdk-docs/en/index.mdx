...
flowchart LR
    ...
    Agent --> Workspace["agent.sessions.create({ workspace })<br/>Session scope"]
...
Network serving does not belong to Agent. Use `AgentHTTP` or `AgentRPC` from `@downcity/agent` to expose a local Agent.
