...
`channelAccountId` 仍可作为账号池绑定能力使用，但必须由宿主显式注入 `account_store`。Chat Plugin 只依赖存储契约，不读取 CLI 的 `~/.downcity`、SQLite 或密钥文件：
...
如果不使用共享账号池，SDK 主路径仍推荐直接把 channel 需要的 env 传给对应 channel 对象。
