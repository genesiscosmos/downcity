...
CLI 不再为 Chat 建立单独的配置命令。使用统一 Plugin 管理器选择账号、启用渠道并保存当前 Agent 的 Binding：
...
`channel_account_id` 可作为账号池绑定能力使用，但必须由宿主显式注入 `account_store`。Chat Plugin 只依赖存储契约，不读取 CLI 的 `~/.downcity`、SQLite 或密钥文件：
