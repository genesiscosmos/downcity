...
- a plugin does not have to be “used” mainly through `run_action(...)`
...
It does not use `system` or `hooks`. Its job is focused: provide one controlled, authenticated, observable snapshot surface.
...
For `workboard`, the important question is not “how do I `run_action` it?”
