...
The CLI does not have a separate Chat configuration command. Use the shared Plugin manager to select accounts, enable channels, and save the current Agent Binding:
...
`channel_account_id` can bind an account from a shared pool, but the host must explicitly inject an `account_store`. Chat Plugin depends only on the storage contract and never reads the CLI's `~/.downcity`, SQLite database, or key files:

```ts
import type { ChatChannelAccountStore } from "@downcity/plugins/chat";

const account_store: ChatChannelAccountStore = create_my_account_store();
...
When no shared account pool is needed, the SDK-first path remains passing channel-specific env directly to the channel object.
