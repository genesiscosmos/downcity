...
In embedded SDK scenarios, `ChatPlugin` receives already constructed channel objects. The SDK host directly provides the ID, display name, and runtime credentials; it does not read City Profiles:
...
Profiles belong to the City control plane. They do not enter `@downcity/agent` or become a Store dependency of `ChatPlugin`. Chat-owned main actions validate Profile content and provide a credential-safe projection.
