...
Use `create_builtin_plugin_registrations()` for the default collection, or import and instantiate
only the Plugins required by a City. Selection happens at City level, not per Agent.
...
| `web` | web methodology injection and dependency preparation | action + system |
