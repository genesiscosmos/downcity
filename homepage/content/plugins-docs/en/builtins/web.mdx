...
`web` is a real web-capability plugin. City owns one shared instance and one configuration. The Plugin creates browser providers lazily for Agent isolation. Search and document services are read from the current `PluginContext`.
...
- `WebPlugin` accepts explicit constructor options or reads the City-level Config and creates the selected browser provider internally.
- The provider owns browser sessions and is disposed when the final execution lease is released and City stops the shared instance.
