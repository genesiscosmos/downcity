...
`web` is a real web-capability plugin. City creates a shared instance from a Profile, and the Plugin creates the selected browser provider internally. Search and document services are read from the current `PluginContext`.
