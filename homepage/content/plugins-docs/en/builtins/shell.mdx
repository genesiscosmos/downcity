...
Shell is no longer a plugin. It is a built-in capability provided by `@downcity/city`, composed with an independent platform Sandbox Adapter, and mounted on an Agent.
...
- Shell is owned by `@downcity/city`, not `@downcity/plugins`.
