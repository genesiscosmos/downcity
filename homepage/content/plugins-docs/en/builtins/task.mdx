...
`TaskPlugin` only needs the optional timezone setting. Every Task binds one execution Workspace; optional `session_id` only delivers the completed result to a Session. Agent tasks prefer that Session's model when configured and otherwise use the Agent model. By default, cron uses the current machine timezone:
