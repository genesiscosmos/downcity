...
When a definition commits but incremental scheduler synchronization fails, the host action returns the synchronization failure and Desktop reports that the Task was saved but scheduler synchronization failed. A missing or unparseable canonical `task.md` fails visibly instead of making a damaged definition appear absent.
...
- Session-scoped `create` automatically stores the current `(agent_id, workspace_id, origin.type, session_id)` and appends completed results to that Session
...
- all definition mutations commit serially per TaskPlugin instance; one-shot completion conditionally updates the latest definition
