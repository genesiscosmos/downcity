...
Task does not declare Config, so it needs no Profile. It remains visible in the Plugins Catalog for its documentation and also contributes a top-level navigation entry. Its Sidebar groups every Task as `Agent → Task`, while the Mainview can create, edit, enable, pause, run, and delete Tasks. Selecting a Task opens an execution-history Subbar to the right of the primary Sidebar, sorted newest first and defaulting to the latest run. Workspace is a Task-level execution setting rather than a page filter. This workspace reads the selected Agent's real Task runtime and existing run artifacts instead of copying state.
