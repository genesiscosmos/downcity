...
Task does not declare Config, so it needs no Profile. It remains visible in the Plugins Catalog for its documentation and also contributes a top-level navigation entry. Opening the Task entry shows its own Sidebar for selecting an Agent and Task, while the Mainview selects a Workspace and shows the Task list or details. This workspace reads the selected Agent's real Task runtime instead of copying state. The current Desktop workspace is read-only; create, edit, run, and delete operations still use Agent actions or the CLI.
...
- the completed result is appended as an assistant message to the associated Session; the Task runtime does not call the Chat Plugin directly
