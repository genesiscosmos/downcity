...
Skill does not declare Config, so it needs no Profile. It appears in the Plugin List under the Plugins Tab and provides three scope entries in its own Sidebar:

- **Workspace** selects a registered Workspace and browses, filters, reads, or removes its `.agents/skills`.
...
After a scope is selected, the Mainview shows its Skill list and details. These management operations run through host `main` actions and are separate from the instruction-only Agent `find/install` actions below. Removing a Skill permanently deletes its directory from the selected scope after confirmation.
