...
`Workspace` 是统一资源容器，同时提供 SessionStore、WorkspaceTools、Env 与可选 Shell。Session 历史和元数据默认保存在同一个项目的 `.downcity` 目录中，也可以被 Workspace 的文件工具正常读取和编辑；SessionStore 不保存 Agent 配置。
