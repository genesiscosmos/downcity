...
PluginRegistry 的投影变化在 Session 检查点生效。单个 Plugin 启动失败只隔离自身，不阻断其他 Plugin。Agent 关闭时停止 Session 与 ActionSchedule；Plugin 的 `connect/disconnect/start/stop` 生命周期由 City 收口。
...
flowchart LR
    ...
    Plugin["City 注册 Plugin"] --> Start["Plugin start"]
    Register --> Scope["连接 Agent/Workspace Scope"]
    Start --> Scope
    Scope --> Running["执行 Session"]
    Running --> Close["city.close()"]
    Close --> Sessions["停止 Agent Sessions"]
    Sessions --> Disconnect["Plugin disconnect"]
    Disconnect --> Stop["Plugin stop"]
    Stop --> Workspace["释放 Workspace / Shell"]
```
