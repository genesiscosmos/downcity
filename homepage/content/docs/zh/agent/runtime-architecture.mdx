...
ActionSchedule 是本地延迟 Plugin Action 调度器。事件存放在当前 AgentWorkspace 的 `schedule.jsonl`，Agent 重启时会把遗留的 running 状态恢复为 pending。它不是分布式调度系统，也不提供跨机器选主。
...
flowchart LR
    ...
    Schedule --> Barrier["内部初始化屏障"]
    Barrier --> Running["执行 Session"]
...
Session 执行前会等待 Agent 的内部初始化屏障。`dispose()` 会释放 Agent、Plugin、Schedule、Workspace 和 Shell 资源，但不会关闭由 `@downcity/agent` 创建的 HTTP/RPC transport。
