...
namespace 可见性由 City 授予，不由 Agent 自己声明。需要收窄时添加 `~/.downcity/plugins/city/config.toml`；没有该文件时上述 namespace 全部可见。
...
`deny` 永远优先，Agent 条目也可以用 `allow` 整体替换默认集合，敏感 namespace 不参与默认集合。配置在下一个 Session step 读取生效，不需要重启。
...
| City Tool | City 授予的 namespace 可见性 | 只读运行时事实；可按 Agent 收窄 namespace |
