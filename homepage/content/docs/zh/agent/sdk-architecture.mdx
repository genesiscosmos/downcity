...
和 Plugin 注册；`Workspace` 拥有项目资源与 Shell；调用 `agent.sessions.create({ workspace })` 后创建
Session 执行边界。
...
flowchart TD
    ...
    Session --> Context["带 Workspace 上下文的 PluginContext"]
    City["City<br/>Storage 与资源容器"] --> Storage["StorageProvider<br/>默认 Memory"]
    Session --> Storage
...
`agent.sessions.create({ workspace })` 组合两个主体，但不混合它们的所有权。Agent 与 Session 内部负责创建：
