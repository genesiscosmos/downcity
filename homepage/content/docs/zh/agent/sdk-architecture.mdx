...
Downcity 将执行主体、项目资源与扩展运行时分开。`Agent` 拥有身份、指令、模型、自定义 Tool 和
Session；`Workspace` 拥有项目资源与 Shell；`City` 持有 Plugin 唯一实例、生命周期与 Agent 集合。
...
flowchart TD
    ...
    City --> Plugins["Plugins<br/>唯一实例、配置投影、生命周期"]
...
- `@downcity/agent` 只依赖 `@downcity/type` 中的中立资源协议。
- `@downcity/city` 依赖 Agent，并通过 Agent 内部绑定注入 Workspace、Tool 与 SessionHooks；Agent 不反向依赖 City。
- 宿主在构造每个 Shell 时选择 Sandbox Provider；City 不理解具体后端。
- Edge Adapter 使用 `@downcity/type/workspace`，不会加载 Node.js 本地实现。
...
Agent 不持有 Plugin。City 按 Plugin ID 持有唯一实例，并把全部能力投影到 Agent/Workspace
execution entry。每次 Plugin Action 都获得当前 `PluginContext`，可以直接使用受限的 Agent、
Workspace、Session 与 Turn 句柄。Plugin 只有 City 级 `initialize/dispose` 生命周期；
Agent/Workspace 只定义当前调用范围。
...
| Package | 拥有 | 不拥有 |
| --- | --- | --- |
| `@downcity/type` | 跨 package 的中立资源协议 | 具体资源实现与生命周期 |
| `@downcity/agent` | Agent、AgentSessions、Session、Store、SessionHooks、模型 Tool Loop | City 组合、Plugin 生命周期、项目资源实现、平台 Sandbox |
| `@downcity/city` | City、Workspace/Shell 实现、Plugin SDK/runtime、Hook 调度、RemoteAgent、HTTP/RPC transport、本地宿主数据 | Agent 执行内核、具体内建 Plugin |
