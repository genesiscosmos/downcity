...
- `@downcity/agent` 只依赖 `@downcity/type` 中的 Workspace、Shell 与 Session Extension 协议。
- `@downcity/city` 依赖 Agent，并向 Agent 投影中立的宿主扩展能力；Agent 不反向依赖 City。
...
| Package | 拥有 | 不拥有 |
| --- | --- | --- |
| `@downcity/type` | Workspace、Shell、Session Extension 等中立协议 | 具体资源实现与生命周期 |
| `@downcity/agent` | Agent、AgentSessions、Session、Store、宿主扩展端口、模型 Tool Loop | City 组合、Plugin 生命周期、项目资源实现、平台 Sandbox |
