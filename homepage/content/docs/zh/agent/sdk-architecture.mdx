...
- `SessionStore`：Session 集合和单 Session 数据的结构化存储分支。
...
LocalFileSystem 是基于 Node.js 的 rooted 文件能力，负责安全路径解析、读写移动、原子覆盖、跨进程 lock 和有界搜索。它既是 WorkspaceTools 的底层能力，也是 SessionStore 的文件原语。
...
SessionStore 不是第二套权限系统，而是同一 FileSystem 上的领域分支。集合级
`SessionStore` 负责 Session 列表和归档，单 Session 的 `SessionDataStore` 负责 Message、
Metadata、Instruction 和附件。它们都不保存 Agent 配置，也不参与 Agent 实例装配。
...
`@downcity/workspace-cloudflare-computer` 将 Cloudflare Durable Object 中的
`@cloudflare/computer` Workspace 接入同一套 Agent Workspace 基类。它负责把远程虚拟文件系统提供给
SessionStore 和 Workspace 生命周期；适配器内部调用 Cloudflare 官方 `createAITools()` 提供文件、目录和发布工具，并包装 Computer Shell 为 `exec`
Tool。Agent 仍然运行模型、Session、Plugin 和 Tool Loop。
...
在 Cloudflare 部署中，通常一个 Durable Object 实例对应一个 Agent 和一个 Workspace。Worker 负责入口
与连接，Worker Shell 或 Cloudflare Container 负责实际命令执行。
...
| `@downcity/city` | HTTP/RPC 服务端与 transport | 本地 Agent 的领域状态 |
