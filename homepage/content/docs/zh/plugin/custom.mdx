...
`setup` 是 City 的装配边界，不是安装脚本，也不是 Plugin 生命周期。每次调用都必须创建新的 Plugin 实例；`context.embassy`、`context.data_path` 和 `context.extensions` 只代表当前宿主提供的能力。需要登录态的 Plugin 应显式检查 `context.embassy` 是否存在。
...
`config.toml` 只保存命名的明文 profile 值，文件权限为 `0600`。CLI 与 Desktop 在需要配置或装配时加载 `setup` 模块的 `schema`，构建表单并脱敏 `writeOnly` 字段。普通安装和 Plugin 列表不会调用 `setup()`。
