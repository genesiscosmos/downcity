...
`DOWNCITY_BUREAU_TOKEN`。也可以直接运行 `fed bureau token create`。Federation 数据库
...
`bureau.bureaus` 等入口使用 `bureau_token` 调用 Federation；`fed bureau token create`
仍由 CLI 生成明文并将用途与 hash 写入 Federation 注册表。
