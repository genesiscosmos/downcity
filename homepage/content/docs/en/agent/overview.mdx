...
The **Agent SDK** (`@downcity/agent`) is the core runtime for executing agent work. An Agent owns identity, model, instructions, Tools, and one `AgentSessions` collection. Each Session may receive a Workspace. `City` owns Workspaces, the Plugin catalog, shared instances, Agent bindings, and transports.
...
- **City** — owns Workspaces, the Plugin catalog, Profile-shared instances, Agent bindings, and unified transports.
...
- **Plugin** — an extension capability provided and owned by City, then bound to an Agent by Plugin/Profile.
