...
The **Agent SDK** (`@downcity/agent`) is the core runtime for executing agent work. An Agent owns identity, model, instructions, plugins, and one `AgentSessions` collection. Each Session may receive a Workspace for project tools and environment. `City` is the lifecycle container for Agent instances inside one host process.
...
When an Agent Session finishes a Turn in the background, Desktop shows a blue unread dot on the Agent in the Chat Sidebar and on the corresponding Session in the conversation Subbar. The system app icon also shows the number of unread Sessions. No unread item is created while you are actively viewing that Session; opening it clears both the dots and the system badge. Multiple completed Turns in the same Session are grouped into one unread item.
