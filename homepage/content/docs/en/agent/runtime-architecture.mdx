...
Namespace visibility is granted by City, not declared by the Agent. Add `~/.downcity/plugins/city/config.toml` when you need to narrow it; without that file every namespace above stays visible.
...
`deny` always wins, an Agent entry may also use `allow` to replace the default set, and Sensitive namespaces are never part of the default set. Changes are read at the next Session step, with no restart needed.
...
| City Tool | City-granted namespace visibility | Read-only runtime facts; namespaces can be denied per Agent |
