...
PluginRegistry projection changes take effect at Session checkpoints. A Plugin startup failure isolates that Plugin without blocking others. Agent disposal stops Sessions and ActionSchedule; City closes the Plugin `connect/disconnect/start/stop` lifecycle.
...
flowchart LR
    ...
    Plugin["City registers Plugin"] --> Start["Plugin start"]
    Register --> Scope["connect Agent/Workspace scope"]
    Start --> Scope
    Scope --> Running["execute Sessions"]
    Running --> Close["city.close()"]
    Close --> Sessions["stop Agent Sessions"]
    Sessions --> Disconnect["Plugin disconnect"]
    Disconnect --> Stop["Plugin stop"]
    Stop --> Workspace["release Workspace / Shell"]
```
