...
Availability follows the same rule as Plugins: everything City registers is available to every Agent, with no per-Agent gating and no configuration file. Namespaces marked sensitive would be excluded from that default set, and none of the namespaces above is.
