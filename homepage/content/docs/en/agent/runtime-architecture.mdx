...
- Default: use the current Agent identity, Agent instruction, Power system, and Session context at execution time.
...
Every Agent receives one read-only `city` Tool: the single entry point for City capabilities. It is organised as **methods**, and one call looks like:
...
| `image` | `models`, `create`, `result` — generate images |
| `sound` | `models`, `asr`, `tts` — transcribe audio and synthesize speech |

Calling `city` with no arguments lists the methods. Omitting `action` lists one method's actions with their arguments, return shape and whether they write. Nothing here returns credential values, and each action declares `read` or `write` so you can see which ones consume quota or change state before calling them.
...
Both use the models available to your Federation account, so there is no separate configuration and nothing to set up. Because there is no configured default, pass `model` explicitly or take it from `image.models` / `sound.models`; `asr` and `tts` fall back to the first suitable model when none is given.

Image generation is a two-step job: `create` returns a `job_id`, `result` reads it. Creating an image consumes quota, so the model must confirm with you first. `result` writes the generated images into the Agent's private directory and returns their local paths; render them with `![](path)`.

Automatic transcription of inbound voice messages belongs to the Chat plugin, which calls the `sound` method when a channel delivers a voice or audio attachment. If transcription fails for one attachment, the message still arrives.
...
| City Tool | Read-only by construction | Answers runtime facts and runs declared `read` actions; `write` actions consume quota or write files under the Agent's directory |
