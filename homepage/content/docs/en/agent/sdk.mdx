...
`Workspace` comes from `@downcity/workspace` and owns project files, search tools, Env, private storage capability, and an optional Shell. It does not own Agent identity, Plugins, or Session semantics. `AgentWorkspace`, returned by `agent.enter(workspace)`, creates the SessionStore inside the Agent-specific private storage scope.
