...
Construction starts plugin lifecycle and background work. Use `await agent.ready()` when an explicit readiness barrier is needed, and `await agent.dispose()` to release resources. HTTP and RPC are supplied by `AgentHTTP` and `AgentRPC` from `@downcity/city`.
