...
`Workspace` comes from `@downcity/workspace` and owns project files, search tools, Env, and an optional Shell. It does not own Agent identity, storage, Plugins, or Session semantics. `agent.sessions.create({ workspace })` creates the SessionStore inside the Agent scope supplied by City Storage.
