...
Create the Bureau and its required one-to-one Server configuration in the Federation control plane, then register its machine token. The create input still accepts `server_url` directly:
...
City resolves the entrypoint from `bureau.server.server_url`, so callers only provide a relative path:
