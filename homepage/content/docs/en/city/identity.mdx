...
plaintext `DOWNCITY_BUREAU_TOKEN` once. You can also run `fed bureau token create` directly.
...
`bureau.bureaus` use `bureau_token` to call Federation; `fed bureau token create` still generates
the plaintext locally and writes the purpose and registry hash.
