...
When you run `city on`, the CLI creates one City, restores every bound Agent from the global DB, resolves each `execution.modelId`, and passes the live model instance to the Agent SDK.
