---
title: Channel Accounts

description: How City manages bot credentials for messaging platforms
---

# Channel Accounts

Messaging platform bot credentials (Telegram, Feishu, QQ) are stored and managed by City, not in individual Agent projects. This keeps secrets centralized and makes it easy to reuse the same bot across multiple projects.

## How it works

1. User adds a bot credential through the City Console
2. City stores the credential securely in `~/.downcity/downcity.db`
3. The credential gets a `channel_account_id`
4. The Agent's Chat Plugin Binding references the credential by ID

## Supported platforms

- Telegram
- Feishu (Lark)
- QQ

## Security

- Credentials are encrypted at rest
- Projects never see the raw token
- Revocation is instant across all projects using the account

## Configuration

```json
{
  "channels": {
    "telegram": {
      "enabled": true,
      "channel_account_id": "telegram-main"
    }
  }
}
```

Continue with:

- [Model Pool](/en/docs/city/model-pool)
- [City Identity](/en/docs/city/identity)
