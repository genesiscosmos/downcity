...
A third-party source directory defines exactly one Plugin. Its `plugin.json` declares `schema_version: 1`, a globally unique `id`, a version, and optional `agent`, `main`, and `renderer` entries. `agent/main` are self-contained ESM files, `renderer` is a single-file ESM bundle that default-exports the only React Mainview component, and at least one entry is required. `package.json` must declare `"type": "module"`, and `README.md` is required. The installed directory keeps only declared artifacts, documentation, an optional icon, and local `config.toml`. Installation never runs dependencies, build scripts, or any third-party entry.

Plugin configuration belongs to named Profiles in `plugins/<plugin_id>/config.toml`. Desktop creates, selects, and removes Profiles in each Plugin page; the Plugin's own Mainview and main actions own the actual configuration. CLI `config --set` only replaces a complete JSON object explicitly. It does not generate a generic form or print credential values. An Agent without `--profile` receives `{}` and never implicitly reads a persisted `default` Profile.
