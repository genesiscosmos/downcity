...
Agents and Workspaces are registered independently, and each Agent persistently binds one Workspace. Agents have no independent started state. `city agent chat` uses RPC while City is running and creates a temporary local City while it is off.
