...
A third-party source directory defines exactly one Plugin. Its `plugin.json` declares `schema_version: 1`, a globally unique `id`, a version, and optional `agent`, `main`, and `renderer` entries. `agent/main` are self-contained ESM files, `renderer` is the single self-contained Mainview HTML, and at least one entry is required. `package.json` must declare `"type": "module"`, and `README.md` is required. The installed directory keeps only declared artifacts, documentation, an optional icon, and local `config.toml`. Installation never runs dependencies, build scripts, or third-party entries.
