...
city web --no-open --port 43128  # do not open a browser and choose a port
```
...
Agents and Workspaces are registered independently. Without `--workspace`, the CLI tries the current daemon Workspace, the registered Workspace matching the current directory, and then a unique registered Workspace. A TTY presents a selector when multiple candidates remain. Non-interactive calls with multiple Workspaces must pass `--workspace` explicitly.
...
You can also use “Configure Env” in `city agent` to select the Workspace used by a running Agent. After a change, City asks affected
