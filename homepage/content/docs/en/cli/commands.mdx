...
A third-party directory defines exactly one Plugin. Its `plugin.json` declares `schema_version: 1`, a globally unique `id`, version, a self-contained ESM entry, and optional `config.schema/defaults`. The entry exports a Plugin constructor as the named `plugin` export. Code and resources live directly in `~/.downcity/plugins/<plugin_id>/` without an `artifact/` layer. Installation never runs `npm install`, build scripts, Plugin lifecycle hooks, or the entry module.
