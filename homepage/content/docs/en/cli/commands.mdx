...
A third-party artifact defines exactly one Plugin. Its v4 `downcity.plugin.json` declares a globally unique `id`, version, self-contained ESM entry, and optional profile Schema. The entry exports one named `plugin` constructor. The final directory is always `~/.downcity/plugins/<plugin_id>/`; installation never runs `npm install`, build scripts, lifecycle scripts, or the Plugin entry.

Plugin configuration belongs to named profiles in `plugins/<plugin_id>/config.toml`. Accounts, channels, endpoints, and other structures are defined directly by that Plugin's profile Schema. Configuration is plaintext with file mode `0600`; fields marked `writeOnly` are redacted in command output. The CLI executes `new PluginType({ config: selected_profile })`.
