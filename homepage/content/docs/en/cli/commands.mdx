...
A third-party directory defines exactly one Plugin. Its `plugin.json` declares `schema_version: 1`, a globally unique `id`, version, and self-contained ESM entry. The entry exports a Plugin constructor as the named `plugin` export; configurable Plugins declare a Zod type through `static type.config`. Code and resources live directly in `~/.downcity/plugins/<plugin_id>/` without an `artifact/` layer. Installation never runs `npm install`, build scripts, or Plugin lifecycle hooks, but it imports the entry to validate the constructor and Zod type.

Plugin configuration belongs to named profiles in `plugins/<plugin_id>/config.toml`. Accounts, channels, endpoints, and other structures are defined directly by that Plugin's Zod type. Configuration is plaintext with file mode `0600`; CLI builds forms from the generated JSON Schema and redacts `writeOnly` fields. The Loader runs Zod `parse()` before invoking the constructor.
