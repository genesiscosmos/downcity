...
A third-party repository must contain `downcity.plugin.json`, any required configuration JSON Schema, and a prebuilt self-contained ESM entry. Installation never runs `npm install`, build scripts, lifecycle scripts, or the Plugin entry. The manifest declares configuration schema/defaults and can declare a complete Resource Item Schema through `resources.schema`.

Plugin configuration belongs to the Agent Binding. Plugin Resources belong to the global Resource Store, while a Binding stores only `resource_ids`. The CLI collects user fields from the Schema, lets an optional Resolver write dynamic `readOnly` fields, and resolves IDs into complete Items before instantiation. The Runtime Factory receives `{ config, resources }`. `writeOnly` fields are stored encrypted and never echoed by CLI output.
