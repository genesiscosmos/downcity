...
Agents and Workspaces are independent global entities. An Agent is identified by `agent_id` and owns its model, Plugin Bindings, and tokens. A Workspace is identified by `workspace_id` and points to a project directory. They are combined explicitly for one runtime and are never stored as a permanent binding. Configuration lives in `~/.downcity/downcity.db`; Global Env lives in `~/.downcity/.env`, Workspace Env lives in the project `.env`, and CLI daemon files live in `~/.downcity/runtimes/<agent_id>/`.
