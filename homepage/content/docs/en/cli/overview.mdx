...
Each Plugin uses its stable ID as `~/.downcity/plugins/<plugin_id>/`, with one plaintext configuration in `config.toml`. A third-party installation contains one `plugin.json`, the `package.json` that establishes its ESM boundary, and optional self-contained `main` and `renderer` entries declared by the manifest—not source or build configuration. City provides every registered Plugin and the same configuration to every Agent; `agent.json` stores no Plugin reference. `downcity.db` stores Workspace indexes, tokens, and platform state, not Agent or Plugin configuration.
...
| `city plugin` | Manage Plugin artifacts, unique configuration, and actions |
