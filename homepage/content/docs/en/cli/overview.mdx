...
- `fed` (alias `downfed`) is the Federation Server Manager. Its TUI manages registered servers, while project commands read `federation.json` from the current project.

## City data model

An Agent is a global entity identified by `agent_id`; its Workspace path is only a mutable binding. Agents, plugin bindings, tokens, and environment variables live in `~/.downcity/downcity.db`. Daemon files live in `~/.downcity/runtimes/<agent_id>/`.
