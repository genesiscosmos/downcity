...
Each Plugin uses its stable ID as `~/.downcity/plugins/<plugin_id>/`. Plaintext Profiles live in `config.toml`. A third-party installation contains one `plugin.json`, the `package.json` that establishes its ESM boundary, and optional self-contained `main` and `renderer` entries declared by the manifest—not source or build configuration. City provides every registered Plugin to every Agent. An optional Plugin/Profile reference in `agent.json` only selects the configuration projected into `PluginContext.config`; it does not own the Plugin instance or lifecycle. `downcity.db` stores Workspace indexes, tokens, and platform state, not Agent or Plugin configuration.
