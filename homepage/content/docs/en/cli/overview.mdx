...
Each Plugin uses its stable ID as `~/.downcity/plugins/<plugin_id>/`. Its plaintext profiles live in `config.toml`; a third-party Plugin's single `plugin.json`, code, and resources live directly in that directory. An Agent registers a Plugin and selects an optional profile in its own `agent.json`. `downcity.db` stores Workspace indexes, tokens, and platform state, not Agent or Plugin configuration.
