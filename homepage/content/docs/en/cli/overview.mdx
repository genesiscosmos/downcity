...
Each Plugin uses its stable ID as `~/.downcity/plugins/<plugin_id>/`. Its plaintext profiles live in `config.toml`; third-party code lives in `artifact/` with a normalized `plugin.json`. An Agent registers a Plugin and selects an optional profile in its own `agent.json`. `downcity.db` stores Workspace indexes, tokens, and platform state, not Agent or Plugin configuration.
