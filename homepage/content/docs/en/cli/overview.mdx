...
Each Plugin uses its stable ID as `~/.downcity/plugins/<plugin_id>/`. Plaintext Profiles live in `config.toml`. A third-party installation contains one `plugin.json`, the `package.json` that establishes its ESM boundary, and the optional self-contained `agent`, `main`, and `renderer` entries declared by the manifest—not source or build configuration. An Agent enables a Plugin with `agent` capability and selects an optional Profile in its own `agent.json`. `downcity.db` stores Workspace indexes, tokens, and platform state, not Agent or Plugin configuration.
