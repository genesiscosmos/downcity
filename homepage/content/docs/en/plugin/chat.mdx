...
Chat is configured like every other Plugin through an Agent Plugin Binding. The CLI generates a shared form from JSON Schema and stores the Binding in the global DB:
...
The actual bot credentials live in the CLI global DB (managed by City). The Chat Binding references them only by `channel_account_id`.
