...
`main` default-exports a `CityPluginModule`. It provides both the execution-instance factory and host main lifecycle; there is no separate `agent` entry. `plugin.action()` registers Profile-independent actions for Sidebar/Mainview, while `plugin.config_action()` registers Config actions:
...
One main is activated once per City, and one `(plugin_id, profile_id)` maps to one shared execution instance. The current Profile is injected into `create()` and config action calls; do not cache it in main-global state.
