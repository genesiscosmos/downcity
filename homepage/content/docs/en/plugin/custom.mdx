...
`main` default-exports a Plugin instance, or `{ plugin, main }` when it also provides a host main
lifecycle. There is no separate `agent` entry. `plugin.action()` registers Sidebar/Mainview actions,
while `plugin.config_action()` registers Config actions:
...
One main is activated once per City, and one Plugin ID maps to one instance. City injects the selected
Profile store into Config actions, while the execution instance reads current Agent configuration
from `PluginContext.config`.
