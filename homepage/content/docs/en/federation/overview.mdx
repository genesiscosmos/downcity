...
Non-interactive environments must also pass `--yes`. A successful recovery revokes every previous administrator session and shows the new password once. Federation exposes no anonymous password-reset API.
