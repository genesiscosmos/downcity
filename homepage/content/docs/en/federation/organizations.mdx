...
Organizations Service supports better-sqlite3, PostgreSQL, and Cloudflare D1. Every runtime exposes
the same `context.transaction` API to services. City Runtime encapsulates D1 snapshot-conflict
detection, retries, and atomic `batch()` commits, so Organizations does not branch on database type.
