...
`variant` supports `neutral`, `zinc`, `slate`, `stone`, and `blue`; `mode` supports `light` and `dark`. Dropdown, Select, Popover, and ContextMenu portals automatically mount inside ThemeContainer, so no separate theme synchronization is required.
