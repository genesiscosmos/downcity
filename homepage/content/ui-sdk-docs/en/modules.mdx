...
The shortcut module provides cross-platform matching, registration, and a React provider. `Mod` means `Command` on macOS and `Ctrl` on Windows/Linux.
...
It supports combinations such as `Mod+B`, `Mod+L`, `Mod+I`, and `Mod+,`. Host applications provide the actions; the UI SDK does not impose business behavior.
...
- Exports: `ChatPanel`, `ChatHistory`, `ChatMessage`, `ChatComposer`, `session_message_to_chat_message`
