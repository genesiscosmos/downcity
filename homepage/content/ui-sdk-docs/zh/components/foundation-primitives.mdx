...
所有原语共用同一组语义变量：`background`、`foreground`、`surface-subtle`、`control-surface`、`divider`、`interaction-hover`、`interaction-selected` 和 `primary`。宿主只需在根节点设置 `data-theme` 即可定义颜色主题，不需要修改任何组件 class。
...
展示站保持页面框架不变，只将该选择应用到组件展示区域。内置 `neutral`、`zinc`、`slate`、`stone`、`blue` 五套预设，且每套都同时支持浅色与深色外观。
