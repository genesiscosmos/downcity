 ...
 *
 * 正文不另开字号档位，直接用**默认档** `--text-base`（0.875rem），
...
test("消息字号只有一个来源，两侧共用；行高按角色各一档", () => {
  ...
  /*
   * **行高按角色分开**，这是有意的：
   * - Agent 正文是长文，1.8 是长段落被验证过的阅读节奏（1.7 被否过）；
  ...
  // 必须等于默认档 base（0.875rem = 14px）。
  assert.equal(size_rem, 0.875, `消息字号不是默认档 base（0.875rem）：${size_rem}rem`);
  /*
