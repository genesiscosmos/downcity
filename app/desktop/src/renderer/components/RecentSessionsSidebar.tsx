...
export function RecentSessionsSidebar({ controller, close_sidebar, embedded = false }: RecentSessionsSidebarProps) {
  ...
  const entries = useMemo(() => [
    ...Object.entries(sessions_by_workspace).flatMap(([workspace_id, sessions]) => sessions.map(({ agent_id, session }) => ({ kind: "agent" as const, workspace_id, agent_id, session_id: session.session_id, title: session.title || "新对话", preview: session.preview_text, updated_at: session.updated_at }))),
    ...groups.flatMap((group) => group.sessions.filter((session) => session.workspace_id).map((session) => ({ kind: "group" as const, group_id: group.group_id, group_name: group.name, workspace_id: session.workspace_id!, session_id: session.session_id, title: session.title || "新对话", preview: session.preview_text, updated_at: session.updated_at }))),
  ].sort((left, right) => right.updated_at - left.updated_at).slice(0, 30), [groups, sessions_by_workspace]);
