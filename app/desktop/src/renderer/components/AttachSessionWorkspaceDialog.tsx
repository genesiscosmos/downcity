    ...
    {creating ? (
      ...
      <>
        ...
        <DialogBody className="flex flex-col gap-2">
          {workspaces.length > 0 ? workspaces.map((workspace) => <button key={workspace.workspace_id} type="button" onClick={() => set_selected_workspace_id(workspace.workspace_id)} className={`flex min-w-0 items-center gap-2 rounded-lg border px-2.5 py-2 text-left text-xs outline-none transition-colors focus-visible:ring-2 focus-visible:ring-ring/30 ${selected_workspace_id === workspace.workspace_id ? "border-primary bg-interaction-selected" : "border-input hover:bg-interaction-hover"}`}><TbFolder className="size-4 shrink-0 text-muted-foreground" /><span className="min-w-0 flex-1 truncate">{workspace.name}</span><span className="truncate text-[0.625rem] text-muted-foreground">{workspace.workspace_path}</span></button>) : <div className="px-2 py-5 text-center text-[0.625rem] text-muted-foreground">{translate("workspace.empty_for_session")}</div>}
          {form_error ? <div className="text-[0.6875rem] text-destructive">{form_error}</div> : null}
        </DialogBody>
