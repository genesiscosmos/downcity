    ...
    {creating ? (
      <form onSubmit={(event) => void submit_create(event)}>
        ...
        <DialogBody className="flex flex-col gap-3">
          ...
          <label className="flex flex-col gap-1.5"><span className="text-2xs font-medium text-foreground">{translate("workspace.name")}</span><input value={create_name} placeholder={translate("workspace.name_placeholder")} className="h-8 w-full rounded-lg border border-input bg-background px-2.5 text-xs text-foreground" onChange={(event) => { set_create_name_edited(true); set_create_name(event.target.value); }} /></label>
