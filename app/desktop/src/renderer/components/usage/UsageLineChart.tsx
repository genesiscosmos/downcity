...
export function UsageLineChart({ series, period, credits_per_usd }: UsageLineChartProps) {
  ...
  return <div className="relative" onPointerLeave={() => set_active_index(null)}>
    ...
    <svg viewBox={`0 0 ${width} ${height}`} className="block h-auto w-full overflow-visible" role="img" aria-label={translate("usage.trend_label", { period: period_label })}>
      ...
      <text x={padding.left - 8} y={padding.top + 3} textAnchor="end" className="fill-muted-foreground/65 text-3xs">{format_amount(maximum)}</text>
      <text x={padding.left - 8} y={padding.top + plot_height + 3} textAnchor="end" className="fill-muted-foreground/65 text-3xs">$0.00</text>
