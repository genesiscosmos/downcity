 ...
 *
 * 图表标识由**源码**派生，不用 `useId`：分段离屏回收会卸载并重建这个组件，而 Mermaid 的
 * 确定性 id 与渲染缓存都需要一个跨重挂载稳定的标识。`useId` 随组件实例变化，用它会让
 * 「滚出去再滚回来」被当成一张新图，渲染缓存也用不上。
 */
...
export function MermaidDiagram({ children }: { /** rehype 转换写入的图表源码。 */ children?: ReactNode }) {
   ...
   * 图表标识：由源码派生，跨组件重挂载稳定。
   *
   * Mermaid 的确定性 id 与渲染缓存都依赖它；若换成 `useId`，分段回收造成的重挂载会被当成新图。
   */
