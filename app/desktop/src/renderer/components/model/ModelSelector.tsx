          ...
          <div className="flex-1 overflow-auto px-1 pb-0.5 pt-1">
            {sorted_models.length === 0 ? <div className="py-4 text-center text-xs text-muted-foreground">{resolved_empty_text}</div> : <div className="space-y-0.5">
              {sorted_models.map((model) => {
                ...
                return <Button key={model.model_id} size="full" className={cn("rounded-item text-foreground hover:bg-interaction-hover", active && "bg-foreground/10 text-foreground hover:bg-foreground/10")} onClick={() => select_model(model.model_id)}>
