...
export function ModelPricingChart({ data }: ModelPricingChartProps) {
  ...
  return <div className="overflow-x-auto">
    <svg viewBox={`0 0 ${width} ${Math.max(80, data.length * row_height + 28)}`} className="block h-auto min-w-[34rem] w-full" role="img" aria-label={translate("pricing.chart_label")}>
      <text x={left_padding} y="12" className="fill-muted-foreground/65 text-[9px]">{translate("pricing.unit")}{logarithmic_scale ? ` · ${translate("pricing.log_scale")}` : ""}</text>
      {data.map((item, index) => {
