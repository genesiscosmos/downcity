...
export function SettingItem({ label, description, leading, children }: { /** 设置名称。 */ label: string; /** 设置说明。 */ description?: string; /** 左侧语义图标。 */ leading?: ReactNode; /** 行尾控件。 */ children: ReactNode }) {
  return <div className="flex min-h-14 items-center justify-between gap-6 px-3.5 py-2.5">{leading ? <span className="flex size-5 shrink-0 items-center justify-center text-muted-foreground [&_svg]:size-4">{leading}</span> : null}<div className="flex min-w-0 flex-1 flex-col gap-0.5"><span className="text-base text-foreground">{label}</span>{description ? <p className="text-xs text-muted-foreground">{description}</p> : null}</div><div className="flex max-w-[55%] shrink-0 justify-end">{children}</div></div>;
}
