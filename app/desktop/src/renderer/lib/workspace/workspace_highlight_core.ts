 ...
 * Workspace 源码视图的着色核心。
 *
 * 只依赖 Shiki，不触碰 DOM，也不关心自己运行在哪个线程：主线程与 Web Worker 都直接引用它，
 * 因此两条路径上的着色行为完全一致。线程调度与失败回退在 `workspace_code_highlighter.ts`。
 *
 * 四个取舍：
 ...
 * 超过此字符数就不着色。
 *
 * 预览上限是 2 MB，但着色的开销远大于渲染纯文本，且发生在主线程：命中小概率的大文件会拖住界面。
 * 宁可对超大文件退回纯文本。
 */
 ...
 * 判断这次请求是否值得进入着色。
 *
 * 空内容、非法行数与超限文件都直接跳过；调度层据此提前返回，省去一次 Worker 往返。
 */
export function should_skip_highlight(code: string, expected_line_count: number): boolean {
  return !code || expected_line_count <= 0 || code.length > highlight_max_characters;
}
 ...
 */
export async function highlight_code_lines_core(code: string, language: WorkspaceLanguage, expected_line_count: number): Promise<HighlightedToken[][] | undefined> {
