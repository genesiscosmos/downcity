...
function ModelSettingsExact({ controller }: { controller: DesktopViewController }) {
  ...
  return <SettingsContainer>{!controller.user.authenticated ? <SettingSection title="模型"><SettingGroup><div className="flex items-center justify-between gap-4 rounded-md p-2"><div className="min-w-0"><div className="text-sm text-foreground/90">登录后管理模型</div><div className="mt-0.5 text-xs text-muted-foreground">登录 Federation 后才能读取文本和生图模型。</div></div><Button onClick={() => controller.open_settings("user")} disabled={controller.loading}><TbLogin2 />登录</Button></div></SettingGroup></SettingSection> : <>
    <SettingSection title="文本模型" action={<Button title="刷新模型" aria-label="刷新模型" disabled={controller.models_loading} onClick={() => void controller.refresh_models()}>{controller.models_loading ? <TbLoader2 className="animate-spin" /> : <TbRefresh />}刷新</Button>}><CollapsibleModelGroup title={default_text?.name || "暂无文本模型"} count={text_models.length} model={default_text} models={text_models} active_model_id={controller.settings.default_text_model_id} on_select={(model_id) => void controller.update_settings({ default_text_model_id: model_id })} empty_text="暂无文本模型" /></SettingSection>
    <SettingSection title="生图模型"><CollapsibleModelGroup title={default_image?.name || "暂无生图模型"} count={image_models.length} model={default_image} models={image_models} active_model_id={controller.settings.default_image_model_id} on_select={(model_id) => void controller.update_settings({ default_image_model_id: model_id })} empty_text="暂无生图模型" /></SettingSection>
  </>}</SettingsContainer>;
}
function CollapsibleModelGroup({ title, count, model, models, active_model_id, on_select, empty_text }: { title: string; count: number; model?: DesktopViewController["models"][number]; models: DesktopViewController["models"]; active_model_id: string; on_select(model_id: string): void; empty_text: string }) { const [expanded, set_expanded] = useState(false); return <div className="overflow-hidden rounded-lg bg-surface-subtle"><button type="button" className="flex min-h-12 w-full items-center gap-2 px-3.5 py-2.5 text-left outline-none transition-colors hover:bg-interaction-hover focus-visible:bg-interaction-hover" onClick={() => set_expanded((current) => !current)}><TbChevronRight className={cn("size-3.5 shrink-0 text-muted-foreground transition-transform", expanded && "rotate-90")} />{model ? <LLMModelIcon model_id={model.model_id} size_class="size-4" /> : null}<span className="min-w-0 flex-1 truncate text-xs text-foreground/90">{title}</span><span className="text-[10px] tabular-nums text-muted-foreground/70">{count}</span></button>{expanded ? <div className="divide-y divide-divider border-t border-divider">{models.length ? models.map((item) => <ModelRowExact key={item.model_id} model={item} active={item.model_id === active_model_id || (!active_model_id && item.model_id === model?.model_id)} on_select={() => on_select(item.model_id)} />) : <div className="flex min-h-16 items-center justify-center px-3 py-3 text-center text-xs text-muted-foreground/70">{empty_text}</div>}</div> : null}</div>; }
function ModelRowExact({ model, active, on_select }: { model: DesktopViewController["models"][number]; active: boolean; on_select(): void }) { return <div className={cn("group flex h-9 w-full items-center gap-2 px-3.5 transition-colors hover:bg-interaction-hover", active && "bg-interaction-selected hover:bg-interaction-active")}><button type="button" onClick={on_select} aria-pressed={active} className="flex h-full min-w-0 flex-1 items-center gap-2 text-left"><LLMModelIcon model_id={model.model_id} size_class="size-4" /><span className="min-w-0 flex-1 truncate text-xs font-medium text-foreground/90">{model.name}</span></button><div className="flex shrink-0 items-center gap-1">{model.context_window ? <span className="rounded bg-foreground/[0.04] px-1.5 text-[10px] leading-4 text-muted-foreground/75 tabular-nums">{format_context_window(model.context_window)}</span> : null}<TbInfoCircle className="size-3.5 text-muted-foreground/60" /><TbCheck className={cn("size-4 text-foreground transition-opacity", active ? "opacity-100" : "opacity-0")} aria-hidden="true" /></div></div>; }
