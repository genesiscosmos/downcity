...
import { TbFolder, TbMessageCircle, TbPlus } from "react-icons/tb";
...
export function WorkspaceView({ workspace, agents, sessions_by_agent, open_create_agent, select_agent, select_session }: WorkspaceViewProps) {
  ...
  return <MainViewLayout>
    <MainViewHeader title={<span className="flex min-w-0 items-center gap-1.5"><TbFolder /><span className="truncate">{workspace.name}</span></span>} right_actions={<Button variant="primary" onClick={open_create_agent}><TbPlus /><span>创建 Agent</span></Button>} />
