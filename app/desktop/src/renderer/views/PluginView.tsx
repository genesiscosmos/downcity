...
export function PluginView({ plugin, controller }: PluginViewProps) {
  ...
  const [profile_id, set_profile_id] = useState("default");
  ...
  const select_profile = useCallback((next_id: string, next_definition?: DesktopPluginDefinition) => {
    ...
    set_draft(structuredClone(current?.profiles[next_id] ?? current?.default_config ?? {}));
  }, [definition]);
  const load_definition = useCallback(async () => {
    set_loading(true); set_error("");
    try { const next = await controller.get_plugin(plugin.plugin_id); set_definition(next); select_profile(next.profiles[profile_id] ? profile_id : (Object.keys(next.profiles)[0] ?? "default"), next); }
    catch (reason) { set_error(to_error(reason)); } finally { set_loading(false); }
  }, [controller, plugin.plugin_id, profile_id, select_profile]);
  useEffect(() => { set_editor_open(false); set_definition(undefined); set_profile_id("default"); set_draft({}); set_error(""); }, [plugin.plugin_id]);
  ...
  const remove = async () => { set_saving(true); set_error(""); try { const next = await controller.remove_plugin_profile(plugin.plugin_id, profile_id); set_definition(next); select_profile(Object.keys(next.profiles)[0] ?? "default", next); } catch (reason) { set_error(to_error(reason)); } finally { set_saving(false); } };
  const open_editor = () => { set_editor_open(true); if (!definition && !loading) void load_definition(); };
  ...
  return <div className="flex h-full min-h-0 min-w-0 flex-1 bg-background"><MainViewLayout><header className="header-drag-region flex h-10 items-center gap-2 px-3 text-xs"><TbComponents className="text-muted-foreground" /><span className="truncate font-medium text-foreground/80">{plugin.title}</span></header><MainViewBody><div className="min-h-0 min-w-0 flex-1 overflow-y-auto"><div className="mx-auto w-full max-w-[42rem] px-6 pb-12 pt-14">
    ...
    <Group title="Capabilities"><Row label="Enabled Agents" value={plugin.agent_ids.length ? plugin.agent_ids.join(", ") : "None"} /><button type="button" className="grid min-h-11 w-full grid-cols-[1rem_8rem_minmax(0,1fr)_1rem] items-center gap-3 px-3.5 text-left hover:bg-interaction-hover" onClick={open_editor}><TbSettings className="size-4 text-muted-foreground" /><span className="text-[0.6875rem] text-muted-foreground">Configuration</span><span className="truncate text-right text-[0.6875rem] text-foreground/80">{plugin.configurable ? `${plugin.profile_count} profiles` : "Not required"}</span><TbChevronRight className="size-3.5 text-muted-foreground" /></button></Group>
...
function PluginEditor(props: { /** 完整定义。 */ definition?: DesktopPluginDefinition; /** 当前 Profile ID。 */ profile_id: string; /** 配置草稿。 */ draft: JsonObject; /** 是否读取中。 */ loading: boolean; /** 是否提交中。 */ saving: boolean; /** 错误文本。 */ error: string; /** 选择 Profile。 */ select_profile(value: string): void; /** 修改 ID。 */ set_profile_id(value: string): void; /** 修改草稿。 */ set_draft(value: JsonObject): void; /** 保存。 */ save(): Promise<void>; /** 删除。 */ remove(): Promise<void>; /** 重载。 */ reload(): Promise<void>; /** 关闭。 */ close(): void }) {
  ...
  const existing = Boolean(definition?.profiles[profile_id]);
