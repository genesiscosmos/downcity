/** Plugin manifest、profile 与 Agent 引用概览页。 */

import { TbComponents, TbGhost3, TbSettings } from "react-icons/tb";
...
export function PluginView({ plugin }: PluginViewProps) {
  return <MainViewLayout><header className="header-drag-region flex h-10 items-center gap-2 px-3 text-xs"><TbComponents className="text-muted-foreground" /><span className="truncate font-medium text-foreground/80">{plugin.title}</span></header><MainViewBody><div className="min-h-0 min-w-0 flex-1 overflow-y-auto bg-background"><div className="mx-auto w-full max-w-[42rem] px-6 pb-12 pt-14">
    ...
    <section className="mb-7"><h2 className="mb-2 px-1 text-xs font-semibold text-foreground">Plugin</h2><div className="overflow-hidden rounded-lg bg-surface-subtle"><InfoRow label="Plugin ID" value={plugin.plugin_id} /><InfoRow label="Source" value={plugin.source === "builtin" ? "Official" : "Installed"} /><InfoRow label="Profiles" value={String(plugin.profile_count)} /><InfoRow label="Version" value={plugin.version || "Built-in"} last /></div></section>
    <section className="mb-7"><h2 className="mb-2 px-1 text-xs font-semibold text-foreground">Capabilities</h2><div className="overflow-hidden rounded-lg bg-surface-subtle"><CapabilityRow icon={<TbGhost3 />} label="Enabled Agents" value={plugin.agent_ids.length ? plugin.agent_ids.join(", ") : "None"} /><CapabilityRow icon={<TbSettings />} label="Configuration" value={plugin.configurable ? "Supported" : "Not required"} last /></div></section>
