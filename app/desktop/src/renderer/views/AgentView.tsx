...
export function AgentView({ agent, workspaces, plugins, sessions, create_session, select_session }: AgentViewProps) {
  ...
  return <MainViewLayout>
    ...
    <MainViewBody><div className="min-h-0 min-w-0 flex-1 overflow-y-auto bg-background"><div className="mx-auto w-full max-w-[42rem] px-6 pb-12 pt-14">
      <div className="mb-9 flex min-w-0 items-center gap-4"><div className="flex size-12 shrink-0 items-center justify-center rounded-xl bg-surface-subtle text-muted-foreground"><TbRobot className="size-6" /></div><div className="min-w-0"><h1 className="truncate text-lg font-semibold text-foreground">{agent.agent_id}</h1><p className="mt-1 truncate text-xs text-muted-foreground">可进入任意已登记 Workspace</p></div></div>
      <SettingsGroup title="Runtime"><PropertyRow icon={<TbFolder />} label="Workspaces" value={`${workspace_count} available`} /><PropertyRow icon={<LLMModelIcon model_id={agent.model_id} size_class="size-4" />} label="Model" value={agent.model_id || "未配置"} /><PropertyRow icon={<TbRobot />} label="Version" value={agent.version} last /></SettingsGroup>
      <SettingsGroup title="Plugins">{bound_plugins.length > 0 ? bound_plugins.map((plugin, index) => <PropertyRow key={plugin.plugin_id} icon={<TbComponents />} label={plugin.title} value={plugin.plugin_id} last={index === bound_plugins.length - 1} />) : <EmptyRow text="尚未启用 Plugin" />}</SettingsGroup>
