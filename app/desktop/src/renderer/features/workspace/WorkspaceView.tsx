  ...
  return <>
    ...
    <MainViewBody><div className="min-h-0 min-w-0 flex-1 overflow-y-auto"><main className="mx-auto w-full max-w-4xl px-8 pb-12 pt-10">
      <header className="min-w-0 px-1">
        ...
        <div className="mt-2 min-w-0"><p className="truncate font-mono text-2xs text-muted-foreground" title={workspace.workspace_path}>{workspace.workspace_path}</p></div>
      </header>
      <section className="mt-8" aria-labelledby="workspace-readme-title"><div className="mb-2 px-1"><h2 id="workspace-readme-title" className="text-xs text-muted-foreground">README.md</h2></div><article className="min-h-32 rounded-surface bg-surface-subtle px-6 py-5 text-base leading-[1.7] text-foreground">{workspace.readme ? <Markdown text={workspace.readme} mode="static" /> : <div className="flex min-h-24 items-center justify-center text-xs text-muted-foreground">{translate_resources("workspace.no_readme")}</div>}</article></section>
