...
function UserSettingsExact({ controller }: { controller: DesktopController }) {
  ...
  return <SettingsContainer>
    {!account.authenticated ? <><section className="flex min-h-56 flex-col items-center justify-center rounded-lg bg-surface-subtle px-6 py-10 text-center"><span className="flex size-11 items-center justify-center rounded-full bg-surface-emphasis text-muted-foreground"><TbUser className="size-5" /></span><h2 className="mt-4 text-base font-medium text-foreground">{translate("account.no_current")}</h2><p className="mt-1.5 max-w-72 text-xs leading-5 text-muted-foreground">{translate("account.login_description")}</p></section><AccountLoginPanel controller={controller} /></> : <>
