...
function UserSettingsExact({ controller }: { controller: DesktopController }) {
  ...
  return <SettingsContainer>
    {!account.authenticated ? <><section className="flex min-h-56 flex-col items-center justify-center rounded-lg bg-surface-subtle px-6 py-10 text-center"><span className="flex size-11 items-center justify-center rounded-full bg-foreground/[0.05] text-muted-foreground"><TbUser className="size-5" /></span><h2 className="mt-4 text-sm font-medium text-foreground/90">{translate("account.no_current")}</h2><p className="mt-1.5 max-w-72 text-xs leading-5 text-muted-foreground">{translate("account.login_description")}</p></section><AccountLoginPanel controller={controller} /></> : <>
      ...
      <SettingSection title="Credits"><div className="overflow-hidden divide-y divide-divider rounded-lg bg-surface-subtle"><CreditsRow name="Primary Credits" amount={resources?.credits ? format_credits(resources.credits.available_credits) : "—"} action={<div className="flex items-center gap-1"><Button disabled={!resources?.credits}><TbTicket />{translate("account.redeem")}</Button><Button variant="primary" disabled={!resources?.credits}><TbCoin />{translate("account.top_up")}</Button></div>} />{resources?.credits?.cards.filter((card) => card.kind === "ephemeral").map((card) => <CreditsRow key={card.card_id} name={card.name} description={card.expires_at ? translate("account.expires", { date: format_date(card.expires_at) }) : undefined} status={card.status === "active" ? undefined : card.status === "depleted" ? translate("account.depleted") : translate("account.expired")} urgent={card.status === "active" && Boolean(card.expires_at && new Date(card.expires_at).getTime() - Date.now() < 7 * 86400000)} amount={format_credits(card.credits)} />)}</div></SettingSection>
      <UsagePanelExact resources={resources} />
      <SettingSection title={translate("account.details")}><SettingGroup><AccountDetailRow label="Federation" value={account.federation_url} /><AccountDetailRow label={translate("account.user_id")} value={account.user_id || "—"} /><AccountDetailRow label={translate("account.recent")} value={account.user_id ? translate("account.current") : "—"} /></SettingGroup></SettingSection>
...
function AccountLoginPanel({ controller, on_completed }: { controller: DesktopController; on_completed?: () => void }) {
  ...
  if (step === "federation") {
    return <section className="overflow-hidden rounded-lg bg-surface-subtle">
      <button type="button" className="flex min-h-10 items-center gap-1 px-3.5 text-xs text-muted-foreground transition-colors hover:text-foreground" onClick={() => { set_error(""); set_step("providers"); }}><TbArrowLeft />{translate("account.back")}</button>
      <label className="block border-y border-border/45 px-3.5 py-2.5"><span className="block text-xs text-muted-foreground">{translate("account.federation_address")}</span><input autoFocus type="url" className="mt-1 h-8 w-full rounded-md bg-background px-2 text-xs ring-1 ring-border" value={federation_input} placeholder="https://example.com" disabled={loading} onChange={(event) => { set_federation_input(event.target.value); set_error(""); }} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void connect_federation(); } }} /></label>
  ...
  return <section className="overflow-hidden rounded-lg bg-surface-subtle">
    ...
    <Button size="full" className="h-9 gap-2 rounded-none border-t border-border/45 px-3 text-xs" disabled={Boolean(starting_provider_id)} onClick={() => { set_federation_input(""); set_error(""); set_step("federation"); }}><TbPlugConnected /><span>{translate("account.other_federation")}</span><TbArrowRight className="ml-auto" /></Button>
  </section>;
}
...
function UsagePanelExact({ resources }: { resources?: DesktopAccountResources }) {
  ...
  return <SettingSection title={translate("usage_panel.title")} description={period_label} action={<SegmentedControl<UsagePeriod> value={period} options={usage_period_options} aria_label={translate("usage_panel.period")} on_value_change={set_period} />}>
    <div className="overflow-hidden rounded-lg bg-surface-subtle">
      <div className="grid grid-cols-2 divide-x divide-y divide-divider sm:grid-cols-4 sm:divide-y-0"><UsageMetric label={translate("usage_panel.total_credits")} value={format_credits_as_usd(total_credits, credits_per_usd)} /><UsageMetric label={period_credits_label} value={format_credits_as_usd(period_summary.credits_used, credits_per_usd)} /><UsageMetric label={translate("usage_panel.total_tokens")} value={format_compact(total_tokens)} /><UsageMetric label={period_tokens_label} value={format_compact(period_summary.total_tokens)} /></div>
      <div className="border-t border-divider px-3.5 py-4"><p className="mb-3 text-[11px] text-muted-foreground">{translate("usage_panel.activity")}</p><UsageHeatmap heatmap={heatmap} credits_used={heatmap_credits_used} credits_per_usd={credits_per_usd} /></div>
      <div className="border-t border-divider px-3.5 py-4"><p className="mb-2 text-[11px] text-muted-foreground">{translate("usage_panel.trend")}</p><UsageLineChart series={trend} period={period} credits_per_usd={credits_per_usd} /></div>
    </div>
  </SettingSection>;
}
...
function GeneralSettings({ controller, open_global_env }: { /** Renderer 稳定控制器。 */ controller: DesktopController; /** 打开 Global Env 编辑器。 */ open_global_env(): void }) {
  ...
  return <SettingsContainer>
    ...
    <SettingSection title={translate("language.title")}>
      <SettingGroup>
        <SettingRow label={translate("language.title")} description={translate("language.description")}>
    ...
    <SettingSection title={translate("general.startup")}>
      <SettingGroup>
        <SettingRow label={translate("general.default_agent")} description={translate("general.default_agent_description")}>
        ...
        <SettingRow label={translate("general.open_empty_chat")} description={translate("general.open_empty_chat_description")}><SettingSwitch checked={settings.open_empty_chat_on_start} label={translate("general.open_empty_chat")} on_change={(checked) => void controller.actions.update_settings({ open_empty_chat_on_start: checked })} /></SettingRow>
      </SettingGroup>
    </SettingSection>
    <SettingSection title={translate("network.title")} description={translate("network.description")}>
      <SettingGroup>
        <SettingRow label={translate("network.enabled")} description={translate("network.enabled_description")}><SettingSwitch checked={settings.proxy_enabled} label={translate("network.enabled")} on_change={(checked) => void controller.actions.update_settings({ proxy_enabled: checked })} /></SettingRow>
        <SettingRow label={translate("network.address")} description={translate("network.address_description")}>
...
function AppearanceSettings({ controller }: { /** Renderer 稳定控制器。 */ controller: DesktopController }) {
  ...
  return <SettingsContainer>
    ...
    <SettingSection title={translate("appearance.global")}>
      <SettingGroup>
        <SettingRow label={translate("appearance.mode")} description={translate("appearance.mode_description")}>
          <SegmentedControl
        ...
        <SettingRow label={translate("appearance.color_theme")} description={translate("appearance.color_theme_description")}>
        ...
        <SettingRow label={translate("appearance.ui_scale")} description={translate("appearance.ui_scale_description")}>
...
function ChatSettings({ controller }: { /** Renderer 稳定控制器。 */ controller: DesktopController }) {
  ...
  return <SettingsContainer>
    ...
    <SettingSection title={translate("chat.messages")}>
      <SettingGroup>
        <SettingRow label={translate("chat.show_reasoning")} description={translate("chat.show_reasoning_description")}><SettingSwitch checked={settings.show_reasoning} label={translate("chat.show_reasoning")} on_change={(checked) => void controller.actions.update_settings({ show_reasoning: checked })} /></SettingRow>
        <SettingRow label={translate("chat.auto_scroll")} description={translate("chat.auto_scroll_description")}><SettingSwitch checked={settings.auto_scroll} label={translate("chat.auto_scroll")} on_change={(checked) => void controller.actions.update_settings({ auto_scroll: checked })} /></SettingRow>
      </SettingGroup>
    </SettingSection>
