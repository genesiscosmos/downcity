...
function ModelSettingsExact({ controller }: { controller: DesktopController }) {
  ...
  return <SettingsContainer>{!user.authenticated ? <SettingSection title={translate("sections.models")}><SettingGroup><div className="flex items-center justify-between gap-4 rounded-md p-2"><div className="min-w-0"><div className="text-base text-foreground">{translate("model_catalog.manage_after_login")}</div><div className="mt-0.5 text-xs text-muted-foreground">{translate("model_catalog.login_description")}</div></div><Button onClick={() => controller.actions.open_settings("user")} disabled={loading}><TbLogin2 />{translate("model_catalog.login")}</Button></div></SettingGroup></SettingSection> : <>
