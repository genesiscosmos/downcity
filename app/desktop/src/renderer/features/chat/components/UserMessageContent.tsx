...
import { user_message_text_class_name } from "@/features/chat/components/messages/message_layout";
...
export function UserMessageContent({ message_id, parts }: { /** canonical 用户消息标识。 */ message_id: string; /** 保持发送顺序的 canonical 内容。 */ parts: SessionUserMessagePart[] }) {
  ...
  return <div className="user-message-content" data-chat-selectable-message data-chat-message-id={message_id} data-chat-message-role="user">{parts.map((part) => {
    if (part.type === "text") return part.text ? <div key={part.part_id} className={cn("user-message-text-part", user_message_text_class_name, is_inline_text(part.text) && "is-inline")}><Markdown text={part.text} mode="static" /></div> : null;
