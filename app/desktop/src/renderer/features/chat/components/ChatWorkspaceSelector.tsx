...
export function ChatWorkspaceSelector(props: ChatWorkspaceSelectorProps) {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild><Button className={props.variant === "field" ? "h-9 min-w-52 max-w-72 justify-start gap-2 rounded-lg border border-border-subtle bg-background px-3 text-xs font-normal text-foreground shadow-none hover:bg-interaction-hover" : "h-5 min-w-0 max-w-40 gap-1 rounded-full bg-surface-subtle px-2 text-[0.625rem] font-normal text-muted-foreground hover:bg-interaction-hover hover:text-foreground"} disabled={props.disabled || props.workspaces.length < 2} title={props.disabled ? translate("workspace_runtime.switch_disabled") : current?.name || "Workspace"}><TbFolder className={props.variant === "field" ? "size-4 shrink-0 text-muted-foreground" : "size-3 shrink-0"} /><span className="min-w-0 flex-1 truncate text-left">{current?.name || "Workspace"}</span><TbChevronDown className={props.variant === "field" ? "size-3.5 shrink-0 text-muted-foreground" : "size-2.5 shrink-0"} /></Button></DropdownMenuTrigger>
