...
import type { RespondSessionInteractionInput, SessionAgentInteractionPart, SessionInteractionQuestion } from "@downcity/agent";
...
export function AgentInteraction({ part, respond }: { /** canonical Interaction Part。 */ part: SessionAgentInteractionPart; /** 提交结构化响应。 */ respond(input: RespondSessionInteractionInput): Promise<void> }) {
...
export function resolve_agent_interaction_title(part: SessionAgentInteractionPart): string {
...
function ApprovalCard({ part, respond }: { /** Approval Interaction。 */ part: SessionAgentInteractionPart; /** 提交审批。 */ respond(input: RespondSessionInteractionInput): Promise<void> }) {
  ...
  return <section className="interaction-card approval-interaction" aria-labelledby={`${part.part_id}-title`} aria-busy={Boolean(submitting)}>
    <header className="interaction-card-header"><span id={`${part.part_id}-title`} className="interaction-card-title">{translate("chat:activity.confirmation")}</span><span className="interaction-card-meta">{interaction_status_label(part)}</span></header>
...
function QuestionCard({ part, respond }: { /** Question Interaction。 */ part: SessionAgentInteractionPart; /** 提交回答。 */ respond(input: RespondSessionInteractionInput): Promise<void> }) {
  ...
  return <form className="interaction-card question-interaction" aria-labelledby={`${part.part_id}-title`} aria-busy={submitting} onSubmit={(event) => void submit(event)}>
    <header className="interaction-card-header"><span id={`${part.part_id}-title`} className="interaction-card-title">{request.title || translate("chat:activity.input_required")}</span><span className="interaction-card-meta">{pending ? `${current_index + 1} / ${questions.length}` : interaction_status_label(part)}</span></header>
...
function read_interaction_note(part: SessionAgentInteractionPart): string { const payload = part.response?.payload; if (!payload || typeof payload !== "object" || Array.isArray(payload)) return ""; const record = payload as { note?: unknown; reason?: unknown }; return typeof record.note === "string" ? record.note : typeof record.reason === "string" ? record.reason : ""; }
...
function interaction_payload(request: SessionAgentInteractionPart["request"]): Record<string, unknown> { return request.payload && typeof request.payload === "object" && !Array.isArray(request.payload) ? request.payload as Record<string, unknown> : {}; }
function interaction_questions(request: SessionAgentInteractionPart["request"]): SessionInteractionQuestion[] { const questions = interaction_payload(request).questions; return Array.isArray(questions) ? questions as SessionInteractionQuestion[] : []; }
