...
export function UserMessage({ message, fork_message, rewrite_message, has_later_visible_message, can_use_history_actions, can_replace_session }: { /** canonical User Message。 */ message: SessionUserMessage; /** 创建分支 Session。 */ fork_message(message_id: string): Promise<void>; /** 重写历史用户消息。 */ rewrite_message?(input: DesktopChatRewriteInput): Promise<void>; /** 当前消息之后是否仍有可见内容。 */ has_later_visible_message: boolean; /** 当前是否允许历史操作。 */ can_use_history_actions: boolean; /** 当前 Session 是否允许被替换。 */ can_replace_session: boolean }) {
  ...
  return <article className="group is-user flex w-full items-end justify-end gap-2 py-2">
    <div className="w-full flex justify-end"><div className={cn("user-message-stack flex w-full min-w-0 flex-col items-end gap-0.5", editing ? "max-w-[42rem]" : "max-w-[min(80%,42rem)]")}>
      <div className={cn("ml-auto max-w-full text-sm text-foreground", editing ? "flex w-full flex-col overflow-visible" : "w-fit overflow-hidden rounded-2xl rounded-tr-none bg-surface-subtle px-3 py-2")}>{editing ? <UserMessageRewriteEditor initial_document={initial_document} submitting={submitting} error={rewrite_error} cancel={cancel_editing} submit={confirm_editing} /> : <UserMessageContent message_id={message.message_id} parts={message.parts} />}</div>
      {!editing ? <div className="flex h-6 items-center gap-1"><ChatMessageTimestamp created_at={message.created_at} class_name="mr-0.5 opacity-0 transition-opacity duration-150 group-hover:opacity-100 group-focus-within:opacity-100" /><span className="flex items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100 focus-within:opacity-100">{message.parts.length > 0 && rewrite_message ? <MessageActionButton title={translate_chat("message.edit")} disabled={!can_use_history_actions} on_click={start_editing}><TbPencil /></MessageActionButton> : null}<MessageActionButton title={translate_chat("message.fork")} disabled={forking || !can_use_history_actions} on_click={() => void fork()}>{forking ? <TbLoader2 className="animate-spin" /> : <TbGitBranch />}</MessageActionButton></span></div> : null}
    </div></div>
