...
function ActivityGroup({ parts, message_streaming, respond_interaction }: { /** 聚合的活动 parts。 */ parts: AssistantActivityPart[]; /** 消息是否仍在流式生成。 */ message_streaming: boolean; /** 响应组内 Interaction。 */ respond_interaction(input: RespondSessionInteractionInput): Promise<void> }) {
  ...
  return <details open={force_open || open} onToggle={(event) => handle_details_toggle(event.currentTarget, force_open, set_open)} className={cn("activity-tool-row", tool_parts.some(should_force_open_tool) && "has-change-preview", failed ? "is-failed" : pending_interaction ? "is-waiting" : running ? "is-running" : "is-complete")}>
    <summary className="activity-tool-summary activity-tool-group-summary"><span className="activity-tool-main"><ActivityIcon visual_kind={visual_kind} /><span className="activity-tool-state">{translate_chat(failed ? "activity.failed" : pending_interaction ? "activity.waiting_response" : running ? "activity.running" : "activity.completed")}</span><span className="activity-tool-name">{detail}{parts.length > 1 ? ` (+${parts.length - 1})` : ""}</span><TbChevronRight className="activity-tool-chevron" aria-hidden /></span></summary>
...
function ToolDetails({ part, visual_kind }: { /** Tool part。 */ part: Extract<AssistantActivityPart, { type: "tool" }>; /** Tool 视觉类型。 */ visual_kind: AssistantToolVisualKind }) {
  ...
  return <div className="activity-tool-terminal"><pre className="activity-tool-terminal-output">{input !== undefined ? `${translate("chat:activity.input")}\n${format_value(input)}` : ""}{part.output !== undefined ? `${input !== undefined ? "\n\n" : ""}${translate("chat:activity.output")}\n${format_value(part.output)}` : ""}{part.error ? `${input !== undefined || part.output !== undefined ? "\n\n" : ""}${translate("chat:activity.error")}\n${part.error}` : ""}</pre></div>;
}
...
function ApprovalCard({ part, respond }: { /** Approval interaction。 */ part: SessionAgentInteractionPart; /** 提交审批。 */ respond(input: RespondSessionInteractionInput): Promise<void> }) {
  ...
  return <div className="interaction-card approval-interaction" role="group" aria-label={translate("chat:activity.confirmation")}>
    <header className="interaction-card-header"><span className="interaction-card-title">{translate("chat:activity.confirmation")}</span><span className="interaction-card-meta">{interaction_status_label(part)}</span></header>
...
function QuestionCard({ part, respond }: { /** Question interaction。 */ part: SessionAgentInteractionPart; /** 提交回答。 */ respond(input: RespondSessionInteractionInput): Promise<void> }) {
  ...
  return <form className="interaction-card question-interaction" aria-label={request.title || request.type} onSubmit={(event) => void submit(event)}>
    <header className="interaction-card-header"><span className="interaction-card-title">{request.title || translate("chat:activity.input_required")}</span><span className="interaction-card-meta">{pending ? `${current_index + 1} / ${questions.length}` : interaction_status_label(part)}</span></header>
