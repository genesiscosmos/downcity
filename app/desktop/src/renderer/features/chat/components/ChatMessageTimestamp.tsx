...
export function ChatMessageTimestamp({ created_at, class_name }: { /** 消息创建时间戳，单位为毫秒。 */ created_at: number; /** 调整布局的附加样式。 */ class_name?: string }) {
  ...
  return <time dateTime={new Date(created_at).toISOString()} className={cn("shrink-0 text-3xs font-normal tabular-nums text-muted-foreground", class_name)}>{label}</time>;
}
