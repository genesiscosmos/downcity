...
/** 判断完整 Chat 缓存键是否属于指定 Workspace。 */
export function is_workspace_chat_key(key: string, workspace_id: string): boolean {
  return get_workspace_chat_key_prefixes(workspace_id).some((prefix) => key.startsWith(prefix));
}
