...
import { WorkspaceFileMarkdownBody, WorkspaceFilePlaceholder, WorkspaceFileSourceBody, WorkspaceFileViewModeControl, use_workspace_text_file, type WorkspaceFileViewMode } from "@/features/workspace/components/WorkspaceFilePreview";
 ...
 */
export function ChatFilePanelProvider({ view_key, workspace_id, children }: {
  ...
  const open_file = useCallback<ChatFilePanelOpen>((relative_path, line) => {
    ...
    open_baybar(files_tab(view_key, workspace_id, relative_path, translate_chat), FILE_SECTION_ID);
  }, [open_baybar, translate_chat, view_key, workspace_id]);
 ...
 */
export function files_tab(view_key: string, workspace_id: string, relative_path: string, t_chat: BayBarTranslate): BayBarTab {
  return {
    ...
    sections: [{
      ...
      content: <ChatFilesTab view_key={view_key} workspace_id={workspace_id} />,
    }],
  };
}
 ...
 */
export function ChatFilesTab({ view_key, workspace_id }: {
  /** 所属 Chat 的稳定标识。 */
  view_key: string;
  ...
  return <ChatFilePanelContent workspace_id={workspace_id} file={file} />;
}

/** 「文件」域内容：当前打开文件的只读预览，未打开时为引导空态。 */
export function ChatFilePanelContent({ workspace_id, file }: {
  /** 当前对话所属 Workspace。 */
  workspace_id: string;
  /** 当前打开的文件；为空时展示空态。 */
  file?: ChatPanelFile;
  ...
  const translate_chat = use_translation("chat");
  const [view_mode, set_view_mode] = useState<WorkspaceFileViewMode>("preview");
  ...
  // 带行号的链接指向源码位置，默认落在源码视图。
  useEffect(() => set_view_mode(line ? "source" : "preview"), [relative_path, line]);
