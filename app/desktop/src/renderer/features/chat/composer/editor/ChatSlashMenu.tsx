...
import { TbAdjustments, TbCode, TbFile, TbPhoto, TbTrash } from "react-icons/tb";
import { MenuItemShell } from "@/components/ui/item";
import type { ChatSlashCommand } from "@/types/ChatComposer";
import { is_plain_enter } from "./chatComposerKeymap";
...
export function ChatSlashMenu({ commands, select_command }: ChatSlashMenuProps) {
  ...
  return <div className="absolute bottom-full left-1 z-30 mb-2 w-56 overflow-hidden rounded-surface border border-border bg-background p-1 text-popover-foreground outline-none">
    {commands.map((command, index) => {
      const Icon = command.command_id === "attach" ? TbFile : command.command_id === "image" ? TbPhoto : command.command_id === "code" ? TbCode : command.command_id === "clear" ? TbTrash : TbAdjustments;
