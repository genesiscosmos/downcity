...
function ModelOptions({ models, current_model_id, loading, on_select }: { models: DesktopModelSummary[]; current_model_id: string; loading: boolean; on_select(model_id: string): Promise<void> }) {
  ...
  return <div className="flex min-h-0 flex-1 flex-col">
    <div className="flex items-center gap-2 px-2 pb-1">
      ...
      <input ref={input_ref} value={query} aria-label={translate("model.search_placeholder")} placeholder={translate("model.search_placeholder")} spellCheck={false} className="h-6 min-w-0 flex-1 bg-transparent text-xs text-foreground outline-none placeholder:text-muted-foreground/60" onChange={(event) => set_query(event.target.value)} onKeyDown={handle_key_down} />
    </div>
