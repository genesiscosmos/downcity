...
function ModelOptions({ models, current_model_id, loading, on_select }: { models: DesktopModelSummary[]; current_model_id: string; loading: boolean; on_select(model_id: string): Promise<void> }) {
  const translate = use_translation("chat");
  if (loading && models.length === 0) return <div className="flex min-h-12 items-center justify-center text-xs text-muted-foreground"><TbLoader2 className="mr-2 size-4 animate-spin" />{translate("model.loading")}</div>;
  if (models.length === 0) return <div className="px-2 py-4 text-center text-xs text-muted-foreground">{translate("model.empty")}</div>;
    ...
    <PreviewCard.Positioner side="right" align="start" sideOffset={8} collisionPadding={12} className="z-[60] outline-none">
      <PreviewCard.Popup className="w-80 max-w-[calc(100vw-1.5rem)] rounded-xl border border-border bg-background p-3 text-foreground outline-none data-open:animate-in data-closed:animate-out data-open:fade-in-0 data-closed:fade-out-0 data-[side=right]:slide-in-from-left-1 duration-150">
        ...
        <div className="mt-3 flex items-center gap-2 text-[11px]">
          ...
          <span className="ml-auto tabular-nums text-foreground/85">{pricing ? translate("model.price_detail", { input: format_usd(pricing.input_usd_per_1m), output: format_usd(pricing.output_usd_per_1m) }) : translate("model.not_provided")}</span>
        </div>
