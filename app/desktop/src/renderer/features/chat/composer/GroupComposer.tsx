...
export function GroupComposer({ selection, stores, actions }: GroupComposerProps) {
  ...
  const busy = use_store_selector(stores.chat_stream, state => {
    const phase = state.group_phase_by_group[group_id];
  ...
  const commands = useMemo(() => (group?.sessions ?? []).map(session => ({ command_id: `sessions:${session.session_id}`, title: `/sessions ${session.session_id.slice(0, 8)}`, description: "切换 Group Session", keywords: ["session", "sessions", session.session_id], run: () => actions.open_group(group_id, session.session_id) })), [actions, group?.sessions, group_id]);
  return <RichTextEditor editor_key={chat_key} draft_content={draft ?? empty_chat_content}
    placeholder="输入消息，发送给 Group…" busy={busy} spellcheck_enabled={spellcheck_enabled}
