...
export function GroupChatRouteMainView({ selection, controller, sidebar_collapsed }: { /** Group Chat 导航目标。 */ selection: Extract<NavigationTarget, { kind: "group_session" | "group_draft" }>; /** Desktop 稳定控制器。 */ controller: DesktopController; /** 全局 Sidebar 是否折叠。 */ sidebar_collapsed: boolean }) {
  ...
  return <GroupChatMainView group={group} controller={controller} view_key={`group-chat:${group.group_id}:${session_id}`}>
    <GroupChatSurface selection={selection} group={group} session={session} workspaces={workspaces} agents={agents} settings={settings} controller={controller} />
  </GroupChatMainView>;
}

/** Group Chat 的消息、状态和草稿消费边界。 */
export function GroupChatSurface({ selection, group, session, workspaces, agents, settings, controller }: { /** 当前 Group Chat 导航目标。 */ selection: Extract<NavigationTarget, { kind: "group_session" | "group_draft" }>; /** 当前 Group。 */ group: DesktopGroupSummary; /** 当前 Group Session 摘要。 */ session: DesktopGroupSummary["sessions"][number]; /** 可用 Workspace。 */ workspaces: DesktopWorkspaceSummary[]; /** 可用 Agent。 */ agents: DesktopAgentSummary[]; /** Chat 设置。 */ settings: DesktopSettings; /** Desktop 稳定控制器。 */ controller: DesktopController }) {
  const session_id = selection.kind === "group_draft" ? selection.draft_id : selection.session_id;
