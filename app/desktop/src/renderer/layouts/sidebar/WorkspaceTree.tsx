...
export function WorkspaceTree(props: WorkspaceTreeProps) {
  ...
  return <SidebarContent>
    {props.workspaces.map((workspace) => {
      ...
      return <section key={workspace.workspace_id} className="mb-0.5"><div className={cn("group/item flex min-h-8 items-center gap-1 rounded-lg px-1 py-0.5", props.selected_workspace_id === workspace.workspace_id ? "bg-interaction-selected" : "hover:bg-interaction-hover")}>
        <button type="button" aria-label={translate(expanded ? "workspace.collapse" : "workspace.expand")} title={translate(expanded ? "workspace.collapse" : "workspace.expand")} className="flex size-6 shrink-0 cursor-pointer items-center justify-center rounded-md text-muted-foreground outline-none transition-colors hover:bg-interaction-hover hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring/30" onClick={() => void toggle_directory(workspace.workspace_id, "")}><TbChevronRight className={cn("size-3.5 transition-transform", expanded && "rotate-90")} /></button>
        <button type="button" className="flex min-w-0 flex-1 cursor-pointer items-center rounded-md text-left outline-none focus-visible:ring-2 focus-visible:ring-ring/30" onClick={() => props.select_workspace(workspace.workspace_id)} onDoubleClick={() => void toggle_directory(workspace.workspace_id, "")}><span className="min-w-0 flex-1 truncate text-xs font-medium">{workspace.name}</span></button>
...
function DirectoryChildren({ workspace_id, relative_path, depth, entries_by_key, expanded_keys, loading_keys, toggle_directory, select_file }: { /** Workspace 标识。 */ workspace_id: string; /** 当前目录相对路径。 */ relative_path: string; /** 当前缩进深度。 */ depth: number; /** 已加载目录缓存。 */ entries_by_key: Record<string, DesktopWorkspaceEntry[]>; /** 展开节点集合。 */ expanded_keys: Set<string>; /** 正在加载节点集合。 */ loading_keys: Set<string>; /** 切换目录。 */ toggle_directory(workspace_id: string, relative_path: string): Promise<void>; /** 打开文件预览。 */ select_file(workspace_id: string, relative_path: string): void }) {
  const translate = use_translation("resources");
  const key = directory_key(workspace_id, relative_path);
  if (loading_keys.has(key)) return <div className="flex min-h-8 items-center gap-1.5 text-2xs text-muted-foreground" style={{ paddingLeft: depth * 12 }}><TbLoader2 className="size-3 animate-spin" />{translate("workspace.reading")}</div>;
  ...
  return <div className="flex flex-col gap-0.5">{entries.map((entry) => {
    ...
    if (entry.kind === "directory") return <div key={entry.relative_path} style={{ paddingLeft: depth * 12 }}><div className="flex min-h-8 items-center gap-1 rounded-lg px-1 py-0.5 text-left text-xs hover:bg-interaction-hover"><button type="button" aria-label={translate(expanded ? "workspace.collapse_directory" : "workspace.expand_directory")} title={translate(expanded ? "workspace.collapse_directory" : "workspace.expand_directory")} className="flex size-6 shrink-0 cursor-pointer items-center justify-center rounded-md text-muted-foreground outline-none transition-colors hover:bg-interaction-hover hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring/30" onClick={() => void toggle_directory(workspace_id, entry.relative_path)}><TbChevronRight className={cn("size-3.5 transition-transform", expanded && "rotate-90")} /></button><button type="button" className="flex min-w-0 flex-1 cursor-pointer items-center gap-1 rounded-md text-left outline-none focus-visible:ring-2 focus-visible:ring-ring/30" onClick={() => void toggle_directory(workspace_id, entry.relative_path)}><TbFolder className="size-4 shrink-0 text-muted-foreground" /><span className="min-w-0 flex-1 truncate">{entry.name}</span></button></div>{expanded ? <DirectoryChildren workspace_id={workspace_id} relative_path={entry.relative_path} depth={depth + 1} entries_by_key={entries_by_key} expanded_keys={expanded_keys} loading_keys={loading_keys} toggle_directory={toggle_directory} select_file={select_file} /> : null}</div>;
    return <div key={entry.relative_path} style={{ paddingLeft: depth * 12 }}><button type="button" className="flex min-h-8 w-full items-center gap-1.5 rounded-lg px-1 py-0.5 text-left text-xs outline-none hover:bg-interaction-hover focus-visible:ring-2 focus-visible:ring-ring/30" onClick={() => select_file(workspace_id, entry.relative_path)}><TbFile className="size-4 shrink-0 text-muted-foreground" /><span className="min-w-0 flex-1 truncate">{entry.name}</span></button></div>;
  })}{entries.length === 0 ? <div className="min-h-8 p-0.5 text-2xs leading-7 text-muted-foreground" style={{ marginLeft: depth * 12 }}>{translate("workspace.empty_directory")}</div> : null}</div>;
}
