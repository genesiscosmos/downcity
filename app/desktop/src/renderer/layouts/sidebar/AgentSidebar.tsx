...
export function AgentSidebar({ controller, open_create_agent }: AgentSidebarProps) {
  ...
  return <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
    ...
    <GroupEditorDialog open={create_group_open} close_dialog={() => set_create_group_open(false)} agents={controller.agents} workspaces={controller.workspaces} create_group={controller.create_group} />
    <GroupEditorDialog open={Boolean(editing_group)} close_dialog={() => set_editing_group(null)} group={editing_group || undefined} agents={controller.agents} workspaces={controller.workspaces} update_group={controller.update_group} />
  </div>;
}

/** 创建运行时 Group 的最小配置对话框。 */
function GroupEditorDialog({ open, close_dialog, group, agents, workspaces, create_group, update_group }: { open: boolean; close_dialog(): void; group?: DesktopGroupSummary; agents: DesktopViewController["agents"]; workspaces: DesktopViewController["workspaces"]; create_group?: (input: DesktopCreateGroupInput) => Promise<void>; update_group?: (group_id: string, input: DesktopUpdateGroupInput) => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    set_workspace_id(group?.workspace_id || "");
  }, [group, open]);
  const submit = async () => {
    if ((!group_id.trim() && !group) || member_agent_ids.length === 0) return;
    if (group && update_group) await update_group(group.group_id, { name: name.trim() || group.group_id, instruction, member_agent_ids, workspace_id: workspace_id || undefined });
    else if (create_group) await create_group({ group_id: group_id.trim(), name: name.trim() || group_id.trim(), instruction, member_agent_ids, workspace_id: workspace_id || undefined });
    set_group_id("");
    set_name("");
    set_instruction("");
  ...
  return <Dialog open={open} onOpenChange={(value) => { if (!value) close_dialog(); }}><DialogContent><DialogHeader><DialogTitle>{group ? "编辑 Group" : "创建 Group"}</DialogTitle><DialogDescription>{group ? "修改 Group 的名称、目标、成员和共享 Workspace。" : "选择成员和共享 Workspace 后创建 Group。"}</DialogDescription></DialogHeader><DialogBody className="space-y-3 px-3 pb-3">{group ? <div className="rounded-md bg-muted px-3 py-2 text-xs text-muted-foreground">ID：{group.group_id}</div> : <input value={group_id} onChange={(event) => set_group_id(event.target.value)} placeholder="Group ID" className="h-9 w-full rounded-md border border-border bg-background px-3 text-sm" />}<input value={name} onChange={(event) => set_name(event.target.value)} placeholder="Group 名称" className="h-9 w-full rounded-md border border-border bg-background px-3 text-sm" /><textarea value={instruction} onChange={(event) => set_instruction(event.target.value)} placeholder="协作目标（可选）" className="min-h-20 w-full resize-none rounded-md border border-border bg-background px-3 py-2 text-sm" /><select value={workspace_id} onChange={(event) => set_workspace_id(event.target.value)} className="h-9 w-full rounded-md border border-border bg-background px-2 text-sm"><option value="">不绑定 Workspace（内存执行）</option>{workspaces.map((workspace) => <option key={workspace.workspace_id} value={workspace.workspace_id}>{workspace.name}</option>)}</select><div className="space-y-1">{agents.map((agent) => <label key={agent.agent_id} className="flex items-center gap-2 rounded-md px-2 py-1.5 text-xs hover:bg-muted"><input type="checkbox" checked={member_agent_ids.includes(agent.agent_id)} onChange={(event) => set_member_agent_ids((current) => event.target.checked ? [...current, agent.agent_id] : current.filter((item) => item !== agent.agent_id))} />{agent.agent_id}</label>)}</div><div className="flex justify-end gap-2"><Button variant="default" onClick={close_dialog}>取消</Button><Button variant="primary" disabled={(!group && !group_id.trim()) || member_agent_ids.length === 0} onClick={() => void submit()}>{group ? "保存" : "创建"}</Button></div></DialogBody></DialogContent></Dialog>;
}
