...
export function ChatSubjectList(props: ChatSubjectListProps) {
  ...
  return <SidebarContent class_name="space-y-1">
    {props.agents.map((agent) => <AgentSubject key={agent.agent_id} agent={agent} controller={props.controller} active={agent.agent_id === props.selected_agent_id} active_workspace_id={props.active_workspace_id} workspaces={props.workspaces} unread={has_unread_agent_notification(props.notification_state, agent.agent_id)} />)}
    {props.groups.map((group) => <GroupSubject key={group.group_id} group={group} controller={props.controller} active={group.group_id === props.selected_group_id} active_workspace_id={props.active_workspace_id} workspaces={props.workspaces} agents={props.agents} open_group_config={props.open_group_config} />)}
...
const AgentSubject = memo(function AgentSubject({ agent, controller, active, active_workspace_id, workspaces, unread }: { /** Agent 摘要。 */ agent: DesktopAgentSummary; /** 稳定控制器。 */ controller: DesktopController; /** 是否为当前 Agent。 */ active: boolean; /** 当前 Workspace。 */ active_workspace_id: string; /** Workspace 列表。 */ workspaces: DesktopWorkspaceSummary[]; /** 当前 Agent 是否存在未读结果。 */ unread: boolean }) {
  const translate = use_translation("navigation");
    ...
    <button type="button" aria-current={active ? "page" : undefined} className="flex min-w-0 flex-1 items-center gap-2.5 text-left outline-none focus-visible:ring-2 focus-visible:ring-ring/30" onClick={on_select}>
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className={cn("mt-1 flex h-3.5 min-w-0 items-center truncate text-[10px] leading-3.5", status_active ? "font-medium text-muted-foreground" : "text-muted-foreground/70")}>
          ...
          {unread ? <span className="sr-only">{translate_chat("conversation.unread_result")}</span> : null}
        </span>
      </span>
    </button>
