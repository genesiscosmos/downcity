...
import { TbChevronDown, TbChevronUp, TbDots, TbEdit, TbGhost3, TbLoader2, TbMessageCircle, TbPlus, TbTrash, TbUsers } from "react-icons/tb";
  ...
  return <div ref={sidebar_ref} className="flex min-h-0 flex-1 flex-col overflow-hidden">
    ...
    {selected_agent || selected_group ? <section className="relative flex shrink-0 flex-col border-t border-border/35" style={sessions_collapsed ? undefined : { height: sessions_height }}>
      ...
      {!sessions_collapsed ? <div className="sidebar-body-scroll min-h-0 flex-1 space-y-0.5 overflow-y-auto px-2 pb-2">
        ...
        {selected_group ? [...selected_group.sessions].sort((left, right) => right.updated_at - left.updated_at).map((session) => <SessionListRow key={session.session_id} title={session.preview_text || "新对话"} active={controller.selection?.kind === "group_session" && controller.selection.session_id === session.session_id} on_select={() => void controller.open_group(selected_group.group_id, session.session_id)} />) : null}
...
function AgentSubject({ agent, controller }: { /** Agent 摘要。 */ agent: DesktopAgentSummary; /** 根控制器。 */ controller: DesktopViewController }) {
  ...
  return <ChatSubjectItem avatar={<AgentAvatar agent={agent} class_name="size-8 rounded-md" />} title={agent.name} description={running ? <><TbLoader2 className="size-3 animate-spin" />正在回复</> : model_label} active={Boolean(active)} unread={unread} status_active={running} on_select={() => void controller.open_agent_chat(agent.agent_id)} create_disabled={!workspace_id} on_create={() => { if (workspace_id) void controller.create_session(workspace_id, agent.agent_id); }} menu={<DropdownMenu><DropdownMenuTrigger asChild><Button size="icon" className="opacity-0 group-hover:opacity-100 data-[state=open]:opacity-100" title="Agent 操作" aria-label="Agent 操作"><TbDots /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem disabled={!workspace_id} onClick={() => { if (workspace_id) void controller.create_session(workspace_id, agent.agent_id); }}><TbMessageCircle /><span>新对话</span></DropdownMenuItem><DropdownMenuItem onClick={() => controller.select_agent(agent.agent_id)}><TbEdit /><span>Agent 配置</span></DropdownMenuItem></DropdownMenuContent></DropdownMenu>} />;
}
...
function GroupSubject({ group, controller, open_group_config }: { /** Group 摘要。 */ group: DesktopGroupSummary; /** 根控制器。 */ controller: DesktopViewController; /** 打开 Group 配置。 */ open_group_config(group_id: string): void }) {
  ...
  const workspace_id = controller.active_workspace_id || controller.workspaces[0]?.workspace_id;
  return <ChatSubjectItem avatar={<GroupAvatar group={group} agents={controller.agents} />} title={group.name} description={`${group.members.length} 个成员`} active={Boolean(active)} on_select={() => void controller.open_group(group.group_id)} create_disabled={!workspace_id} on_create={() => { if (workspace_id) void controller.create_group_session(group.group_id, workspace_id); }} menu={<DropdownMenu><DropdownMenuTrigger asChild><Button size="icon" className="opacity-0 group-hover:opacity-100 data-[state=open]:opacity-100" title="Group 操作" aria-label="Group 操作"><TbDots /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem disabled={!workspace_id} onClick={() => { if (workspace_id) void controller.create_group_session(group.group_id, workspace_id); }}><TbMessageCircle /><span>新对话</span></DropdownMenuItem><DropdownMenuItem onClick={() => open_group_config(group.group_id)}><TbEdit /><span>Group 配置</span></DropdownMenuItem><DropdownMenuItem className="text-destructive" onClick={() => { if (window.confirm(`确定删除 Group「${group.name}」吗？`)) void controller.remove_group(group.group_id); }}><TbTrash /><span>删除 Group</span></DropdownMenuItem></DropdownMenuContent></DropdownMenu>} />;
}

/** Agent 与 Group 共用的聊天主体行，只负责稳定展示结构和基础交互。 */
function ChatSubjectItem({ avatar, title, description, active, unread = false, status_active = false, on_select, create_disabled, on_create, menu }: { /** 主体头像。 */ avatar: ReactNode; /** 主体名称。 */ title: string; /** 主体状态或说明。 */ description: ReactNode; /** 是否为当前主体。 */ active: boolean; /** 是否存在未读结果。 */ unread?: boolean; /** 副标题是否使用运行中强调色。 */ status_active?: boolean; /** 打开主体。 */ on_select(): void; /** 是否禁止新建对话。 */ create_disabled: boolean; /** 新建主体对话。 */ on_create(): void; /** 主体专属操作菜单。 */ menu: ReactNode }) {
  return <div className={cn("group flex min-h-12 items-center gap-2 rounded-lg border border-transparent px-1.5 py-1 transition-colors duration-150", active ? "bg-primary/[0.1] hover:bg-primary/[0.12]" : "hover:bg-foreground/[0.07]")}>
    <button type="button" className="flex min-w-0 flex-1 items-center gap-2.5 text-left" onClick={on_select}><span className="flex size-8 shrink-0 items-center justify-center overflow-hidden rounded-md">{avatar}</span><span className="min-w-0 flex-1"><span className="flex items-center gap-1.5"><span className="min-w-0 truncate text-xs font-medium text-foreground">{title}</span>{unread ? <span className="size-1.5 shrink-0 rounded-full bg-blue-500" aria-label="有未读完成结果" /> : null}</span><span className={cn("mt-0.5 flex items-center gap-1 truncate text-[10px]", status_active ? "text-primary" : "text-muted-foreground/70")}>{description}</span></span></button>
    <Button size="icon" className="opacity-0 group-hover:opacity-100" title="新对话" aria-label="新对话" disabled={create_disabled} onClick={on_create}><TbPlus /></Button>
    {menu}
  </div>;
}
...
function GroupAvatar({ group, agents }: { /** Group 摘要。 */ group: DesktopGroupSummary; /** Agent 列表。 */ agents: DesktopAgentSummary[] }) {
  ...
  return <span className="relative flex size-8 shrink-0 items-center" aria-label={`${group.name} 成员头像`}>{members.map((agent, index) => <AgentAvatar key={agent.name} agent={agent} class_name={cn("absolute size-6 rounded-md border-2 border-muted", index === 0 && "left-0 top-0", index === 1 && "right-0 top-1", index === 2 && "left-1 bottom-0")} />)}</span>;
}
