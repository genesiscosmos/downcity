...
import { TbDots, TbEdit, TbGhost3, TbLoader2, TbMessageCircle, TbPlus, TbTrash, TbUsers } from "react-icons/tb";
...
export function ChatSidebar({ controller, open_create_agent, open_create_group, open_group_config }: ChatSidebarProps) {
  ...
  return <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
    ...
    <div data-sidebar-scrollable="true" className="sidebar-body-scroll min-h-0 flex-1 space-y-1 overflow-y-auto px-2 pb-2">
      {controller.agents.map((agent) => <AgentSubject key={agent.name} agent={agent} session_count={session_count_by_agent[agent.agent_id] ?? 0} controller={controller} />)}
...
function AgentSubject({ agent, session_count, controller }: { /** Agent 摘要。 */ agent: DesktopAgentSummary; /** Agent Session 数量。 */ session_count: number; /** 根控制器。 */ controller: DesktopViewController }) {
  ...
  return <div className={cn("group flex min-h-12 items-center gap-2 rounded-lg border border-transparent px-1.5 py-1 transition-colors duration-150", active ? "bg-primary/[0.1] hover:bg-primary/[0.12]" : "hover:bg-foreground/[0.07]")}>
    <button type="button" className="flex min-w-0 flex-1 items-center gap-2.5 text-left" onClick={() => void controller.open_agent_chat(agent.agent_id)}><span className="flex size-8 shrink-0 items-center justify-center overflow-hidden rounded-md"><AgentAvatar agent={agent} class_name="size-8 rounded-md" /></span><span className="min-w-0 flex-1"><span className="flex items-center gap-1.5"><span className="min-w-0 truncate text-xs font-medium text-foreground">{agent.name}</span>{unread ? <span className="size-1.5 shrink-0 rounded-full bg-blue-500" aria-label="有未读完成结果" /> : null}</span><span className={cn("mt-0.5 flex items-center gap-1 truncate text-[10px]", running ? "text-primary" : "text-muted-foreground/70")}>{running ? <><TbLoader2 className="size-3 animate-spin" />正在回复</> : `${model_label} · ${session_count} 个对话`}</span></span></button>
...
function GroupSubject({ group, controller, open_group_config }: { /** Group 摘要。 */ group: DesktopGroupSummary; /** 根控制器。 */ controller: DesktopViewController; /** 打开 Group 配置。 */ open_group_config(group_id: string): void }) {
  ...
  return <div className={cn("group flex min-h-12 items-center gap-2 rounded-lg border border-transparent px-1.5 py-1 transition-colors duration-150", active ? "bg-primary/[0.1] hover:bg-primary/[0.12]" : "hover:bg-foreground/[0.07]")}>
    ...
    <Button size="icon" className="opacity-0 group-hover:opacity-100" title="新对话" aria-label="新对话" disabled={!workspace_id} onClick={() => { if (workspace_id) void controller.create_group_session(group.group_id, workspace_id); }}><TbPlus /></Button>
