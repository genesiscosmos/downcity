...
export const WorkspaceSidebar = memo(function WorkspaceSidebar({ controller, open_create_workspace }: WorkspaceSidebarProps) {
  ...
  return <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
    <SidebarHeader title={translate_resources("workspace.title")} actions={<Button size="icon" title={translate_resources("workspace.create")} aria-label={translate_resources("workspace.create")} onClick={open_create_workspace}><TbFolderPlus /></Button>} />
    <div data-sidebar-scrollable="true" className="sidebar-body-scroll min-h-0 flex-1 overflow-y-auto px-2 pb-2">
      {workspaces.map((workspace) => {
        ...
        return <section key={workspace.workspace_id} className="mb-0.5"><div className={cn("group flex min-h-8 items-center gap-1 rounded-lg px-1 py-0.5", selection?.kind === "workspace" && selection.workspace_id === workspace.workspace_id ? "bg-primary/[0.1]" : "hover:bg-foreground/[0.07]")}>
          <button type="button" aria-label={translate_resources(expanded ? "workspace.collapse" : "workspace.expand")} title={translate_resources(expanded ? "workspace.collapse" : "workspace.expand")} className="flex size-6 shrink-0 cursor-pointer items-center justify-center rounded-md text-muted-foreground/70 outline-none transition-colors hover:bg-interaction-hover hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring/30" onClick={() => void toggle_directory(workspace.workspace_id, "")}><TbChevronRight className={cn("size-3.5 transition-transform", expanded && "rotate-90")} /></button>
          <button type="button" className="flex min-w-0 flex-1 cursor-pointer items-center text-left" onClick={() => controller.actions.select_workspace(workspace.workspace_id)} onDoubleClick={() => void toggle_directory(workspace.workspace_id, "")}><span className="min-w-0 flex-1 truncate text-xs font-medium">{workspace.name}</span></button>
          <WorkspaceRowMenu workspace={workspace} on_remove={controller.actions.remove_workspace} />
        </div>{expanded ? <DirectoryChildren workspace_id={workspace.workspace_id} relative_path="" depth={1} entries_by_key={entries_by_key} expanded_keys={expanded_keys} loading_keys={loading_keys} toggle_directory={toggle_directory} select_file={controller.actions.select_workspace_file} /> : null}</section>;
      })}
      {!loading && workspaces.length === 0 ? <div className="flex flex-col items-center px-4 py-10 text-center"><TbFolderPlus className="mb-2 size-5 text-muted-foreground" /><div className="text-xs text-foreground">{translate_resources("workspace.create")}</div><Button className="mt-3" variant="primary" onClick={open_create_workspace}>{translate_resources("workspace.create")}</Button></div> : null}
    </div>
  </div>;
});
...
function DirectoryChildren({ workspace_id, relative_path, depth, entries_by_key, expanded_keys, loading_keys, toggle_directory, select_file }: { /** Workspace 标识。 */ workspace_id: string; /** 当前目录相对路径。 */ relative_path: string; /** 当前缩进深度。 */ depth: number; /** 已加载目录缓存。 */ entries_by_key: Record<string, DesktopWorkspaceEntry[]>; /** 展开节点集合。 */ expanded_keys: Set<string>; /** 正在加载节点集合。 */ loading_keys: Set<string>; /** 切换目录。 */ toggle_directory(workspace_id: string, relative_path: string): Promise<void>; /** 打开文件预览。 */ select_file(workspace_id: string, relative_path: string): void }) {
  const translate_resources = use_translation("resources");
  const key = directory_key(workspace_id, relative_path);
  if (loading_keys.has(key)) return <div className="flex min-h-8 items-center gap-1.5 text-[0.6875rem] text-muted-foreground" style={{ paddingLeft: depth * 12 }}><TbLoader2 className="size-3 animate-spin" />{translate_resources("workspace.reading")}</div>;
  ...
  })}{entries.length === 0 ? <div className="min-h-8 p-0.5 text-[0.6875rem] leading-7 text-muted-foreground/60" style={{ marginLeft: depth * 12 }}>{translate_resources("workspace.empty_directory")}</div> : null}</div>;
}
...
function WorkspaceRowMenu({ workspace, on_remove }: { /** 当前 Workspace。 */ workspace: DesktopWorkspaceSummary; /** 从 Registry 移除 Workspace。 */ on_remove(workspace_id: string): Promise<void> }) {
  ...
  return <>
    <DropdownMenu><DropdownMenuTrigger asChild><Button size="icon" className="size-6 opacity-0 transition-opacity group-hover:opacity-100 data-[state=open]:opacity-100" title={translate_resources("workspace.actions")} aria-label={translate_resources("workspace.item_actions", { name: workspace.name })} onClick={(event) => event.stopPropagation()}><TbDots /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" sideOffset={5} onClick={(event) => event.stopPropagation()}><DropdownMenuItem onClick={() => void copy_path()}><TbCopy /><span>{translate_resources(copied ? "workspace.copied" : "workspace.copy_path")}</span></DropdownMenuItem><DropdownMenuItem onClick={() => void window.downcity.system.open_local_file(workspace.workspace_path)}><TbExternalLink /><span>{translate_resources("workspace.open_finder")}</span></DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem className="text-destructive" onClick={() => { set_remove_error(""); set_remove_open(true); }}><TbTrash /><span>{translate_resources("workspace.remove")}</span></DropdownMenuItem></DropdownMenuContent></DropdownMenu>
    <Dialog open={remove_open} onOpenChange={(next_open) => { if (!removing) set_remove_open(next_open); }}><DialogContent><DialogHeader><DialogTitle>{translate_resources("workspace.remove_title", { name: workspace.name })}</DialogTitle><DialogDescription>{translate_resources("workspace.remove_description")}</DialogDescription></DialogHeader><DialogBody>{remove_error ? <div className="text-xs text-destructive">{remove_error}</div> : <div className="text-xs text-muted-foreground">{translate_resources("workspace.path_detail", { path: workspace.workspace_path })}</div>}</DialogBody><DialogFooter><Button disabled={removing} onClick={() => set_remove_open(false)}>{translate_common("actions.cancel")}</Button><Button className="text-destructive" disabled={removing} onClick={() => void remove()}>{translate_resources(removing ? "workspace.removing" : "workspace.remove")}</Button></DialogFooter></DialogContent></Dialog>
  </>;
}
