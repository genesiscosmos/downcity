    ...
    <div data-sidebar-scrollable="true" className="sidebar-body-scroll min-h-0 flex-1 overflow-y-auto px-2 pb-2">
      {groups.map((group) => {
        ...
        return <section key={group.source} className="mb-3"><h3 className="px-2 pb-1 pt-1 text-[0.625rem] font-medium uppercase text-muted-foreground">{group.label}</h3>{plugins.map((plugin) => {
          const active = controller.selection?.kind === "plugin" && controller.selection.plugin_name === plugin.plugin_name;
          return <button key={plugin.plugin_name} className={cn("flex min-h-8 w-full items-center gap-2 rounded-lg px-2 text-left transition-colors duration-150", active ? "bg-primary/[0.1]" : "hover:bg-foreground/[0.07]")} onClick={() => controller.select_plugin(plugin.plugin_name)}><TbComponents className="size-4 shrink-0 text-muted-foreground" /><span className="min-w-0 flex-1 truncate text-xs text-foreground">{plugin.title}</span>{plugin.agent_ids.length > 0 ? <span className="size-1.5 rounded-full bg-emerald-500" title="已绑定 Agent" /> : null}</button>;
        })}</section>;
      })}
