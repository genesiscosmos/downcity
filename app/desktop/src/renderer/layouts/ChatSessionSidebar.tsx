...
export function GroupChatSessionSidebar({ group, controller, collapsed, toggle_collapsed }: GroupChatSessionSidebarProps) {
  ...
  const workspace_id = controller.active_workspace_id || controller.workspaces[0]?.workspace_id;
  return <ChatSessionSidebarFrame collapsed={collapsed} toggle_collapsed={toggle_collapsed} title={group.name} avatar={<TbMessageCircle className="size-4 text-muted-foreground" />} subject_active={controller.selection?.kind === "group" && controller.selection.group_id === group.group_id} select_subject={() => controller.select_group(group.group_id)} create_session={() => { if (workspace_id) void controller.create_group_session(group.group_id, workspace_id); }} create_disabled={!workspace_id} create_active={controller.selection?.kind === "group_draft" && controller.selection.group_id === group.group_id}>
