...
import { use_store_selector } from "./store/store_types";
import type { Store } from "../types/DesktopStore";
import { use_chat_stream_store } from "./store/use_chat_stream_store";
import { use_catalog_store } from "./store/use_catalog_store";
import { use_navigation_store } from "./store/use_navigation_store";
import { use_session_store } from "./store/use_session_store";
import { use_composer_store } from "./store/use_composer_store";
import { use_settings_store, to_error_message, normalize_global_env_text } from "./store/use_settings_store";
import { use_notification_store } from "./store/use_notification_store";
import { use_desktop_management_actions } from "./controller/use_desktop_management_actions";
import { use_desktop_group_actions } from "./controller/use_desktop_group_actions";
import { use_desktop_navigation_actions } from "./controller/use_desktop_navigation_actions";
import { update_group_session_title } from "../lib/group/group_session_projection";
...
export function use_desktop_controller(): DesktopController {
  ...
  const process_next_queue = useCallback(async (workspace_id: string, agent_id: string, session_id: string): Promise<void> => {
    ...
    if (deleting_session_keys_ref.current.has(session_key) || deleted_session_keys_ref.current.has(session_key) || processing_queue_ref.current.has(session_key) || composer.state_ref.current.queue_paused_by_session[session_key] || is_chat_busy(chat_stream.state_ref.current.chat_runtime_by_session[session_key])) return;
  ...
  }, [chat_stream, commit_queue, composer, settings]);

  // ---- Group 流式订阅 ----
  useEffect(() => {
    const unsubscribe = window.downcity.group.subscribe((event) => {
      if (event.type === "title") {
        catalog.set_groups(catalog.state_ref.current.groups.map((group) => update_group_session_title(group, event.group_id, event.session_id, event.title)));
        const group = catalog.state_ref.current.groups.find((item) => item.group_id === event.group_id);
        if (group) {
  ...
  }, [chat_stream, process_next_queue, session]);
  ...
  const discard_session_render_state = useCallback((source_key: string) => {
    ...
    // 标记为已删除：后续迟到的 mutation / runtime 事件都会被忽略。
  ...
  }, [discard_session_render_state, select_session, session, settings]);
  ...
  }, [discard_session_render_state, navigation, session, settings]);
  ...
  const send_message = useCallback(async (workspace_id: string, agent_id: string, session_id: string, input: JSONContent, mode: ChatSubmitMode = "send", skip_orphan_check = false) => {
    ...
    if (is_draft_session_id(session_id)) {
      try {
        const created_session = await window.downcity.chat.create_session(agent_id, workspace_id);
        session.prepend_session(workspace_id, agent_id, created_session);
        const actual_key = get_session_key(workspace_id, agent_id, created_session.session_id);
        const agent = catalog.state_ref.current.agents.find((item) => item.agent_id === agent_id);
        const draft_configuration = chat_stream.state_ref.current.configuration_by_session[session_key] ?? {
          model_id: settings.state_ref.current.settings.default_text_model_id || agent?.model_id || "",
          approval_mode: "ask",
        };
        navigation.set_selection({ kind: "session", workspace_id, agent_id, session_id: created_session.session_id });
        let actual_configuration = await window.downcity.chat.get_configuration(agent_id, workspace_id, created_session.session_id);
        if (draft_configuration.model_id && draft_configuration.model_id !== actual_configuration.model_id) {
          actual_configuration = await window.downcity.chat.set_model(agent_id, workspace_id, created_session.session_id, draft_configuration.model_id);
        }
        if (draft_configuration.approval_mode !== actual_configuration.approval_mode) {
          actual_configuration = await window.downcity.chat.set_approval_mode(agent_id, workspace_id, created_session.session_id, draft_configuration.approval_mode);
        }
        chat_stream.set_configuration(actual_key, actual_configuration);
        await window.downcity.chat.send(agent_id, workspace_id, created_session.session_id, input);
        composer.remove_draft(actual_key);
      } catch (reason) {
  ...
  }, [catalog, chat_stream, composer, navigation, session, settings]);
