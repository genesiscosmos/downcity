...
import { create_chat_composer, read_chat_composer_text } from "@/lib/chat/editor/chatComposerCodec";
import { update_group_session_title } from "@/lib/group/group_session_projection";
import { get_group_chat_key } from "@/lib/chat/chat_cache_key";
...
import type { use_chat_stream_store } from "../store/use_chat_stream_store";
import type { use_composer_store } from "../store/use_composer_store";
import type { use_navigation_store } from "../store/use_navigation_store";
import type { use_session_store } from "../store/use_session_store";
import type { use_settings_store } from "../store/use_settings_store";
import { to_error_message } from "../store/use_settings_store";
...
interface DesktopGroupDependencies {
  ...
  /** Session 索引能力。 */ session: ReturnType<typeof use_session_store>;
...
export function use_desktop_group_actions(dependencies: DesktopGroupDependencies) {
  ...
  const rename_group_session = useCallback(async (group_id: string, session_id: string, title: string) => {
    try {
      ...
      const group = groups.find((item) => item.group_id === group_id);
      if (group) session.replace_group_sessions(group);
    } catch (reason) { settings.set_error(to_error_message(reason)); throw reason; }
  ...
  const remove_group_session = useCallback(async (group_id: string, session_id: string) => {
    ...
    try {
      ...
      catalog.upsert_group(group); session.replace_group_sessions(group); chat_stream.reset_group_chat(group_id);
  ...
  }, [catalog, chat_stream, navigation, open_group, open_group_draft, session, settings]);
  ...
  }, [active_group_session_ids_ref, catalog, chat_stream, composer, hydrated_navigation_keys_ref, navigation, session, settings]);
  ...
  const create_group = useCallback(async (input: DesktopCreateGroupInput) => {
    ...
    try {
      ...
      catalog.upsert_group(group); session.replace_group_sessions(group);
  ...
  const update_group = useCallback(async (group_id: string, input: DesktopUpdateGroupInput) => {
    settings.set_error("");
    try { const group = await window.downcity.group.update(group_id, input); catalog.upsert_group(group); session.replace_group_sessions(group); chat_stream.reset_group_chat(group_id); }
    catch (reason) { settings.set_error(to_error_message(reason)); throw reason; }
  }, [catalog, chat_stream, session, settings]);
  const remove_group = useCallback(async (group_id: string) => {
    settings.set_error("");
    try {
      await window.downcity.group.remove(group_id);
      catalog.remove_group(group_id); session.remove_group_sessions(group_id); chat_stream.remove_group(group_id); active_group_session_ids_ref.current.delete(group_id);
      const selection = navigation.state_ref.current.selection;
  ...
  }, [active_group_session_ids_ref, catalog, chat_stream, navigation, session, settings]);
