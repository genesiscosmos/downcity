/**
 * Power Renderer ESM 与宿主 React runtime 的受控协议。
 *
 * 协议只暴露当前已安装清单声明且通过完整性校验的 Renderer 单文件、图标，以及
 * 宿主提供的 React/JSX runtime shim。任意其他 Power 文件和目录都不会被响应。
 */
...
const power_renderer_scheme = "downcity-power";
...
/** 在 Electron ready 前登记 Power Renderer 特权协议。 */
export function register_power_renderer_scheme(): void {
  protocol.registerSchemesAsPrivileged([{
    scheme: power_renderer_scheme,
    privileges: {
      standard: true,
      secure: true,
...
export function register_power_renderer_protocol(data: DesktopLocalData): void {
  protocol.handle(power_renderer_scheme, async (request) => {
    ...
    return await installed_power_response(data, url);
  });
}

/** 为已安装 Power 生成随完整性摘要变化的 Renderer URL。 */
export function create_installed_power_renderer_url(
  definition: LocalInstalledPowerDefinition,
): string | undefined {
  if (!definition.renderer) return undefined;
  return `${power_renderer_scheme}://installed/${encodeURIComponent(definition.id)}/${encodeURIComponent(definition.integrity)}/renderer.js`;
}

/** 为已安装 Power 生成受完整性保护的图标 URL。 */
export function create_installed_power_icon_url(
  definition: LocalInstalledPowerDefinition,
): string | undefined {
  if (!definition.icon) return undefined;
  if (/^https?:\/\//iu.test(definition.icon)) return definition.icon;
  return `${power_renderer_scheme}://installed/${encodeURIComponent(definition.id)}/${encodeURIComponent(definition.integrity)}/icon`;
}
...
async function installed_power_response(
  ...
  const [power_id, integrity, resource] = parts;
  const definition = data.powers.get_installed(power_id!);
  if (!definition || definition.integrity !== integrity) {
    return new Response("Not Found", { status: 404 });
  }
  const power_root = data.powers.power_path(definition.id);
  ...
  if (resource === "renderer.js") {
    ...
    const renderer_path = resolve_power_file_path(power_root, definition.renderer.entry);
    return javascript_response(await fs.readFile(renderer_path, "utf8"));
  }
  if (!definition.icon || /^https?:\/\//iu.test(definition.icon)) {
    return new Response("Not Found", { status: 404 });
  }
  const icon_path = resolve_power_file_path(power_root, definition.icon);
  return new Response(new Uint8Array(await fs.readFile(icon_path)), {
    headers: {
      "Access-Control-Allow-Origin": "*",
      "Content-Security-Policy": "default-src 'none'; style-src 'unsafe-inline'; img-src data:",
      "Content-Type": power_icon_content_type(definition.icon),
    },
  });
}

/** 安全解析清单已经声明并通过完整性校验的 Power 文件。 */
function resolve_power_file_path(power_root_input: string, relative_path: string): string {
  const power_root = path.resolve(power_root_input);
  const renderer_path = path.resolve(power_root, relative_path);
  if (renderer_path === power_root || !renderer_path.startsWith(`${power_root}${path.sep}`)) {
    throw new Error("Power renderer must stay inside the Power directory");
  }
  return renderer_path;
}

/** 根据受支持的图标扩展名生成稳定响应类型。 */
function power_icon_content_type(icon_path: string): string {
