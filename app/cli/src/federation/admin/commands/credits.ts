 ...
 *
 * 本模块只通过 EmbassyAdmin Credits typed invoker 管理 Card 与账务记录，不直接理解
 * 支付订单、奖励活动或模型计费策略。
 */
...
/** 打开 Credits 管理循环。 */
export async function manage_credits(a: EmbassyAdmin, _base_url: string, runtime: admin_tui_runtime): Promise<void> {
  while (true) {
...
/** 展示用户 Credits 汇总。 */
async function show_users(a: EmbassyAdmin, runtime: admin_tui_runtime): Promise<void> {
...
/** 展示限时 Cards。 */
async function show_cards(a: EmbassyAdmin, runtime: admin_tui_runtime): Promise<void> {
...
/** 展示 Transactions。 */
async function show_transactions(a: EmbassyAdmin, runtime: admin_tui_runtime): Promise<void> {
  const user_id = await runtime.text(t({ zh: "user_id（可选）", en: "user_id (optional)" }));
...
/** 展示 Card Entries。 */
async function show_history(a: EmbassyAdmin, runtime: admin_tui_runtime): Promise<void> {
  const user_id = await runtime.text(t({ zh: "user_id（可选）", en: "user_id (optional)" }));
...
async function topup_primary(a: EmbassyAdmin, runtime: admin_tui_runtime): Promise<void> {
...
/** 创建限时 Card 并写入初始额度。 */
async function create_ephemeral_card(a: EmbassyAdmin, runtime: admin_tui_runtime): Promise<void> {
  const user_id = await required_text(runtime, "user_id");
...
/** 自动选择 Cards 消费用户额度。 */
async function charge_credits(a: EmbassyAdmin, runtime: admin_tui_runtime): Promise<void> {
  const user_id = await required_text(runtime, "user_id");
  const credits = await required_credits(runtime);
  const source = await required_text(runtime, "source");
  const idempotency_key = await required_text(runtime, "idempotency_key");
